	...
	init = {
		...
		,quickEdit: function(){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("quanjingJson.php?action=getArticleDetail&dopost="+action, "id="+id, function(data){
					if(data != null && data.length > 0){
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("quanjingJson.php?action=updateDetail&dopost="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
			...
			if(checked.length < 1){
				...
				$.dialog({
					...
					ok: function(){
						...
						huoniao.operaJson("quanjingJson.php?action="+type+"Property&dopost="+action, "id="+id+"&attr="+attr, function(data){
							if(data.state == 100){
								huoniao.showTip("success", data.info, "auto");
								$("#selectBtn a:eq(1)").click();
								setTimeout(function() {
		...
		,del: function(type){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("quanjingJson.php?action="+type+"&dopost="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						$("#selectBtn a:eq(1)").click();
						setTimeout(function() {
		...
		,revert: function(){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("quanjingJson.php?action=revert&dopost="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							getList();
			...
			if(checked.length < 1){
				...
				$.dialog({
					...
					ok: function(){
						var typeid = self.parent.$("#typeid").val();
						if(typeid != ""){
							huoniao.operaJson("quanjingJson.php?action=move&dopost="+action, "id="+id+"&typeid="+typeid, function(data){
								if(data.state == 100){
									huoniao.showTip("success", data.info, "auto");
									setTimeout(function() {
										getList();
		...
		,updateState: function(type){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("quanjingJson.php?action=updateState&dopost="+action, "id="+id+"&arcrank="+arcrank, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage(action+"Add", "quanjing", "添加全景", "quanjing/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("edit"+action+id, "quanjing", title, "quanjing/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("quanjingJson.php?dopost="+action, data.join("&"), function(val){
