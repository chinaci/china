$(function(){
	...
	var init = {
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row60 left"><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row60 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
					}
					typeList.push('  <div class="row20"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
					typeList.push('  <div class="row17 left"><a href="javascript:;" class="edit" id="edit_'+jsonArray['id']+'" data-admin="'+jsonArray['admin']+'">编辑</a><a href=javascript:;" class="del">删除</a></div>');
					typeList.push('</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("organizat.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						var adminList = data[1];
						data = data[0];
						huoniao.hideTip();
							...
							ok: function(){
								...
								if($.trim(typename) == ""){
									...
									if(!/^.{2,30}$/.test(typename)){
										$.dialog.alert("请正确填写分类名称\n2-30个汉字");
										return false;
									}
								}

								huoniao.operaJson("organizat.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("organizat.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".ad", "click", function(){
		...
		var type = action == "pic" ? "article" : action;
		try {
			event.preventDefault();
			parent.addPage("articleOrganizatAd"+id, type, "新闻"+txt+"广告", "siteConfig/advList.php?action="+type+"&type="+id);
		} catch(e) {}
	});
	...
	$(".root").delegate(".del", "click", function(event){
		...
		// }else{
			...
			if(type.indexOf("编辑") > -1){
				$.dialog.confirm("确定后，此分类下的子级将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
					huoniao.showTip("loading", "正在删除，，请稍候...");
					huoniao.operaJson("organizat.php?dopost=del&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("organizat.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("organizat.php?dopost=organizatAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
