...
$(function () {
	...
	//开始、结束时间
	$("#starttime, #endtime").datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 3, language: 'ch'});

	//填充栏目分类
	if(atype && typeListArr){
		var list = [];
		list.push('<option value="0">请选择</option>');
		for (var i = 0; i < typeListArr.length; i++) {
			var selected = ""
			if(typeid == typeListArr[i]["directory"]){
				selected = " selected";
			}
			list.push('<option value="'+typeListArr[i]["directory"]+'"'+selected+'>'+typeListArr[i]["tplname"]+'</option>');
		};
		$("#typeid").html(list.join(""));
	}else{
		$("#typeid").html(init.treeTypeList());
	}

	$("#class"+$("#class").val()).show();
	...
	if($("#dopost").val() == "edit"){
		if(classid != ""){
			...
			if(classid == 1){
				...
				$("input[name=style][value="+adbody[0]+"]").click();
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//类型
		if(class_ == "" || class_ == "0"){
			$("#classList").siblings(".input-tips").removeClass().addClass("input-tips input-error").attr("style", "display:inline-block");
			tj = false;
			huoniao.goTop();
			return false;
		}else{
			$("#classList").siblings(".input-tips").removeClass().addClass("input-tips input-ok").attr("style", "display:inline-block");
		}
		...
		if(tj){
			$.ajax({
				type: "POST",
				url: "advAdd.php?action="+action+"&type="+atype,
						...
						if($("#dopost").val() == "save"){
							...
							$.dialog({
								...
								ok: function(){
									try{
										if(atype){
											$("body",parent.document).find("#nav-"+action+"TypeAd"+atype).click();
											parent.reloadPage($("body",parent.document).find("#body-"+action+"TypeAd"+atype));
										}else{
											$("body",parent.document).find("#nav-advListphpaction"+action).click();
											parent.reloadPage($("body",parent.document).find("#body-advListphpaction"+action));
										}
										$("body",parent.document).find("#nav-editAdv"+action+id+" s").click();
									}catch(e){
										location.href = thisPath + "advList.php?action="+action;
									}
								},
								cancel: false
							});
