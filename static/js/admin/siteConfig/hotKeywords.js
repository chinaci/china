$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("hotKeywords.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("hotKeywordsEdit"+id, "siteConfig", title, "siteConfig/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("hotKeywords.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < keywordsList.length; i++){
				...
				var color = "";
				if(keywordsList[i].color != ""){
					color = " style='color:"+keywordsList[i].color+"'";
				}
				var keyword = keywordsList[i].keyword;
				if(keywordsList[i].blod == 1){
					keyword = '<strong>'+keywordsList[i].keyword+'</strong>';
				}
				list.push('  <td class="row25 left"><span'+color+'>'+keyword+'</span></td>');
				list.push('  <td class="row20 left">'+keywordsList[i].href+'</td>');
				list.push('  <td class="row5">'+keywordsList[i].weight+'</td>');
				var state = "";
				switch (keywordsList[i].state) {
					case "0":
						state = '<span class="audit">显示</span>';
						break;
					case "1":
						state = '<span class="refuse">隐藏</span>';
						break;
				}
				list.push('  <td class="row17 state">'+state+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+keywordsList[i].id+'" data-title="'+keywordsList[i].keyword+'" href="hotKeywordsAdd.php?dopost=edit&id='+keywordsList[i].id+'" title="修改" class="edit">修改</a>');
