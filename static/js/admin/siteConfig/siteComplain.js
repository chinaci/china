		...
		init = {
				...
				if(checked.length < 1){
					...
					$.dialog({
						...
						ok: function(){
							...
							huoniao.operaJson("siteComplain.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
								if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("siteComplain.php?dopost=delComplain", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("siteComplain.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < complainList.length; i++){
				...
				list.push('  <td class="row10 left">'+complainList[i].cityname+'</td>');
				...
				info.push('<strong>举报时间：</strong>'+complainList[i].pubdate);
				info.push('<strong>举报理由：</strong>【'+complainList[i].type + "】" + complainList[i].desc);
				...
				var info = [];
				info.push('<strong>举报人：</strong>' + user);
				info.push('<strong>联系方式：</strong>' + complainList[i].phone + '/' + complainList[i].qq);
				...
				var info = [];
				if(complainList[i].state == "已经处理"){
					info.push('处理人：' + complainList[i].opname);
					info.push('处理时间：' + complainList[i].optime);
					info.push('处理结果：' + complainList[i].note);
				}
				list.push('  <td class="row20 left">'+info.join("<br />")+'</td>');
				list.push('  <td class="row10">'+(complainList[i].state == "等待处理" ? '<a href="javascript:;" data-id='+complainList[i].id+'" title="修改" class="edit">修改</a>' : '')+'<a href="javascript:;" data-id='+complainList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
