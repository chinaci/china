$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("personalList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("personalList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("personalEdit"+id, "homemaking", title, "homemaking/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("personalList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, personalList = val.personalList;
		...
		if(val.state == "100"){
			...
			for(i; i < personalList.length; i++){
				list.push('<tr data-id="'+personalList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+personalList[i].userid+'" class="userinfo">'+personalList[i].username+'</a>&nbsp;&nbsp;<a href="'+personalList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(personalList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+personalList[i].zjurl+'" target="_blank">'+personalList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10 left">'+personalList[i].addr+'</td>');
				list.push('  <td class="row5">'+personalList[i].weight+'</td>');
				list.push('  <td class="row12">'+personalList[i].tel+'</td>');
				var state = "";
				switch (personalList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+personalList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+personalList[i].id+'" data-title="'+personalList[i].username+'" href="personalAdd.php?dopost=edit&id='+personalList[i].id+'" title="修改" class="edit">修改</a>');
