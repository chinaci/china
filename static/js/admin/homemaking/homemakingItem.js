$(function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row60 left"><input title="顶级不可修改" type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'" disabled>'+'<span style="display:inline-block;width:50px;margin:0 10px;">ID:'+jsonArray["id"]+'</span>'+addType+'</div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("homemakingItem.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del, .delete", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("homemakingItem.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() { 
							location.reload();
...
function saveOpera(type){
	...
	huoniao.operaJson("homemakingItem.php?dopost=typeAjax&action="+action, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
