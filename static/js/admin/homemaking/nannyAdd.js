$(function(){
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkUser_nanny", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
	...
	$("#btnSubmit").bind("click", function(event){
      ...
      id           = $("#id").val(),
		...
		//异步提交
		huoniao.operaJson("nannyAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "保姆/月嫂添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "保姆/月嫂修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
