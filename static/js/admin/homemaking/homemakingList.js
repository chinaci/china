$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homemakingList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homemakingList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("homemakingEdit"+id, "homemaking", title, "homemaking/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("homemakingList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, homemakingList = val.homemakingList;
		...
		if(val.state == "100"){
			...
			for(i; i < homemakingList.length; i++){
				list.push('<tr data-id="'+homemakingList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20"><a href="'+homemakingList[i].url+'" target="_blank">'+homemakingList[i].title+'</a></td>');
				list.push('  <td class="row15"><span><a href="'+homemakingList[i].storeUrl+'" target="_blank">'+homemakingList[i].store+'</a></span><br><small>'+homemakingList[i].storeaddrname+'</small></td>');
				list.push('  <td class="row6">'+homemakingList[i].typeidname+'<br><small>'+homemakingList[i].addrname+'</small></td>');
				list.push('  <td class="row6">'+homemakingList[i].price+'/'+homemakingList[i].sale+'</td>');
				list.push('  <td class="row10">'+homemakingList[i].username+'</td>');
				list.push('  <td class="row10">'+homemakingList[i].contact+'</td>');
				var state = "";
				switch (homemakingList[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+homemakingList[i].pubdate+'</td>');
				list.push('  <td class="row8">');
				list.push('<a data-id="'+homemakingList[i].id+'" data-title="'+homemakingList[i].title+'" href="homemakingAdd.php?dopost=edit&id='+homemakingList[i].id+'" title="修改" class="edit">修改</a>');
