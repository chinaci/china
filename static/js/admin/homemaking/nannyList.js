$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("nannyList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("nannyList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("nannyEdit"+id, "homemaking", title, "homemaking/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("nannyList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, nannyList = val.nannyList;
		...
		if(val.state == "100"){
			...
			for(i; i < nannyList.length; i++){
				list.push('<tr data-id="'+nannyList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+nannyList[i].username+'</a>&nbsp;&nbsp;<a href="'+nannyList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(nannyList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+nannyList[i].zjurl+'" target="_blank">'+nannyList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10 left">'+nannyList[i].addr+'</td>');
				list.push('  <td class="row5">'+nannyList[i].weight+'</td>');
				var flag = "";
				switch (nannyList[i].certifyState) {
					case "0":
				...
				list.push('  <td class="row15">'+nannyList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+nannyList[i].id+'" data-title="'+nannyList[i].username+'" href="nannyAdd.php?dopost=edit&id='+nannyList[i].id+'" title="修改" class="edit">修改</a>');
