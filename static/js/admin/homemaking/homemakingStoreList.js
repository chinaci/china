		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homemakingStoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("homemakingStoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("storeEdit"+id, "homemaking", title, "homemaking/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("homemakingStoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				list.push('  <td class="row7">'+storeList[i].typename+'</td>');
				var state = "";
				...
				list.push('  <td class="row13">'+storeList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].title+'" href="homemakingStoreAdd.php?dopost=edit&id='+storeList[i].id+'" title="修改" class="edit">修改</a>');
