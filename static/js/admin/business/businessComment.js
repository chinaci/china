		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("businessComment.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("businessComment.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("businessComment.php?dopost=delComment", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("businessComment.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("businessComment.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">商家：<a href="'+commonList[i].storeUrl+'" target="_blank">'+commonList[i].storeTitle+'</a><br />内容：'+commonList[i].content);
