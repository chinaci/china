$(function(){
	...
	$("#manageType").bind("click", function(){
		$.ajax({
			url: "pageElementAdd.php?dopost=manageType",
			type: "POST",
			dataType: "json",
			success: function(data){
				...
				$.dialog({
					...
					ok: function(){
						...
						$.ajax({
							url: "pageElementAdd.php?dopost=saveManageType",
							data: "data=["+data.join(",")+"]",
							type: "POST",
							dataType: "json",
							success: function(data){
				...
				parent.$('.menu-itemlist').delegate("a", "click", function(){
					...
					if(id != ""){
						$.dialog.confirm("确定要删除吗？<br />此操作将同时删除分类下的应用，请谨慎操作！", function(){
							...
							$.ajax({
								url: "pageElementAdd.php?dopost=delManageType",
								data: "id="+id,
								type: "POST"
							});
	...
	$(".themeList").delegate(".icon-trash", "click", function(){
		...
		if(id != ""){
			$.dialog.confirm("确认要删除吗？", function(){
				$.ajax({
					url: "pageElementAdd.php?dopost=delTheme",
					data: "id="+id,
					type: "POST"
				});
				t.parent().remove();
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("pageElementAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-pageElementphp").click();
								parent.reloadPage($("body",parent.document).find("#body-pageElementphp"));
								$("body",parent.document).find("#nav-pageElementEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "pageElement.php";
							}
						},
						cancel: false
					});
