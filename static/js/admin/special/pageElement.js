$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pageElement.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pageElement.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("pageElementAdd", "special", "新增页面元素", "special/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("pageElementEdit"+id, "special", title, "special/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("pageElement.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, pageElement = val.pageElement;
		...
		if(val.state == "100"){
			...
			for(i; i < pageElement.length; i++){
				list.push('<tr data-id="'+pageElement[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row40 left">'+pageElement[i].title+'</td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+pageElement[i].sort+'" class="type">'+pageElement[i].sortName+'</a></td>');
				list.push('  <td class="row5 left">'+pageElement[i].weight+'</td>');
				list.push('  <td class="row15 left">'+pageElement[i].pubdate+'</td>');
				var state = "";
				switch (pageElement[i].state) {
					case "0":
				...
				list.push('<a data-id="'+pageElement[i].id+'" data-title="'+pageElement[i].title+'" href="pageElementAdd.php?dopost=edit&id='+pageElement[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
