$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("shopNews.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$("#typeManage").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopNewsType", type, "商城资讯分类", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopNewsAdd", type, "新增商城资讯", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopNewsEdit"+id, "shop", title, "shop/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("shopNews.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, shopNewsList = val.shopNewsList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < shopNewsList.length; i++){
				list.push('<tr data-id="'+shopNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+shopNewsList[i].title+'</td>');
				list.push('  <td class="row5">'+shopNewsList[i].cityname+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+shopNewsList[i].typeid+'" class="type">'+shopNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+shopNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+shopNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+shopNewsList[i].id+'" data-title="'+shopNewsList[i].title+'" href="shopNews.php?dopost=Edit&id='+shopNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
