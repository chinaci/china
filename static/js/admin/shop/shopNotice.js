$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("shopNotice.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopNoticeAdd", "shop", "新增商城公告", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopNoticeEdit"+id, "shop", title, "shop/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("shopNotice.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < shopNoticeList.length; i++){
				...
				list.push('  <td class="row50 left"><a href="/article.php?id='+shopNoticeList[i].id+'" target="_blank"'+color+'>'+shopNoticeList[i].title+'</a></td>');
				...
				list.push('  <td class="row15"><a data-id="'+shopNoticeList[i].id+'" data-title="'+shopNoticeList[i].title+'" href="shopNotice.php?dopost=Edit&id='+shopNoticeList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
