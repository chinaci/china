$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("productList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("productList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$(".dropdown-toggle").bind("click", function(){
		if($(this).parent().attr("id") != "typeBtn" && $(this).parent().attr("id") != "industryBtn"){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("productEdit"+id, "shop", title, "shop/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">已上架</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">已下架</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("productList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, productList = val.productList;
		...
		if(val.state == "100"){
			...
			for(i; i < productList.length; i++){
				list.push('<tr data-id="'+productList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var flag = "";
				if(productList[i].flag != ""){
					flag = "<span class='append'>（"+productList[i].flag+"）</span>";
				}
				var barcode = '';
				if(productList[i].barcode){
					barcode = "<small><span class='label label-success'>"+productList[i].barcode+"</span></small>";
				}
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+productList[i].litpic+'" class="litpic" /><span>'+barcode+'<a href="'+productList[i].url+'" target="_blank">'+productList[i].title+'</a>'+flag+'</span></td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+productList[i].typeid+'" class="type">'+productList[i].typename+'</a></td>');
				var store = productList[i].store;
				if(productList[i].storeid != 0){
					store = '<a href="'+productList[i].storeUrl+'" target="_blank">'+productList[i].store+'</a>';
				}
				list.push('  <td class="row15 left"><span>'+store+'</span><br><small>'+productList[i].addrname+'</small></td>');
				list.push('  <td class="row7 left">'+productList[i].price+'</td>');
				list.push('  <td class="row7 left">'+productList[i].sales+"/"+productList[i].inventory+'</td>');
				var state = "";
				switch (productList[i].state) {
					case "0":
						state = '<span class="gray">待审核</span>';
						break;
					case "1":
						state = '<span class="audit">已上架</span>';
						break;
					case "2":
						state = '<span class="refuse">已下架</span>';
						break;
				}
				list.push('  <td class="row10 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row12">'+productList[i].pubdate+'</td>');
				list.push('  <td class="row9">');
				list.push('<a data-id="'+productList[i].id+'" data-title="'+productList[i].title+'" href="productAdd.php?action=productAdd&dopost=edit&id='+productList[i].id+'" title="修改" class="edit">修改</a>');
