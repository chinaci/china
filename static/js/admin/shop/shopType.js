$(function(){
	...
	var init = {
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row17 left"><a href="javascript:;" class="edit" title="编辑">编辑</a><a href="javascript:;" class="item" title="属性值">属性值</a><a href="javascript:;" class="del" title="删除">删除</a></div>');
					typeList.push('</div>');
		...
		//规格option
		,speTypeList: function(rank){
			var typeList = [], selected = "";
			if(rank == ""){
				selected = " selected";
			}
			typeList.push('<option value="0"'+selected+'>不关联</option>');
			for(var i = 0; i < speListArr.length; i++){
				var selected = "";
				if(rank != "" && rank.split(",").in_array(speListArr[i]["id"])){
					selected = " selected";
				}
				typeList.push('<option value="'+speListArr[i]["id"]+'"'+selected+'>'+speListArr[i]["typename"]+'</option>');
			}
			return typeList.join("");
		}
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("shopType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("shopType.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
						...
						self.parent.$("#spe").html(init.speTypeList(data.spe));
						if(data.parentid != 0){
							self.parent.$("#parentid").parent().parent().show();
						}
						//self.parent.$("input[name=ishidden]:eq("+data.ishidden+")").attr("checked", true);
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("shopType.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".item", "click", function(){
		...
		try {
			parent.addPage("shopItem"+id, "shop", title+"属性值", "shop/shopItem.php?tid="+id);
		} catch(e) {
			location.href = "shopItem.php?tid="+id;
		}
	});
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("shopType.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
		...
		$.dialog({
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级和属性值也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){

						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("shopType.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input[type=text]"), id = tr.attr("data-id"), val = tr.val().replace("	", ""), icon = $(html).find(".tr .icon").val();
			...
			if(jArray.length > 0 && val != ""){
				...
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						...
						var tr = $(jArray[k]).find(".tr input"), id = tr.attr("data-id"), val = tr.val().replace("	", "");
	...
	huoniao.operaJson("shopType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
