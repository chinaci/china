$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("shopBrand.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   // huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$("#typeManage").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopBrandType", "shop", "品牌分类管理", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopBrandAdd", "shop", "新增品牌", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopBrandEdit"+id, "shop", title, "shop/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("shopBrand.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < shopBrandList.length; i++){
				...
				list.push('  <td class="row30 left"><img width="100" src="'+cfg_attachment+shopBrandList[i].logo+'&type=large" />&nbsp;'+shopBrandList[i].title+'</td>');
                // list.push('  <td class="row15">'+shopBrandList[i].cityname+'</td>');
				list.push('  <td class="row25"><a href="javascript:;" data-id="'+shopBrandList[i].typeid+'" class="type">'+shopBrandList[i].type+'</a></td>');
				list.push('  <td class="row5">'+shopBrandList[i].weight+'</td>');
				list.push('  <td class="row17">'+shopBrandList[i].pubdate+'</td>');
				list.push('  <td class="row20"><a data-id="'+shopBrandList[i].id+'" data-title="'+shopBrandList[i].title+'" href="shopBrand.php?dopost=Edit&id='+shopBrandList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
