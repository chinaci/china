$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("shopBranchStore.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("shopBranchStore.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("shopBranchStoreAddphp", "shop", "添加店铺分店", "shop/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("shopBranchStoreEdit"+id, "shop", title, "shop/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("shopBranchStore.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < shopStoreList.length; i++){
				...
				list.push('  <td class="row15 left">'+shopStoreList[i].title+'<br /><a href="'+shopStoreList[i].shopUrl+'" target="_blank"><small>'+shopStoreList[i].company+'</small></a></td>');
				list.push('  <td class="row10"><a class="userinfo" href="javascript:;" data-id="'+shopStoreList[i].userid+'">'+shopStoreList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+shopStoreList[i].contact+'</td>');
				list.push('  <td class="row10 left">'+shopStoreList[i].industry+'</td>');
				var certi = "";
				...
				list.push('  <td class="row13">'+shopStoreList[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+shopStoreList[i].id+'" data-title="'+shopStoreList[i].title+'" href="shopBranchStoreAdd.php?dopost=edit&sid='+sid+'&id='+shopStoreList[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
