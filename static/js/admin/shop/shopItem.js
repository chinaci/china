$(function(){
	...
	var init = {
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					var jsonArray =arguments[0], jArray = jsonArray.lower, addType = '';
					typeList.push('<li class="li'+level+'">');
					if(level < treeLevel){
						var checked = "";
						if(jsonArray["flag"].split(",").in_array('r')){
							checked = " checked";
						}
						addType  += '<label class="help-inline" style="margin-left:10px;"><input type="checkbox" id="write'+jsonArray["id"]+'"'+checked+' />必填</label>';
						var checked = "";
						if(jsonArray["flag"].split(",").in_array('w')){
							checked = " checked";
						}
						addType  += '<label class="help-inline" style="margin-left:10px;"><input type="checkbox" id="write'+jsonArray["id"]+'"'+checked+' />可输入</label>';
						var checked = "";
						if(jsonArray["flag"].split(",").in_array('c')){
							checked = " checked";
						}
						addType += '&nbsp;&nbsp;<label class="help-inline"><input type="checkbox" id="check'+jsonArray["id"]+'"'+checked+' />可多选</label>';
	...
	$("#list").delegate("input[type=text]", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("shopItem.php?dopost=updateType&id="+id+"&action="+action+"&tid="+tid, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$("#list").delegate("input[type=checkbox]", "click", function(){
		var t = $(this), id = t.parent().siblings("input[type=text]").attr("data-id"), flag = [];
		if(t.parent().parent().find("input[type=checkbox]:eq(0)").is(":checked")){
			flag.push("r");
		}
		if(t.parent().parent().find("input[type=checkbox]:eq(1)").is(":checked")){
			flag.push("w");
		}
		if(t.parent().parent().find("input[type=checkbox]:eq(2)").is(":checked")){
			flag.push("c");
		}
		huoniao.operaJson("shopItem.php?dopost=updateFlag&action="+action+"&tid="+tid, "id="+id+"&flag="+flag, function(data){
			if(data.state == 100){
				huoniao.showTip("success", data.info, "auto");
			}else{
				huoniao.showTip("error", data.info, "auto");
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("shopItem.php?dopost=del&action="+action+"&tid="+tid, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("shopItem.php?dopost=del&action="+action+"&tid="+tid, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("shopItem.php?dopost=typeAjax&tid="+tid, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
