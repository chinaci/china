$(function(){
	...
	var defaultBtn = $("#delBtn, #batchAudit"),
		...
		init = {
			...
			,selectTypeList: function(type){
				var typeList = [], title = type == "addr" ? "全部地区" : (type == "type" ? "授课方式" : '家教分类');
				typeList.push('<ul class="dropdown-menu">');
				typeList.push('<li><a href="javascript:;" data-id="">'+title+'</a></li>');

				var l = type == "addr" ? addrListArr : (type == "type" ? typeListArr : catListArr );
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationfamilyList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationfamilyList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#typeBtnMold").append(init.selectTypeList("type"));
	$("#catBtnMold").append(init.selectTypeList("cat"));
   ...
   //填充分站列表
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sCat").html($("#catBtnMold").attr("data-id"));
	...
	$("#catBtnMold").delegate("a", "click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#catBtnMold").attr("data-id", id);
		$("#catBtnMold button").html(title+'<span class="caret"></span>');
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("educationfamilyEdit"+id, "education", title, "education/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("educationfamilyList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, educationfamilyList = val.educationfamilyList;
		...
		if(val.state == "100"){
			...
			for(i; i < educationfamilyList.length; i++){
				list.push('<tr data-id="'+educationfamilyList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var sex = educationfamilyList[i].sex == 0 ? '<span class="append">（女）</span>' : '<span class="append">（男）</span>';
				list.push('  <td class="row10"><a href="javascript:;" >'+educationfamilyList[i].tutoiname+sex+'</a>&nbsp;&nbsp;<a href="'+educationfamilyList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				var type = educationfamilyList[i].usertype == 0 ? "(个人)" : "(商家)";
				var username = "";
				username = '<a href="javascript:;" data-id="'+educationfamilyList[i].userid+'" class="userinfo">'+educationfamilyList[i].username+'</a>';
				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+educationfamilyList[i].catname+'</td>');
				list.push('  <td class="row7">'+educationfamilyList[i].cityname+'</td>');
				list.push('  <td class="row10">'+educationfamilyList[i].price+'</td>');
				list.push('  <td class="row10">'+educationfamilyList[i].weight+'</td>');
				list.push('  <td class="row10">'+educationfamilyList[i].typename+'</td>');
				var state = "";
				switch (educationfamilyList[i].state) {
					case "0":
				...
				list.push('  <td class="row10">'+educationfamilyList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+educationfamilyList[i].id+'" data-title="'+educationfamilyList[i].tutoiname+'" href="educationfamilyAdd.php?dopost=edit&id='+educationfamilyList[i].id+'" title="修改" class="edit">修改</a>');
