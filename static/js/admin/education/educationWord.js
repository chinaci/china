		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationWord.php?dopost=delAppoint", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationWord.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
  ...
  //填充分站列表
   huoniao.choseCity($(".choseCity"),$("#cityid"));
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sType").html($("#sTypeBtn").attr("data-id"));
		$("#state").html($("#stateBtn").attr("data-id"));
	...
	//搜索分类菜单点击事件
	$("#sTypeBtn a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#sTypeBtn").attr("data-id", id);
		$("#sTypeBtn button").html(title+'<span class="caret"></span>');
	});
	...
	$("#list").delegate(".edit", "click", function(){
		...
		huoniao.operaJson("educationWord.php?dopost=getDetail", "id="+id, function(data){
			huoniao.hideTip();
			$.dialog({
				fixed: true,
				title: '举报内容',
	...
	$("#delBtn").bind("click", function(){
		if($.dialog.confirm("确定要删除吗？", function(){
			init.del("del");
		}));
	});
...
function getList(){
	...
	huoniao.operaJson("educationWord.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, wordList = val.wordList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < wordList.length; i++){
				list.push('<tr data-id="'+wordList[i].id+'">');
				list.push('  <td class="row2"><span class="check"></span></td>');
				list.push('  <td class="row10 left"><a target="_blank" href="'+wordList[i].url+'">'+wordList[i].title+'</a></td>');
				list.push('  <td class="row10 left">'+wordList[i].subjects+'</td>');
				list.push('  <td class="row5">'+wordList[i].cityname+'</td>');
				list.push('  <td class="row5">'+wordList[i].educationname+'</td>');
				list.push('  <td class="row10">'+wordList[i].subjectstime+'</td>');
				list.push('  <td class="row5">'+wordList[i].lecturesnums+'</td>');
				list.push('  <td class="row5">'+wordList[i].price+'</td>');
				var user = '<a href="javascript:;" data-id="'+wordList[i].userid+'" class="userinfo">'+wordList[i].username+'<span class="append">（'+wordList[i].sex+'）</span></a>';
				list.push('  <td class="row8">'+user+'</td>');
				list.push('  <td class="row8">'+wordList[i].tel+'</td>');
				list.push('  <td class="row20 left">'+wordList[i].note+'</td>');
				var state = "";
				switch (wordList[i].state) {
					case "0":
				...
				list.push('  <td class="row5 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row7">'+wordList[i].pubdate+'</td>');
				list.push('</tr>');
			}
