		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationstoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationstoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("educationstoreEdit"+id, "education", title, "education/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("educationstoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				var flag = "";
				switch (storeList[i].flag) {
					case "0":
						flag = '<span class="gray">待认证</span>';
						break;
					case "1":
				...
				list.push('  <td class="row10">'+flag+'</td>');
				var state = "";
				...
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].title+'" href="educationstoreAdd.php?dopost=edit&id='+storeList[i].id+'" title="修改" class="edit">修改</a>');
