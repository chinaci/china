$(function(){
	...
	var defaultBtn = $("#delBtn, #batchAudit"),
		...
		init = {
			...
			,selectTypeList: function(type){
				var typeList = [], title = type == "addr" ? "全部地区" : "全部类型";
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationcoursesList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationcoursesList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("educationcoursesEdit"+id, "education", title, "education/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("educationclassesEdit"+id, "education", title, "education/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("educationcoursesList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, coursesList = val.coursesList;
		...
		if(val.state == "100"){
			...
			for(i; i < coursesList.length; i++){
				list.push('<tr data-id="'+coursesList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+coursesList[i].coursesname+'</a>&nbsp;&nbsp;<a href="'+coursesList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				var type = coursesList[i].usertype == 0 ? "(个人)" : "(商家)";
				var username = "";
				username = '<a href="javascript:;" data-id="'+coursesList[i].userid+'" class="userinfo">'+coursesList[i].username+'</a>';
				list.push('  <td class="row20">'+username+type+'</td>');
				list.push('  <td class="row10">'+coursesList[i].weight+'</td>');
				list.push('  <td class="row10">'+coursesList[i].typename+'</td>');
				var state = "";
				switch (coursesList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+coursesList[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+coursesList[i].id+'" data-title="'+coursesList[i].coursesname+'" href="educationcoursesAdd.php?dopost=edit&id='+coursesList[i].id+'" title="修改" class="edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+coursesList[i].id+'" data-title="'+coursesList[i].coursesname+'" href="educationclass.php?action=educationclass&id='+coursesList[i].id+'" title="课程安排" class="link apart">课程安排</a>&nbsp;|&nbsp;');
