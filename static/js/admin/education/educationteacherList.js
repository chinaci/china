		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationteacherList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationteacherList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("educationteacherAddphp", "education", "新增教师", "education/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("educationteacherEdit"+id, "education", title, "education/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("educationteacherList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, educationteacherList = val.educationteacherList;
		...
		if(val.state == "100"){
			...
			for(i; i < educationteacherList.length; i++){
				list.push('<tr data-id="'+educationteacherList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+educationteacherList[i].username+'</a>&nbsp;&nbsp;<a href="'+educationteacherList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(educationteacherList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+educationteacherList[i].zjurl+'" target="_blank">'+educationteacherList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10 left">'+educationteacherList[i].typename+'</td>');
				list.push('  <td class="row5">'+educationteacherList[i].weight+'</td>');
				var flag = "";
				switch (educationteacherList[i].certifyState) {
					case "0":
				...
				var state = "";
				switch (educationteacherList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+educationteacherList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+educationteacherList[i].id+'" data-title="'+educationteacherList[i].username+'" href="educationteacherAdd.php?dopost=edit&id='+educationteacherList[i].id+'" title="修改" class="edit">修改</a>');
