$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationAppoint.php?dopost=delAppoint", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("educationAppoint.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(){
		...
		huoniao.operaJson("educationAppoint.php?dopost=getDetail", "id="+id, function(data){
			huoniao.hideTip();
			$.dialog({
				fixed: true,
				title: '举报内容',
...
function getList(){
	...
	huoniao.operaJson("educationAppoint.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < wordList.length; i++){
				...
				list.push('  <td class="row20 left"><a target="_blank" href="'+wordList[i].url+'">'+wordList[i].title+'</a></td>');
				var user = '<a href="javascript:;" data-id="'+wordList[i].userid+'" class="userinfo">'+wordList[i].username+'</a>';
				list.push('  <td class="row20">'+user+'</td>');
				list.push('  <td class="row20">'+wordList[i].tel+'</td>');
				var state = "";
