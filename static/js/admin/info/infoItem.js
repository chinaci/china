$(function(){
		...
		init = {
			...
			,popupOpera: function(type, id){
				...
				$.dialog({
					...
					ok: function(){
						...
						huoniao.operaJson("infoItem.php?action="+type, "id="+id+"&tid="+typeid+"&"+serialize, function(data){
							if(data.state == 100){
				...
				if(type == "edit"){
					huoniao.operaJson("infoItem.php?action=getDetail", "id="+id+"&tid="+typeid, function(data){
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("infoItem.php?action=del&tid="+typeid, "id="+id, function(data){
						$("#selectBtn a:eq(1)").click();
						if(data.state == 100){
							//huoniao.showTip("success", data.info, "auto");
							getList();
  ...
  //拷贝已有字段
  $("#copyItem").bind("click", function(){
  	$.dialog({
			fixed: true,
			title: "拷贝字段",
			content: '<form action="" class="quick-editForm" name="editForm"><dl class="clearfix"><dt>所在分类：</dt><dd><select id="typeid"></select></dd></dl></form>',
			width: 400,
			ok: function(){
				var t = this, id = self.parent.$("#typeid").val();
				if(id != ""){
					huoniao.operaJson("infoItem.php?action=copy", "tid="+typeid+"&id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							t.close();
							setTimeout(function() { 
...
function getList(){
	...
	huoniao.operaJson("infoItem.php?action=list", data.join("&"), function(val){
