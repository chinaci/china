$(function(){
    ...
    var defaultBtn = $("#delBtn, #batchAudit"),
        ...
        init = {
            ...
            ,selectTypeList: function(type){
                ...
                typeListArr = $.parseJSON(typeListArr);
            ...
            ,del: function(){
                ...
                if(checked.length < 1){
                    ...
                    huoniao.operaJson("infoShopList.php?dopost=del", "id="+id, function(data){
                        if(data.state == 100){
            ...
            ,updateState: function(type){
                ...
                if(checked.length < 1){
                    ...
                    huoniao.operaJson("infoShopList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
                        if(data.state == 100){
                            huoniao.showTip("success", data.info, "auto");
                            setTimeout(function() {
                                getList();
    ...
    $("#addNew").bind("click", function(){
        ...
        try {
            ...
            parent.addPage("infoShopAdd", "info", "新增商家", "info/"+href);
        } catch(e) {}
    });
    ...
    $("#list").delegate(".edit", "click", function(event){
        ...
        try {
            event.preventDefault();
            parent.addPage("infoShopAddEdit"+id, "info", title, "info/"+href);
        } catch(e) {}
    });
...
function getList(){
    ...
    huoniao.operaJson("infoShopList.php?dopost=getList", data.join("&"), function(val){
        var obj = $("#list"), list = [], i = 0, infoShop = val.infoShop;
        ...
        if(val.state == "100"){
            ...
            for(i; i < infoShop.length; i++){
                list.push('<tr data-id="'+infoShop[i].id+'">');
                list.push('  <td class="row3"><span class="check"></span></td>');
                list.push('  <td class="row15 left"><a href="'+infoShop[i].url+'" target="_blank">'+infoShop[i].company+'</a></td>');
                list.push('  <td class="row9 left"><a href="javascript:;" data-id="'+infoShop[i].uid+'" class="userinfo">'+infoShop[i].username+'</a></td>');
                list.push('  <td class="row9 left">'+infoShop[i].addrname+'</td>');
                list.push('  <td class="row9 left">'+infoShop[i].typename+'</td>');
                list.push('  <td class="row7 left">'+infoShop[i].score+'</td>');
                list.push('  <td class="row15 left">'+infoShop[i].tel+'<br />'+infoShop[i].address+'</td>');

                var state = "";
                switch (infoShop[i].state) {
                    case "0":
                ...
                list.push('  <td class="row13">'+infoShop[i].jointime+'</td>');
                list.push('  <td class="row10">');
                list.push('<a data-id="'+infoShop[i].id+'" data-title="'+infoShop[i].company+'" href="infoShopAdd.php?dopost=edit&id='+infoShop[i].id+'" title="修改" class="edit">修改</a>');
