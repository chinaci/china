$(function(){
        ...
        init = {
            ...
            ,del: function(){
                ...
                if(checked.length < 1){
                    ...
                    huoniao.operaJson("infoOrderList.php?dopost=del", "id="+id, function(data){
                        if(data.state == 100){
    ...
    $("#list").delegate(".edit", "click", function(event){
        ...
        try {
            ...
            parent.addPage("edittuanorder"+id, "info", title, "info/"+href);
        } catch(e) {}
    });
    ...
    $("#list").delegate(".payment", "click", function(){
        ...
        if(id != ""){
            $.dialog.confirm('此操作不可恢复，您确定要付款吗？', function(){
                huoniao.showTip("loading", "正在操作，请稍候...");
                huoniao.operaJson("infoOrderList.php?dopost=payment", "id="+id, function(data){
                    if(data.state == 100){
                        huoniao.showTip("success", data.info, "auto");
                        setTimeout(function() {
                            getList();
    ...
    $("#list").delegate(".refund", "click", function(){
        ...
        if(id != ""){
            $.dialog.confirm('此操作不可恢复，您确定要退款吗？', function(){
                huoniao.showTip("loading", "正在操作，请稍候...");
                huoniao.operaJson("infoOrderList.php?dopost=refund", "id="+id, function(data){
    ...
    $("#list").delegate(".revoke", "click", function(){
        ...
        $.dialog.confirm('确定要撤销此申请吗？', function(){
            huoniao.showTip("loading", "正在操作，请稍候...");
            huoniao.operaJson("infoOrderList.php?dopost=revoke", "id="+id, function(data){
                if(data.state == 100){
                    huoniao.showTip("success", data.info, "auto");
                    setTimeout(function() {
                        getList();
...
function getList(){
    ...
    huoniao.operaJson("infoOrderList.php?dopost=getList", data.join("&"), function(val){
            ...
            for(i; i < tuanOrderList.length; i++){
                ...
                switch (tuanOrderList[i].orderstate) {
                    ...
                    case "3":
                        ...
                        break;
                    case "4":
                        state = '<span class="refuse">已完成</span>';
                        break;
                    case "6":
                        //申请退款
                        if(tuanOrderList[i].retState == 1){
                ...
                if(tuanOrderList[i].orderstate == "1" || tuanOrderList[i].orderstate == "2" || tuanOrderList[i].orderstate == "6"){
                    btn = '<a href="javascript:;" data-id="'+tuanOrderList[i].id+'" class="refund" title="退款">退款</a>';
                }
                list.push('  <td class="row10 left"><a data-id="'+tuanOrderList[i].id+'" data-title="'+tuanOrderList[i].ordernum+'" href="infoOrderEdit.php?dopost=edit&id='+tuanOrderList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a>'+btn+'</td>');
            }
