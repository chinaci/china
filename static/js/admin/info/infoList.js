		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("infoList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("infoList.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				if(articleList[i].is_valid){
					sold = '<span class="label label-success" style="margin-left: 10px;">已售出</span>';
				}
				list.push('  <td class="row20 left"><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].title+'</a>'+valid+sold+'</td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+articleList[i].typeid+'" class="type">'+articleList[i].type+'</a></td>');
				list.push('  <td class="row5">'+articleList[i].price+'</td>');
				...
				list.push('  <td class="row12 state"><a href="javascript:;" class="userinfo" data-id="'+articleList[i].userid+'">'+articleList[i].username+'</a></td>');
				list.push('  <td class="row9 left">'+articleList[i].ip+"<br />"+articleList[i].ipaddr+'</td>');
				list.push('  <td class="row12">'+articleList[i].date+'</td>');

				list.push('  <td class="row10">');
				list.push('<a data-id="'+articleList[i].id+'" data-title="'+articleList[i].title+'" href="infoAdd.php?dopost=edit&id='+articleList[i].id+'" title="修改" class="edit">修改</a>');
