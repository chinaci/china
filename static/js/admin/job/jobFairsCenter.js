		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobFairsCenter.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   huoniao.choseCity($(".choseCity"),$("#cityid"));  //城市分站选择初始化
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobFairsCenterAdd", "job", "新增招聘会场", "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobFairsCenterEdit"+id, "job", title, "job/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此会场下的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("jobFairsCenter.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobFairsCenter = val.jobFairsCenter;
		...
		if(val.state == "100"){
			...
			for(i; i < jobFairsCenter.length; i++){
				list.push('<tr data-id="'+jobFairsCenter[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><a href="'+jobFairsCenter[i].url+'" target="_blank">'+jobFairsCenter[i].title+'</a></td>');
				list.push('  <td class="row12 left">'+jobFairsCenter[i].people+'</td>');
				list.push('  <td class="row12 left">'+jobFairsCenter[i].mobile+'</td>');
				list.push('  <td class="row12 left">'+jobFairsCenter[i].tel+'</td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+jobFairsCenter[i].addrid+'" class="addr">'+jobFairsCenter[i].addr+'</a></td>');
				list.push('  <td class="row12">'+jobFairsCenter[i].pubdate+'</td>');
				list.push('  <td class="row14">');
				list.push('<a data-id="'+jobFairsCenter[i].id+'" data-title="'+jobFairsCenter[i].title+'" href="jobFairsCenter.php?dopost=edit&id='+jobFairsCenter[i].id+'" title="修改" class="edit">修改</a>');
