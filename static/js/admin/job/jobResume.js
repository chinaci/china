$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobResume.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobResume.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("jobResumeAdd", "job", "新增简历", "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobResumeEdit"+id, "job", title, "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobResume.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobResume = val.jobResume;
		...
		if(val.state == "100"){
			...
			for(i; i < jobResume.length; i++){
				list.push('<tr data-id="'+jobResume[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><img onerror="this.error=\'/static/images/noPhoto_60.jpg\'" src="'+(jobResume[i].photo ? jobResume[i].photo : '/static/images/noPhoto_60.jpg')+'" style="width: 50px; height: auto;" />&nbsp;&nbsp;<a href="'+jobResume[i].url+'" target="_blank">'+jobResume[i].name+'</a></td>');
				list.push('  <td class="row10 left">'+jobResume[i].phone+'</td>');
				list.push('  <td class="row15 left">'+jobResume[i].email+'</td>');
				list.push('  <td class="row15 left">'+jobResume[i].type+'</td>');
				list.push('  <td class="row5">'+jobResume[i].gz+'</td>');
				list.push('  <td class="row5">'+jobResume[i].fs+'</td>');
				var state = "";
				switch (jobResume[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+jobResume[i].pubdate+'</td>');
				list.push('  <td class="row7">');
				list.push('<a data-id="'+jobResume[i].id+'" data-title="'+jobResume[i].name+'" href="jobResumeAdd.php?dopost=edit&id='+jobResume[i].id+'" title="修改" class="edit">修改</a>');
