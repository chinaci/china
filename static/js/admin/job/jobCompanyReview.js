$(function(){
		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobCompanyReview.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("jobCompanyReview.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#articleTitle").html(data.title);
							self.parent.$("#commonUser").html(data.name);
							...
							self.parent.$("#score").find("option").each(function(){
								if($(this).val() == data.score){
									$(this).attr("selected", true);
								}
							});
							
							self.parent.$("#commonGx").find("option").each(function(){
								if($(this).val() == data.gx){
									$(this).attr("selected", true);
								}
							});
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobCompanyReview.php?dopost=delCommon", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobCompanyReview.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#list").delegate(".article", "click", function(){
		...
		$("#typeBtn").find("button").html('公司名称<span class="caret"></span>');
		$("#searchBtn").click();
	});
...
function getList(){
	...
	huoniao.operaJson("jobCompanyReview.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">企业：<a href="javascript:;" class="article" data-id='+commonList[i].cid+'">'+commonList[i].company+'</a><br />内容：'+commonList[i].commonContent+'</td>');
				
				var user = '<a href="javascript:;" data-id="'+commonList[i].commonUserId+'" class="user">'+commonList[i].commonname+'</a>';
				if(commonList[i].commonUserId == 0){
					user = commonList[i].commonname;
				}
