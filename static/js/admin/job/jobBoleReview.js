$(function(){
		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobBoleReview.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("jobBoleReview.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#articleTitle").html(data.bole);
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobBoleReview.php?dopost=delCommon", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobBoleReview.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#list").delegate(".article", "click", function(){
		...
		$("#typeBtn").find("button").html('伯乐名称<span class="caret"></span>');
		$("#searchBtn").click();
	});
...
function getList(){
	...
	huoniao.operaJson("jobBoleReview.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">伯乐：<a href="javascript:;" class="article" data-id='+commonList[i].bid+'">'+commonList[i].bole+'</a><br />内容：'+commonList[i].commonContent+'</td>');
