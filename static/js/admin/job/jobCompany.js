		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobCompany.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
...
function getList(){
	...
	huoniao.operaJson("jobCompany.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < jobCompany.length; i++){
				...
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+jobCompany[i].logo+'" width="100" />&nbsp;&nbsp;<a href="'+jobCompany[i].url+'" target="_blank">'+jobCompany[i].title+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" class="userinfo" data-id="'+jobCompany[i].userid+'">'+jobCompany[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+jobCompany[i].people+'</td>');
				list.push('  <td class="row10 left">'+jobCompany[i].contact+'</td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+jobCompany[i].addrid+'" class="addr">'+jobCompany[i].addr+'</a></td>');
				var state = "";
				...
				list.push('  <td class="row12">'+jobCompany[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+jobCompany[i].id+'" data-title="'+jobCompany[i].title+'" href="jobCompanyAdd.php?dopost=edit&id='+jobCompany[i].id+'" title="修改" class="edit">修改</a>');
