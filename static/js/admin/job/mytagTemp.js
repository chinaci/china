$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("mytagTemp.php?action="+module+"&dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("mytagTemp.php?action="+module+"&dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("addmytagTemp"+module, module, "添加标签模板", module+"/"+href);
		} catch(e) {}
	});
	
	//标签模板
	$("#addNewTemp").bind("click", function(event){
		event.preventDefault();
		var href = $(this).attr("href");
		try {
			event.preventDefault();
			parent.addPage("mytagTemp"+module, module, "标签模板", module+"/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("editmytagTemp"+module+id, module, title, module+"/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">暂停</a></li>');
...
function getList(){
	...
	huoniao.operaJson("mytagTemp.php?action="+module+"&dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, mytagTemp = val.mytagTemp;
		obj.attr("data-totalpage", val.pageInfo.totalPage);

		$(".totalCount").html(val.pageInfo.totalCount);
		$(".totalPause").html(val.pageInfo.totalPause);
		$(".totalNormal").html(val.pageInfo.totalNormal);

		if(val.state == "100"){
			//huoniao.showTip("success", "获取成功！", "auto");
			huoniao.hideTip();
			
			for(i; i < mytagTemp.length; i++){
				list.push('<tr data-id="'+mytagTemp[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row40 left"><img src="'+cfg_attachment+mytagTemp[i].litpic+'&type=small" class="litpic" />ID：'+mytagTemp[i].id+'（'+mytagTemp[i].title+'）</td>');
				var type = "";
				switch (mytagTemp[i].type){
					case "post":
						type = "招聘职位";
						break;
					case "resume":
				...
				var isSystem = "系统默认";
				if(mytagTemp[i].isSystem == "0"){
					isSystem = "自定义";
				}
				list.push('  <td class="row10">'+isSystem+'</td>');
				var state = "";
				switch (mytagTemp[i].state) {
					case "0":
						state = '<span class="gray">暂停</span>';
						break;
					case "1":
				...
				list.push('  <td class="row15">'+mytagTemp[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+mytagTemp[i].id+'" data-title="'+mytagTemp[i].title+'" href="mytagTemp.php?action='+module+'&dopost=edit&id='+mytagTemp[i].id+'" title="修改" class="edit">修改</a>');
