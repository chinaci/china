$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobNewsList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));  //城市分站选择初始化
	...
	$("#addNew").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "job";
		try {
			event.preventDefault();
			parent.addPage("jobNewsListphpdopostAdd", type, "新增招聘资讯", "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobNewsEdit"+id, "job", title, "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobNewsList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobNewsList = val.jobNewsList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < jobNewsList.length; i++){
				list.push('<tr data-id="'+jobNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><a href="'+jobNewsList[i].url+'" target="_blank">'+jobNewsList[i].title+'</a></td>');
				list.push('  <td class="row5">'+jobNewsList[i].cityname+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+jobNewsList[i].typeid+'" class="type">'+jobNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+jobNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+jobNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+jobNewsList[i].id+'" data-title="'+jobNewsList[i].title+'" href="jobNewsList.php?dopost=Edit&id='+jobNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
