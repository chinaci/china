$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobBole.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobBole.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	//地区递归分类
	$("#zhinengBtn").append(init.selectTypeList("type"));
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#sZhineng").html($("#zhinengBtn").attr("data-id"));
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#zhinengBtn a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#zhinengBtn").attr("data-id", id);
		$("#zhinengBtn button").html(title+'<span class="caret"></span>');
	});
	...
	$(".dropdown-toggle").bind("click", function(){
		if($(this).parent().attr("id") != "typeBtn" && $(this).parent().attr("id") != "addrBtn" && $(this).parent().attr("id") != "industryBtn" && $(this).parent().attr("id") != "zhinengBtn"){
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("jobBoleAdd", "job", "新增伯乐", "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobBoleEdit"+id, "job", title, "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("sZhineng="+sZhineng);
		data.push("pagestep="+pagestep);
		data.push("page="+page);

	huoniao.operaJson("jobBole.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobBole = val.jobBole;
		...
		if(val.state == "100"){
			...
			for(i; i < jobBole.length; i++){
				list.push('<tr data-id="'+jobBole[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var type = "";
				switch(jobBole[i].type){
					case "1":
						type = "HR";
						break;
					case "2":
				...
				list.push('  <td class="row15 left"><img src="'+cfg_attachment+jobBole[i].photo+'" style="height: 60px;" />&nbsp;&nbsp;'+jobBole[i].realname+' 【'+type+'】</td>');
				list.push('  <td class="row20 left"><a href="'+jobBole[i].companyurl+'" data-id="'+jobBole[i].companyid+'" target="_blank">'+jobBole[i].company+'</a></td>');
				list.push('  <td class="row15 left">'+jobBole[i].work+'</td>');
				list.push('  <td class="row10 left">'+jobBole[i].addr+'</td>');
				var state = "";
				switch (jobBole[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+jobBole[i].pubdate+'</td>');

				list.push('  <td class="row10">');
				list.push('<a data-id="'+jobBole[i].id+'" data-title="'+jobBole[i].realname+'" href="jobBole.php?dopost=edit&id='+jobBole[i].id+'" title="修改" class="edit">修改</a>');
