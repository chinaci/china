		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobSentence.php?dopost=del&type="+type, "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type_){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("jobSentence.php?dopost=updateState&type="+type, "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
  ...
  $("#keyword").keyup(function (e) {
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			var title = type == 1 ? '新增求职' : '新增招聘';
			parent.addPage("jobSentenceAdd"+type, "job", title, "job/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("jobSentenceEdit"+type+id, "job", title, "job/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("jobSentence.php?dopost=getList&type="+type, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, jobSentence = val.jobSentence;
		...
		if(val.state == "100"){
			...
			for(i; i < jobSentence.length; i++){
				list.push('<tr data-id="'+jobSentence[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row10 left">'+jobSentence[i].cityname+'</td>');
				list.push('  <td class="row25 left">'+jobSentence[i].title+'</td>');
				list.push('  <td class="row10 left">'+jobSentence[i].people+'</td>');
				list.push('  <td class="row10 left">'+jobSentence[i].contact+'</td>');
				var state = "";
				switch (jobSentence[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+jobSentence[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+jobSentence[i].id+'" data-title="'+jobSentence[i].title+'" href="jobSentenceAdd.php?dopost=edit&type='+type+'&id='+jobSentence[i].id+'" title="修改" class="edit">修改</a>');
