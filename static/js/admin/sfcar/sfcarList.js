$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("sfcarList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("sfcarList.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#delAllGray").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			...
			huoniao.operaJson("sfcarList.php?dopost=delAllGray", "", function(data){
	...
	$("#type, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		var id = $(this).attr("data-id"), title = $(this).html(), obj = $(this).parent().parent().parent();
		obj.attr("data-id", id);
		if(obj.attr("id") == "paginationBtn"){
			var totalPage = $("#list").attr("data-totalpage");
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("sfcarAdd", "info", "发布信息", "info/"+href);
		} catch(e) {}
	});
	...
	$("#refreshBtn").bind("click", function(){
		...
		if(checked.length < 1){
			...
			huoniao.operaJson("sfcarList.php?dopost=updateTime", "id="+id, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
					$("#selectBtn a:eq(1)").click();
					setTimeout(function() {
...
function getList(){
	...
	var sKeyword = encodeURIComponent($("#sKeyword").html()),
		...
		type     = $("#type").attr("data-id") ? $("#type").attr("data-id") : "",
	...
	huoniao.operaJson("sfcarList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				if(articleList[i].isvalid){
					valid = '<span class="label label-warning" style="margin-left: 10px;">已过期</span>';
				}
				list.push('  <td class="row20 left"><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].startaddr+'——》'+articleList[i].endaddr+'</a>'+(articleList[i].accessaddr ? '<br /><code>途径：'+articleList[i].accessaddr+'</code>' : '')+'</td>');
				list.push('  <td class="row14">'+articleList[i].type+'</td>');
				list.push('  <td class="row12">'+articleList[i].missiontime+'</td>');
				list.push('  <td class="row5">'+articleList[i].cityname+'</td>');
				var state = "";
				...
				list.push('  <td class="row12"><a href="javascript:;" class="userinfo" data-id="'+articleList[i].userid+'">'+articleList[i].username+'</a></td>');
				// list.push('  <td class="row9 left" >'+articleList[i].accessaddr+'</td>');
				list.push('  <td class="row12">'+articleList[i].date+'</td>');

				list.push('  <td class="row10">');
				// list.push('<a data-id="'+articleList[i].id+'" data-title="'+articleList[i].title+'" href="sfcarAdd.php?dopost=edit&id='+articleList[i].id+'" title="修改" class="edit">修改</a>');
