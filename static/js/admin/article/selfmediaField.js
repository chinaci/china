$(function(){
	...
	var init = {
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('  <div class="row15">&nbsp;</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						var retData = data;
						data = retData[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
		...
		,selectTypeList: function(dataArr, def){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('<li'+cl+'><a href="?type='+jsonArray["id"]+'" data-id="'+jsonArray["id"]+'">'+jsonArray["typename"]+'</a>');
					if(jArray.length > 0){
						typeList.push('<ul class="dropdown-menu">');
					}
					for(var k = 0; k < jArray.length; k++){
	...
	//菜单递归分类
	// $("#typeBtn").append(init.selectTypeList(mediaTypeListArr, null));
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row50 left"><input data-id="0" type="text" value=""></div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".ad", "click", function(){
		...
		try {
			...
			parent.addPage("articleTypeAd"+id, type, "新闻"+txt+"广告", "siteConfig/advList.php?action="+type+"&type="+id);
		} catch(e) {}
	});
		...
		// }else{
			...
			if(id != 0){
				$.dialog.confirm("删除后无法恢复，请谨慎操作！！！", function(){
					...
					huoniao.operaJson("?dopost=del&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								location.reload();
		...
		$.dialog({
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级和文章、图片、评论信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){

						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("articleJson.php?action=mediafieldAjax", "type="+mediatype+"&data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
