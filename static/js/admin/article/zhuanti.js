...
$(function(){
	...
	//新增专题
	$("#addZhuanti").bind("click", function(){
		var href = $(this).attr("href");

		try {
			event.preventDefault();
			parent.addPage(action+"zhuantiAdd", "article", "添加专题", "article/"+href);
		} catch(e) {}
	});

	//专题分类
	$("#zhuantiPar").bind("click", function(){
		var href = $(this).attr("href");

		try {
			event.preventDefault();
			parent.addPage(action+"zhuantiPar", "article", "专题分类", "article/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < zhuantiList.length; i++){
				...
				list.push('  <td class="row20 left">'+img+'<span><a href="'+zhuantiList[i].url+'" target="_blank">'+zhuantiList[i].typename+'</a></span></td>');
				list.push('  <td class="row22 left">'+zhuantiList[i].description+'...</td>');
				list.push('  <td class="row10 center"><a href="articleList.php?zhuanti='+zhuantiList[i].id+'" class="zhuantiNews" data-title="'+zhuantiList[i].typename+'" data-id="'+zhuantiList[i].id+'">查看</a></td>');
				var state = "";
				...
				list.push('  <td class="row15">'+zhuantiList[i].date+'</td>');

				list.push('  <td class="row15">');
				list.push('<a href="articleAdd.php?action=article&zhuanti_par='+zhuantiList[i].id+'" title="发布信息" data-id="'+zhuantiList[i].id+'" class="put">发布</a>');
				...
				list.push('<a data-id="'+zhuantiList[i].id+'" data-title="'+zhuantiList[i].typename+'" href="zhuantiType.php?pid='+zhuantiList[i].id+'" title="子分类" class="item">子分类</a>');
	...
	//查看专题新闻
	$("#list").delegate(".zhuantiNews", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			href = $(this).attr("href");
		...
		try {
			event.preventDefault();
			parent.addPage("articleList_zt_"+id, "article", "专题新闻："+title, "article/"+href);
		} catch(e) {}
	});
