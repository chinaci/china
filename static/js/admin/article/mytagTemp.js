$(function(){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("addmytagTemp"+module, "article", "添加标签模板", "article/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("editmytagTemp"+module+id, "article", title, "article/"+href);
		} catch(e) {}
	});
	...
	huoniao.operaJson("mytagTemp.php?action="+module+"&dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < mytagTemp.length; i++){
				...
				list.push('  <td class="row50 left"><img src="'+cfg_attachment+mytagTemp[i].litpic+'&type=small" class="litpic" />ID：'+mytagTemp[i].id+'（'+mytagTemp[i].title+'）</td>');
				var isSystem = "系统默认";
