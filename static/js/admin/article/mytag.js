$(function(){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("add"+module+"Mytag", "article", "生成新标签", "article/"+href);
		} catch(e) {}
	});
	...
	$("#addNewTemp").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("mytagTemp"+module, "article", "标签模板", "article/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("edit"+module+"Mytag"+id, "article", title, "article/"+href);
		} catch(e) {}
	});
	...
	huoniao.operaJson("mytag.php?action="+module+"&dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < mytag.length; i++){
				...
				list.push('  <td class="row40 left">ID：'+mytag[i].id+'（'+mytag[i].name+'）</td>');
				var date = "不限制";
