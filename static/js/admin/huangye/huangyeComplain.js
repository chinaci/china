$(function(){
		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huangyeComplain.php?dopost=getDetail", "id="+id, function(data){
						if(data != null){
							// data = data[0];
							huoniao.hideTip();
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("huangyeComplain.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#title").html('<a href="'+data.url+'" target="_blank">'+data.title+'</a>');
							self.parent.$("#username").html(data.username);
							self.parent.$("#type").html(data.type);
							self.parent.$("#desc").html(data.desc);
							self.parent.$("#pubdate").html(huoniao.transTimes(data.pubdate, 1));
							self.parent.$("#ip").html(data.ip);
							self.parent.$("#ipaddr").html(data.ipaddr);
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huangyeComplain.php?dopost=delComplain", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huangyeComplain.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	//点击文章标题查找此文章的所有评论
	$("#list").delegate(".title", "click", function(){
		var txt = $(this).text();
		$("#keyword").val(txt);
		$("#typeBtn").attr("data-id", 1);
		$("#typeBtn").find("button").html('信息标题<span class="caret"></span>');
		$("#searchBtn").click();
	});

	//点击评论人查找此人的所有评论
	$("#list").delegate(".user", "click", function(){
		var txt = $(this).text();
		$("#keyword").val(txt);
		$("#typeBtn").attr("data-id", 2);
		$("#typeBtn").find("button").html('评论人<span class="caret"></span>');
		$("#searchBtn").click();
	});
...
function getList(){
	...
	huoniao.operaJson("huangyeComplain.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < complainList.length; i++){
				...
				list.push('  <td class="row35 left"><a href="javascript:;" class="title" data-id='+complainList[i].aid+'">'+complainList[i].title+'</a></td>');

				var user = '<a href="javascript:;" data-id="'+complainList[i].userid+'" class="user">'+complainList[i].username+'</a>';
				if(complainList[i].userid == 0){
					user = complainList[i].username;
				}

				list.push('  <td class="row12 left">'+user+'</td>');
				list.push('  <td class="row17 left"><a href="javascript:;" data-id="'+complainList[i].ip+'" class="ip">'+complainList[i].ip+'</a>（'+complainList[i].ipaddr+'）</td>');
				list.push('  <td class="row13 left">'+complainList[i].pubdate+'</td>');
				var state = "";
				switch (complainList[i].state) {
					case "等待审核":
				...
				list.push('  <td class="row10"><a href="javascript:;" data-id='+complainList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" data-id='+complainList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
