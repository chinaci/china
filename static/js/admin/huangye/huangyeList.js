$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huangyeList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huangyeList.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#delAllGray").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			...
			huoniao.operaJson("huangyeList.php?dopost=delAllGray", "", function(data){
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("huangyeAdd", "huangye", "发布信息", "huangye/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("editinfo"+id, "huangye", title, "huangye/"+href);
		} catch(e) {}
	});
	...
	$("#refreshBtn").bind("click", function(){
		...
		if(checked.length < 1){
			...
			huoniao.operaJson("huangyeList.php?dopost=updateTime", "id="+id, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
					$("#selectBtn a:eq(1)").click();
					setTimeout(function() {
...
function getList(){
	...
	huoniao.operaJson("huangyeList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				list.push('  <td class="row30 left"><a href="'+articleList[i].url+'" target="_blank"'+color+'>'+articleList[i].title+'</a>'+valid+'</td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+articleList[i].typeid+'" class="type">'+articleList[i].type+'</a></td>');
				list.push('  <td class="row5">'+articleList[i].sort+'</td>');
				var state = "";
				...
				list.push('  <td class="row13">'+articleList[i].ip+" ("+articleList[i].ipaddr+')</td>');
				list.push('  <td class="row15">'+articleList[i].date+'</td>');

				list.push('  <td class="row10">');
				list.push('<a data-id="'+articleList[i].id+'" data-title="'+articleList[i].title+'" href="huangyeAdd.php?dopost=edit&id='+articleList[i].id+'" title="修改" class="edit">修改</a>');
