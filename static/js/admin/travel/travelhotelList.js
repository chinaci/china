$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelhotelList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelhotelList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelhotelEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("travelhotelroomssEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelhotelList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, hotelList = val.hotelList;
		...
		if(val.state == "100"){
			...
			for(i; i < hotelList.length; i++){
				list.push('<tr data-id="'+hotelList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+hotelList[i].title+'</a>&nbsp;&nbsp;<a href="'+hotelList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(hotelList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+hotelList[i].zjurl+'" target="_blank">'+hotelList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row7">'+hotelList[i].price+'</td>');
				list.push('  <td class="row5">'+hotelList[i].weight+'</td>');
				list.push('  <td class="row10">'+hotelList[i].typename+'</td>');
				var state = "";
				switch (hotelList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+hotelList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+hotelList[i].id+'" data-title="'+hotelList[i].title+'" href="travelhotelAdd.php?dopost=edit&id='+hotelList[i].id+'" title="修改" class="edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+hotelList[i].id+'" data-title="'+hotelList[i].title+'" href="travelhotelroom.php?action=hotelroom&id='+hotelList[i].id+'" title="作品" class="link apart">房间信息</a>&nbsp;|&nbsp;');
