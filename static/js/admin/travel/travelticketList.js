		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelticketList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelticketList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelticketEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("travelticketinfoEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelticketList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, ticketList = val.ticketList;
		...
		if(val.state == "100"){
			...
			for(i; i < ticketList.length; i++){
				list.push('<tr data-id="'+ticketList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+ticketList[i].title+'</a>&nbsp;&nbsp;<a href="'+ticketList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(ticketList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+ticketList[i].zjurl+'" target="_blank">'+ticketList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row5">'+ticketList[i].weight+'</td>');
				list.push('  <td class="row17">'+ticketList[i].travelname+'</td>');
				var state = "";
				switch (ticketList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+ticketList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+ticketList[i].id+'" data-title="'+ticketList[i].title+'" href="travelticketAdd.php?dopost=edit&id='+ticketList[i].id+'" title="修改" class="edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+ticketList[i].id+'" data-title="'+ticketList[i].title+'" href="travelticketinfo.php?action=travelticket&id='+ticketList[i].id+'" title="门票" class="link apart">门票信息</a>&nbsp;|&nbsp;');
