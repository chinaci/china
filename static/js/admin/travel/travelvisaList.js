$(function(){
	...
	var defaultBtn = $("#delBtn, #batchAudit"),
		...
		init = {
			...
			//菜单递归分类
			,selectTypeList_: function(dataArr, def){
				var typeList = [];
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvisaList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvisaList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
  ...
  //填充分站列表
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	//菜单递归分类
	$("#typeBtnMold").append(init.selectTypeList_(typeListArr, '全部类型'));
	$("#countryBtnMold").append(init.selectTypeList_(countryListArr, '全部国家'));
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#cType").html($("#countryBtnMold").attr("data-id"));
	...
	$("#countryBtnMold").delegate("a", "click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#countryBtnMold").attr("data-id", id);
		$("#countryBtnMold button").html(title+'<span class="caret"></span>');
	});
	...
	$("#stateBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#countryBtnMold")
				.attr("data-id", "")
				.find("button").html('全部国家<span class="caret"></span>');
			$("#cType").html("");
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelvisaEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelvisaList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, visaList = val.visaList;
		...
		if(val.state == "100"){
			...
			for(i; i < visaList.length; i++){
				list.push('<tr data-id="'+visaList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="'+visaList[i].url+'" target="_blank">'+visaList[i].title+'</a></td>');
				if(visaList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+visaList[i].zjurl+'" target="_blank">'+visaList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row7">'+visaList[i].price+'/'+visaList[i].sale+'</td>');
				list.push('  <td class="row10">'+visaList[i].countryname+'</td>');
				list.push('  <td class="row10">'+visaList[i].typename+'</td>');
				var state = "";
				switch (visaList[i].state) {
					case "0":
				...
				list.push('  <td class="row10">'+visaList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+visaList[i].id+'" data-title="'+visaList[i].title+'" href="travelvisaAdd.php?dopost=edit&id='+visaList[i].id+'" title="修改" class="edit">修改</a>');
