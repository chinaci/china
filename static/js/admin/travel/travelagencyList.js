		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelagencyList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelagencyList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelagencyEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("travelagencyinfoEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelagencyList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, agencyList = val.agencyList;
		...
		if(val.state == "100"){
			...
			for(i; i < agencyList.length; i++){
				list.push('<tr data-id="'+agencyList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+agencyList[i].title+'</a>&nbsp;&nbsp;<a href="'+agencyList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(agencyList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+agencyList[i].zjurl+'" target="_blank">'+agencyList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row5">'+agencyList[i].weight+'</td>');
				list.push('  <td class="row10">'+agencyList[i].typename+'</td>');
				list.push('  <td class="row7">'+agencyList[i].price+'</td>');
				var state = "";
				switch (agencyList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+agencyList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+agencyList[i].id+'" data-title="'+agencyList[i].title+'" href="travelagencyAdd.php?dopost=edit&id='+agencyList[i].id+'" title="修改" class="edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+agencyList[i].id+'" data-title="'+agencyList[i].title+'" href="travelagencyinfo.php?action=travelagency&id='+agencyList[i].id+'" title="门票" class="link apart">门票信息</a>&nbsp;|&nbsp;');
