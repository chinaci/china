		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelrentcarList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelrentcarList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelrentcarEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelrentcarList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, rentcarList = val.rentcarList;
		...
		if(val.state == "100"){
			...
			for(i; i < rentcarList.length; i++){
				list.push('<tr data-id="'+rentcarList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+rentcarList[i].title+'</a>&nbsp;&nbsp;<a href="'+rentcarList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(rentcarList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+rentcarList[i].zjurl+'" target="_blank">'+rentcarList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row7">'+rentcarList[i].price+'</td>');
				list.push('  <td class="row5">'+rentcarList[i].weight+'</td>');
				list.push('  <td class="row10">'+rentcarList[i].typename+'</td>');
				var state = "";
				switch (rentcarList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+rentcarList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+rentcarList[i].id+'" data-title="'+rentcarList[i].title+'" href="travelrentcarAdd.php?dopost=edit&id='+rentcarList[i].id+'" title="修改" class="edit">修改</a>');
