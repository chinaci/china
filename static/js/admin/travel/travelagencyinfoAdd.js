$(function(){
	...
	if(id == 0){
		...
		$("#stime0, #etime0").datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 0, language: 'ch'});
	}else{
		if(specialtime == 0){
			$(".specialtime").html('<div class="many">'+init.manyHtml()+'</div>');
			$("#stime0, #etime0").datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 0, language: 'ch'});
			...
			for(var i = 0; i<=specialtime; i++){
				$("#stime"+i).datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 0, language: 'ch'});// hh:ii:ss
	...
	$(".specialtime").delegate(".add", "click", function(){
		...
		$("#stime"+m).datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 0, language: 'ch'});
		$("#etime"+m).datetimepicker({format: 'yyyy-mm-dd', autoclose: true, minView: 0, language: 'ch'});
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("travelagencyinfoAdd.php", $("#editform").serialize() +"&specialtime="+data+"&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
