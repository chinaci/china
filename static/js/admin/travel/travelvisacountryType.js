$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvisacountryType.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvisacountryType.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	//地区递归分类
	$("#continentBtn").append(init.selectTypeList("continent"));
	...
	$("#searchBtn").bind("click", function(){
		...
		$("#scontinent").html($("#continentBtn").attr("data-id"));
	...
	$("#delAllGray").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			...
			huoniao.operaJson("travelvisacountryType.php?dopost=delAllGray", "", function(data){
	...
	//二级菜单点击事件
	$("#continentBtn a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#continentBtn").attr("data-id", id);
		$("#continentBtn button").html(title+'<span class="caret"></span>');
	});
	...
	$("#stateBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#continentBtn")
				.attr("data-id", "")
				.find("button").html('所属洲<span class="caret"></span>');
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("travelvisacountryTypeAdd", "travel", "增加签证国家", "travel/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("edittravelvisacountryType"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("scontinent="+scontinent);
		data.push("state="+state);
		data.push("pagestep="+pagestep);
		data.push("page="+page);

	huoniao.operaJson("travelvisacountryType.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				list.push('  <td class="row20 left"><a href="'+articleList[i].url+'" target="_blank">'+articleList[i].typename+'</a></td>');
				list.push('  <td class="row12"><a href="javascript:;" data-id="'+articleList[i].typeid+'" class="type">'+articleList[i].typeidname+'</a></td>');
				list.push('  <td class="row9">'+articleList[i].price+'</td>');
				list.push('  <td class="row10">'+articleList[i].continentname+'</td>');
				var state = "";
				...
				list.push('  <td class="row12">'+articleList[i].weight+'</td>');
				list.push('  <td class="row12">'+articleList[i].date+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+articleList[i].id+'" data-title="'+articleList[i].typename+'" href="travelvisacountryTypeAdd.php?dopost=edit&id='+articleList[i].id+'" title="修改" class="edit">修改</a>');
