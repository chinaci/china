$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelstoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelstoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	//菜单递归分类
	$("#typeBtnMold").append(init.selectTypeList_(moduleListArr, '全部类型'));
	//$("#typeBtn").append(init.selectTypeList_(typeListArr, '全部分类'));
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelstoreEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelstoreList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				list.push('  <td class="row15 left">'+storeList[i].address+'</td>');
				list.push('  <td class="row10">'+storeList[i].typename+'</td>');
				var state = "";
				...
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].title+'" href="travelstoreAdd.php?dopost=edit&id='+storeList[i].id+'" title="修改" class="edit">修改</a>');
