$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvideoList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("travelvideoList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("travelvideoEdit"+id, "travel", title, "travel/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("travelvideoList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, videoList = val.videoList;
		...
		if(val.state == "100"){
			...
			for(i; i < videoList.length; i++){
				list.push('<tr data-id="'+videoList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><a href="javascript:;" >'+videoList[i].title+'</a>&nbsp;&nbsp;<a href="'+videoList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				/* if(videoList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+videoList[i].zjurl+'" target="_blank">'+videoList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">个人</span></td>');
				} */
				var type = videoList[i].usertype == 0 ? "(个人)" : "(商家)";
				var username = "";
				username = '<a href="javascript:;" data-id="'+videoList[i].userid+'" class="userinfo">'+videoList[i].username+'</a>';

				list.push('  <td class="row20">'+username+type+'</td>');

				list.push('  <td class="row12">'+videoList[i].weight+'</td>');
				var state = "";
				switch (videoList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+videoList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+videoList[i].id+'" data-title="'+videoList[i].title+'" href="travelvideoAdd.php?dopost=edit&id='+videoList[i].id+'" title="修改" class="edit">修改</a>');
