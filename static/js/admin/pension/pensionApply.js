$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionApply.php?dopost=delAppoint", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionApply.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(){
		...
		huoniao.operaJson("pensionApply.php?dopost=getDetail", "id="+id, function(data){
			huoniao.hideTip();
			$.dialog({
				fixed: true,
				title: '举报内容',
...
function getList(){
	...
	huoniao.operaJson("pensionApply.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < wordList.length; i++){
				...
				list.push('  <td class="row20 left"><a target="_blank" href="'+wordList[i].url+'">'+wordList[i].storetitle+'</a></td>');
				list.push('  <td class="row10"><a target="_blank" href="'+wordList[i].elderlyurl+'">'+wordList[i].elderlytitle+'</a></td>');
				list.push('  <td class="row10">'+wordList[i].catidname+'</td>');
				var user = '<a href="javascript:;" data-id="'+wordList[i].userid+'" class="userinfo">'+wordList[i].username+'</a>';
				list.push('  <td class="row10">'+user+'</td>');
				list.push('  <td class="row10">'+wordList[i].tel+'</td>');
				var state = "";
				switch (wordList[i].state) {
					case "0":
						state = '<span class="gray">未联系</span>';
						break;
					case "1":
						state = '<span class="audit">已联系</span>';
						break;
				}
				list.push('  <td class="row20">'+state+'</td>');
				list.push('  <td class="row17">'+wordList[i].pubdate+'</td>');
				list.push('</tr>');
			}
