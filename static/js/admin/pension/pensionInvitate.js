		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionInvitate.php?dopost=delAppoint", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionInvitate.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
 ...
 //填充分站列表
  huoniao.choseCity($(".choseCity"),$("#cityid"));
	...
	$("#list").delegate(".edit", "click", function(){
		...
		huoniao.operaJson("pensionInvitate.php?dopost=getDetail", "id="+id, function(data){
			huoniao.hideTip();
			$.dialog({
				fixed: true,
				title: '举报内容',
...
function getList(){
	...
	huoniao.operaJson("pensionInvitate.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < wordList.length; i++){
				...
				list.push('  <td class="row20"><a target="_blank" href="'+wordList[i].elderlyurl+'">'+wordList[i].elderlytitle+'</a></td>');
