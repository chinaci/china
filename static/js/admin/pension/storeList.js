$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionstoreList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionstoreList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
  ...
  huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#typeBtnMold").append(init.selectTypeList_(catListArr, '全部类型'));
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("pensionstoreEdit"+id, "pension", title, "pension/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("pensionalbum"+id, "pension", title + "相册", "pension/"+url);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("pensionstoreList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, storeList = val.storeList;
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				list.push('<tr data-id="'+storeList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="'+storeList[i].url+'" target="_blank">'+storeList[i].title+'</a></td>');
				list.push('  <td class="row5">'+storeList[i].cityname+'</td>');
				list.push('  <td class="row7"><a href="javascript:;" data-id="'+storeList[i].userid+'" class="userinfo">'+storeList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+storeList[i].tel+'</td>');
				list.push('  <td class="row15 left">'+storeList[i].catidname+'</td>');
				list.push('  <td class="row10">'+storeList[i].price+'</td>');
				var state = "";
				switch (storeList[i].state) {
					case "0":
				...
				list.push('  <td class="row10">'+storeList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].title+'" href="pensionstoreAdd.php?dopost=edit&id='+storeList[i].id+'" title="修改" class="edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].title+'" href="pensionalbum.php?action=loupan&id='+storeList[i].id+'" title="'+storeList[i].title+'" class="link apart">相册</a>&nbsp;|&nbsp;');
