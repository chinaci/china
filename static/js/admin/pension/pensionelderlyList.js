$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionelderlyList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionelderlyList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("pensionelderlyEdit"+id, "pension", title, "pension/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("pensionelderlyList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < storeList.length; i++){
				...
				var sex = storeList[i].sex == 0 ? '<span class="append">（女）</span>' : '<span class="append">（男）</span>';
				list.push('  <td class="row20 left"><a href="'+storeList[i].url+'" target="_blank">'+storeList[i].elderlyname+sex+(storeList[i].switch == '1' ? '<span class="label label-info">开启</span>' : '<span class="label">关闭</span>')+'</a></td>');
				...
				list.push('  <td class="row10">'+storeList[i].monthminprice+'-'+storeList[i].monthmaxprice+'</td>');
				var state = "";
				...
				list.push('<a data-id="'+storeList[i].id+'" data-title="'+storeList[i].elderlyname+'" href="pensionelderlyAdd.php?dopost=edit&id='+storeList[i].id+'" title="修改" class="edit">修改</a>');
