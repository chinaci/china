$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionYuyue.php?dopost=delAppoint", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pensionYuyue.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(){
		...
		huoniao.operaJson("pensionYuyue.php?dopost=getDetail", "id="+id, function(data){
			huoniao.hideTip();
			$.dialog({
				fixed: true,
				title: '举报内容',
...
function getList(){
	...
	huoniao.operaJson("pensionYuyue.php?dopost=getList", data.join("&"), function(val){
