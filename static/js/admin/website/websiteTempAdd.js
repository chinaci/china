$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "websiteTempList.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-websiteTempListphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-shopBrandphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-websiteTempListphp"));
									$("body",parent.document).find("#nav-websiteTempEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "websiteTempList.php";
								}
							},
							cancel: false
						});
