...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "websiteCase.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-websiteCase"+$("#website").val()).click();
									//parent.reloadPage($("body",parent.document).find("#body-websiteCasephp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-websiteCase"+$("#website").val()));
									$("body",parent.document).find("#nav-websiteCaseEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "websiteCase.php?website="+$("#website").val();
								}
							},
							cancel: false
						});
