...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "websiteEvents.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-websiteEvents"+$("#website").val()).click();
									//parent.reloadPage($("body",parent.document).find("#body-websiteEventsphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-websiteEvents"+$("#website").val()));
									$("body",parent.document).find("#nav-websiteEventsEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "websiteEvents.php?website="+$("#website").val();
								}
							},
							cancel: false
						});
