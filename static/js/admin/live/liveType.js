$(function(){
	...
	var init = {
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row45 left"><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'"><label class="help-inline" style="margin-left:10px;"><input type="radio" name="write'+jsonArray["id"]+'" value="0" '+(jsonArray['flag'] == 0 ? 'checked' : '')+' />横屏</label><label class="help-inline" style="margin-left:10px;"><input type="radio" name="write'+jsonArray["id"]+'" value="1" '+(jsonArray['flag'] == 1 ? 'checked' : '')+' />竖屏</label></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row45 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
					}
					...
					typeList.push('  <div class="row17 left"><a href=javascript:;" class="del">编辑删除</a></div>');
					typeList.push('</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("liveType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("liveType.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
	...
	$("#addNew_").bind("click", function(){
		...
		var px = 'px_'+ Math.random()*100000;

		html.push('<li class="li0">');
		html.push('  <div class="tr clearfix">');
		html.push('    <div class="row3"><a href="javascript:;" class="fold">折叠</a></div>');
		html.push('    <div class="row60 left"><input data-id="0" type="text" value=""><label class="help-inline" style="margin-left:10px;"><input type="radio" name="write'+px+'" value="0" checked />横屏</label><label class="help-inline" style="margin-left:10px;"><input type="radio" name="write'+px+'" value="1" />竖屏</label></div>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0 && id != undefined){
			huoniao.operaJson("liveType.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".ad", "click", function(){
		...
		try {
			...
			parent.addPage("imageTypeAd"+id, type, "图片"+txt+"广告", "siteConfig/advList.php?action="+type+"&type="+id);
		} catch(e) {}
	});
	...
	$(".root").delegate(".del", "click", function(event){
		...
		// }else{
			...
			if(type.indexOf("编辑") > -1){
				$.dialog.confirm("确定后，此分类下的直播信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
					huoniao.showTip("loading", "正在删除，，请稍候...");
					huoniao.operaJson("liveType.php?dopost=del&action="+action, "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级和文章、图片信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("liveType.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input"), id = tr.attr("data-id"), val = tr.val(), icon = $(html).find(".tr .icon").val(), flag = $(html).find("input[type=radio]:checked").val();

			icon = icon == undefined ? '' : icon;

			if(jArray.length > 0 && val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "icon": "'+icon+'", "flag": "'+flag+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						arguments.callee(jArray[k]);
					}else{
						var tr = $(jArray[k]).find(".tr input"), id = tr.attr("data-id"), val = tr.val(), icon = $(jArray[k]).find(".tr .icon").val(), flag = $(jArray[k]).find("input[type=radio]:checked").val();
						icon = icon == undefined ? '' : icon;
						if(val != ""){
							json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "icon": "'+icon+'", "flag": "'+flag+'", "lower": null},';
						}else{
							count++;
						}
					}
	...
	huoniao.operaJson("liveJson.php?action=typeAjax&dopost="+action, "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
