$(function(){
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkUser_adviser", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
				t.siblings(".input-tips").removeClass().addClass("input-tips input-error").html('<s></s>此会员已经加入其它经销商，不可以重复添加！');
			}else{
				if(data) {
					for(var i = 0; i < data.length; i++){
	...
	$("#zjcom").bind("input", function(){
		...
		if(val != ""){
			t.addClass("input-loading");
			huoniao.operaJson("../inc/json.php?action=checkCarStore", "key="+val, function(data){
				t.removeClass("input-loading");
				if(!data) {
					$("#comList").html("").hide();
					return false;
	...
	$("#zjcom").bind("blur", function(){
		...
		if(val != ""){
			...
			t.siblings(".input-tips").removeClass().addClass("input-tips input-error").html('<s></s>请从列表中选择经销商');
		}
	});
	...
	$("#btnSubmit").bind("click", function(event){
      ...
      id           = $("#id").val(),
		...
		if(tel == ""){
			huoniao.goTop();
			$("#tel").siblings(".input-tips").removeClass().addClass("input-tips input-error").attr("style", "display:inline-block");
  			huoniao.goTop();
  			return false;
		}
		...
		//异步提交
		huoniao.operaJson("gwUserAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
					huoniao.parentTip("success", "顾问添加成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
					huoniao.goTop();
					location.reload();
				}else{
					huoniao.parentTip("success", "顾问修改成功！<a href='"+data.url+"' target='_blank'>"+data.url+"</a>");
