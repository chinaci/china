...
$(function(){
	...
	function checkGw(t, val, id){
		var flag = false;
		t.addClass("input-loading");
		huoniao.operaJson("../inc/json.php?action=checkUser_car", "key="+val+"&id="+id, function(data){
			t.removeClass("input-loading");
			if(data == 200){
	...
	$("#mark").bind("click", function(){
		$.dialog({
			...
			content: 'url:'+adminPath+'../api/map/mark.php?mod=car&lnglat='+$("#lnglat").val()+"&city="+mapCity+"&address="+$("#address").val(),
			width: 800,
			height: 500,
			max: true,
			ok: function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("carStoreAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
