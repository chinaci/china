$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("carEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carList = val.carList;
		...
		if(val.state == "100"){
			...
			for(i; i < carList.length; i++){
				list.push('<tr data-id="'+carList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '';
				if(carList[i].litpic!=''){
					img = '<img src="'+cfg_attachment+carList[i].litpic+'&type=small" class="litpic" />';
				}
				list.push('  <td class="row25 left">'+img+'<span><a href="'+carList[i].url+'" target="_blank">'+carList[i].title+'</a></span></td>');
				list.push('  <td class="row10">'+carList[i].weight+'</td>');

				list.push('  <td class="row12 left">'+carList[i].brandname+'<br><small>'+carList[i].addrname+'</small></td>');

				var type = carList[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(carList[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+carList[i].userid+'" class="userinfo">'+carList[i].username+'</a>';
				}else{
					username = carList[i].username;
				}

				list.push('  <td class="row10">'+username+type+'</td>');
				list.push('  <td class="row10">'+carList[i].contact+'</td>');
				var state = "";
				switch (carList[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+carList[i].pubdate+'</td>');
				list.push('  <td class="row8">');
				list.push('<a data-id="'+carList[i].id+'" data-title="'+carList[i].title+'" href="carAdd.php?dopost=edit&id='+carList[i].id+'" title="修改" class="edit">修改</a>');
