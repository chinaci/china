$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carBrand.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#typeManage").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carBrandType", "car", "品牌分类管理", "car/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carBrandAdd", "car", "新增品牌型号", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carBrandEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var sKeyword = encodeURIComponent($("#sKeyword").html()),
    	...
    	adminCity  = $("#cityList").val() ? $("#cityList").val() : "0";
	...
	huoniao.operaJson("carBrand.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, shopBrandList = val.shopBrandList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < shopBrandList.length; i++){
				list.push('<tr data-id="'+shopBrandList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				if(shopBrandList[i].logo!=''){
					list.push('  <td class="row30 left"><img width="100" src="'+cfg_attachment+shopBrandList[i].logo+'&type=large" />&nbsp;'+shopBrandList[i].brandname+'</td>');
				}else{
					list.push('  <td class="row30 left">'+shopBrandList[i].brandname+'</td>');
				}
                list.push('  <td class="row15">'+shopBrandList[i].title+'</td>');
				list.push('  <td class="row15">'+shopBrandList[i].levelname+'</td>');
				list.push('  <td class="row5">'+shopBrandList[i].weight+'</td>');
				list.push('  <td class="row12">'+shopBrandList[i].pubdate+'</td>');
				list.push('  <td class="row20"><a data-id="'+shopBrandList[i].id+'" data-title="'+shopBrandList[i].title+'" href="carBrand.php?dopost=Edit&id='+shopBrandList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
