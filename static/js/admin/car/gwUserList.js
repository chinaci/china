$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("gwUserList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("gwUserList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("gwUserEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("gwUserList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < zjUserList.length; i++){
				...
				if(zjUserList[i]['zjcom'] != "0"){
					list.push('  <td class="row20 left"><a href="'+zjUserList[i].zjurl+'" target="_blank">'+zjUserList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row20 left"><span style="color:#f60;">独立顾问</span></td>');
				}
				list.push('  <td class="row10 left">'+zjUserList[i].addr+'</td>');
				list.push('  <td class="row5">'+zjUserList[i].weight+'</td>');
				var flag = "";
				switch (zjUserList[i].type) {
					case "1":
						flag = '<span class="audit">金牌顾问</span>';
						break;
				}
				...
				list.push('  <td class="row15">'+zjUserList[i].joindate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+zjUserList[i].id+'" data-title="'+zjUserList[i].username+'" href="gwUserAdd.php?dopost=edit&id='+zjUserList[i].id+'" title="修改" class="edit">修改</a>');
