$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("carNewsList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carNewsAdd", type, "新增汽车资讯", "car/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("carNewsEdit"+id, "car", title, "car/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("carNewsList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, carNewsList = val.carNewsList;
		if(val.state == "100"){
			huoniao.hideTip();

			obj.attr("data-totalpage", val.pageInfo.totalPage);

			for(i; i < carNewsList.length; i++){
				list.push('<tr data-id="'+carNewsList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><a href="'+carNewsList[i].url+'" target="_blank">'+carNewsList[i].title+'</a></td>');
				list.push('  <td class="row5">'+carNewsList[i].cityname+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+carNewsList[i].typeid+'" class="type">'+carNewsList[i].type+'</a></td>');
				list.push('  <td class="row15">'+carNewsList[i].date+'</td>');
				list.push('  <td class="row12">'+carNewsList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+carNewsList[i].id+'" data-title="'+carNewsList[i].title+'" href="carNewsList.php?dopost=Edit&id='+carNewsList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
