		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huodongReply.php?dopost=del", "hid="+hid+"&id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("huodongReply.php?dopost=updateState", "hid="+hid+"&id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("huodongReply.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, huodongReply = val.huodongReply;
		...
		if(val.state == "100"){
			...
			for(i; i < huodongReply.length; i++){
				list.push('<tr data-id="'+huodongReply[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row50 left">帖子：<a href="'+huodongReply[i].url+'" target="_blank">'+huodongReply[i].title+'</a><br />回复：'+huodongReply[i].content+'</td>');
				list.push('  <td class="row10"><a href="javascript:;" class="userinfo" data-id="'+huodongReply[i].uid+'">'+huodongReply[i].username+'</a></td>');
				list.push('  <td class="row15">'+huodongReply[i].pubdate+'<br />'+huodongReply[i].ip+'</td>');
				var state = "";
				switch (huodongReply[i].state) {
					case "0":
