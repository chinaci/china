...
$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "tuanFaq.php?dopost="+$("#dopost").val(),
				...
				if(data.state == 100){
					...
					if($("#dopost").val() == "Add"){
						...
						$.dialog({
							...
							ok: function(){
								try{
									$("body",parent.document).find("#nav-tuanFaqphp").click();
									//parent.reloadPage($("body",parent.document).find("#body-tuanFaqphp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-tuanFaqphp"));
									$("body",parent.document).find("#nav-tuanFaqEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "tuanFaq.php";
								}
							},
							cancel: false
						});
