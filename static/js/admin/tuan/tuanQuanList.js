		...
		init = {
			...
			,operaJson: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanQuanList.php?dopost="+type, "id="+id, function(data){
						if(data.state == 100){
...
function getList(){
	...
	huoniao.operaJson("tuanQuanList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, tuanQuanList = val.tuanQuanList;
		...
		if(val.state == "100"){
			...
			for(i; i < tuanQuanList.length; i++){
				list.push('<tr data-id="'+tuanQuanList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row10 left">'+tuanQuanList[i].cardnum+'</td>');
				list.push('  <td class="row12 left"><a href="tuanOrderEdit.php?dopost=edit&id='+tuanQuanList[i].orderid+'" data-id="'+tuanQuanList[i].orderid+'" class="order">'+tuanQuanList[i].ordernum+'</a></td>');
				list.push('  <td class="row20 left"><a href="'+tuanQuanList[i].prourl+'" target="_blank" data-id="'+tuanQuanList[i].proid+'">'+tuanQuanList[i].proname+'</a></td>');
				list.push('  <td class="row13 left">'+tuanQuanList[i].orderdate+'</td>');
				list.push('  <td class="row10 left">&yen;'+tuanQuanList[i].orderprice+'</td>');
				list.push('  <td class="row12 left">'+tuanQuanList[i].expireddate+'</td>');
				var state = "";
				if(tuanQuanList[i].state == 0){
					state = "<span class='audit'>可使用</span>";
				}else if(tuanQuanList[i].state == 1){
					state = "<span class='gray'>已过期</span>";
				}else if(tuanQuanList[i].state == 2){
					state = "<span class='refuse'>已消费&nbsp;消费于："+tuanQuanList[i].usedate+"</span>";
				}
