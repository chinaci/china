$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanStore.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanStore.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("tuanStoreAddphp", "tuan", "新增商家", "tuan/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("tuanStoreEdit"+id, "tuan", title, "tuan/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("tuanStore.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, tuanStore = val.tuanStore;
		...
		if(val.state == "100"){
			...
			for(i; i < tuanStore.length; i++){
				list.push('<tr data-id="'+tuanStore[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left">'+tuanStore[i].weight+' / <a href="'+tuanStore[i].url+'" target="_blank">'+tuanStore[i].company+'</a></td>');
				list.push('  <td class="row9 left"><a href="javascript:;" data-id="'+tuanStore[i].uid+'" class="userinfo">'+tuanStore[i].username+'</a></td>');
				list.push('  <td class="row9 left">'+tuanStore[i].addrname+'</td>');
				list.push('  <td class="row9 left">'+tuanStore[i].typename+'</td>');
				list.push('  <td class="row7 left">'+tuanStore[i].score+'</td>');
				list.push('  <td class="row15 left">'+tuanStore[i].tel+'<br />'+tuanStore[i].address+'</td>');

				var state = "";
				switch (tuanStore[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+tuanStore[i].jointime+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+tuanStore[i].id+'" data-title="'+tuanStore[i].company+'" href="tuanStoreAdd.php?dopost=edit&id='+tuanStore[i].id+'" title="修改" class="edit">修改</a>');
