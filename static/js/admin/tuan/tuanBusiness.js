$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanBusiness.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanBusiness.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">等待审核</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">审核通过</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">审核失败</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("tuanBusiness.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < businessList.length; i++){
				...
				list.push('  <td class="row10 left">'+businessList[i].name+'</td>');
				list.push('  <td class="row10 left">'+businessList[i].phone+'</td>');
				list.push('  <td class="row25 left">'+businessList[i].shopname+'</td>');
				list.push('  <td class="row15">'+businessList[i].pubdate+'</td>');
				list.push('  <td class="row17">'+businessList[i].ip+'（'+businessList[i].ipaddr+'）</td>');
				var style = "audit";
				if(businessList[i].state == "审核失败"){
					style = 'refuse';
				}else if(businessList[i].state == "等待审核"){
					style = 'gray';
				}
				list.push('  <td class="row10 state"><span class="'+style+'">'+businessList[i].state+'</span><span class="more"><s></s></span></td>');
				list.push('  <td class="row10"><a href="javascript:;" data-id='+businessList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
