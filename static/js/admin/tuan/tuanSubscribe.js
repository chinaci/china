$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanSubscribe.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanSubscribe.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">已退订</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">订阅中</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("tuanSubscribe.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, subscribeList = val.subscribeList;
		if(val.state == "100"){
			huoniao.hideTip();
			...
			for(i; i < subscribeList.length; i++){
				list.push('<tr data-id="'+subscribeList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row60 left">'+subscribeList[i].email+'</td>');
				list.push('  <td class="row15">'+subscribeList[i].pubdate+'</td>');
				var style = "audit";
				if(subscribeList[i].state == "已退订"){
					style = 'refuse';
				}
				list.push('  <td class="row12 state"><span class="'+style+'">'+subscribeList[i].state+'</span><span class="more"><s></s></span></td>');
				list.push('  <td class="row10"><a href="javascript:;" data-id='+subscribeList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
