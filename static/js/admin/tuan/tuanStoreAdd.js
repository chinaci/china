...
$(function(){
	...
	function getCircle(){
		...
		var addrBtnid = $('.addrBtn').attr('data-id');
		var aid = addrid;
		if(addrid == cityid){
			aid = addrBtnid;
		}
		huoniao.operaJson("tuanStoreAdd.php?action=getCircle", "addrid="+aid, function(data){
	...
	$("#company").bind("input", function(){
		...
		if(val != ""){
			...
			huoniao.operaJson("tuanStoreAdd.php?action=checkUser", "key="+val+"&id="+id, function(data){
  ...
  $("#editform input").keyup(function (e) {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("tuanStoreAdd.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "save"){
