$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tuanFaq.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#typeManage").bind("click", function(event){
		...
		var href = $(this).attr("href"), type = "tuan";
		
		try {
			event.preventDefault();
			parent.addPage("tuanFaqType", type, "常见问题分类", "tuan/"+href);
		} catch(e) {}
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("tuanFaqAdd", type, "新增常见问题", "tuan/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("tuanFaqEdit"+id, "tuan", title, "tuan/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("tuanFaq.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, tuanFaqList = val.tuanFaqList;
		if(val.state == "100"){
			huoniao.hideTip();
			
			obj.attr("data-totalpage", val.pageInfo.totalPage);
			
			for(i; i < tuanFaqList.length; i++){
				list.push('<tr data-id="'+tuanFaqList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left">'+tuanFaqList[i].title+'</td>');
				list.push('  <td class="row5">'+tuanFaqList[i].weight+'</td>');
				list.push('  <td class="row15"><a href="javascript:;" data-id="'+tuanFaqList[i].typeid+'" class="type">'+tuanFaqList[i].type+'</a></td>');
				list.push('  <td class="row15">'+tuanFaqList[i].date+'</td>');
				list.push('  <td class="row12">'+tuanFaqList[i].state+'</td>');
				list.push('  <td class="row15"><a data-id="'+tuanFaqList[i].id+'" data-title="'+tuanFaqList[i].title+'" href="tuanFaq.php?dopost=Edit&id='+tuanFaqList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
