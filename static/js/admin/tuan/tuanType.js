$(function(){
	...
	var init = {
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					var jsonArray =arguments[0], jArray = jsonArray.lower, hot = '', style = '';
					...
					if(jsonArray["hot"] == 1){
						hot = '<i class="icon-tag" style="margin:5px 0 0 10px;" title="热门"></i>';
					}
					if(jsonArray["color"] != ""){
						style = " style='color:"+jsonArray["color"]+";'";
					}
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row40 left"><input'+style+' type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+hot+addType+'</div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row40 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input'+style+' type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+hot+addType+'</div>');
					}

					var zd = "";
					if(level == 0){
						zd = '<a href="javascript:;" class="item" title="字段">字段</a>';
					}
					...
					typeList.push('  <div class="row17 left"><a href="javascript:;" class="edit" title="编辑">编辑</a>'+zd+'<a href="javascript:;" class="del" title="删除">删除</a></div>');
					typeList.push('</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("tuanType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("tuanType.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
						...
						self.parent.$("#hot").attr("checked", data.hot == 1 ? true : false);

						self.parent.$("#color").val(data.color);
						self.parent.$(".color_pick").find("em").css({"background": data.color != "" ? data.color : null});
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("tuanType.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	$(".root").delegate(".item", "click", function(){
		...
		try {
			parent.addPage("tuanItem"+id, "tuan", title+"字段", "tuan/tuanItem.php?tid="+id);
		} catch(e) {
			location.href = "tuanItem.php?tid="+id;
		}
	});
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(t.parent().parent().next("ul").html() != undefined && t.parent().parent().next("ul").html() != ""){
			tip = "确定后，此分类下的子级以及所有信息也将同时删除！<br />";
		}
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("tuanType.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
		...
		$.dialog({
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级以及所有信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){

						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("tuanType.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input[type=text]"), id = tr.attr("data-id"), val = tr.val(), icon = $(html).find(".tr .icon").val();
	...
	huoniao.operaJson("tuanType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
