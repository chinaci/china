		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("integralList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("integralList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("productEdit"+id, "integral", title, "integral/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("integralList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, integralList = val.integralList;
		...
		if(val.state == "100"){
			...
			for(i; i < integralList.length; i++){
				list.push('<tr data-id="'+integralList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var flag = "";
				if(integralList[i].flag != ""){
					flag = "<span class='append'>（"+integralList[i].flag+"）</span>";
				}
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+integralList[i].litpic+'" class="litpic" /><span><a href="'+integralList[i].url+'" target="_blank">'+integralList[i].title+'</a>'+flag+'</span></td>');
				list.push('  <td class="row12 left"><a href="javascript:;" data-id="'+integralList[i].typeid+'" class="type">'+integralList[i].typename+'</a></td>');
				list.push('  <td class="row10 left">'+integralList[i].price+'</td>');
				list.push('  <td class="row12 left">'+integralList[i].point+'</td>');
				list.push('  <td class="row7 left">'+integralList[i].sales+"/"+integralList[i].inventory+'</td>');
				var state = "";
				switch (integralList[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+integralList[i].pubdate+'</td>');
				list.push('  <td class="row9">');
				list.push('<a data-id="'+integralList[i].id+'" data-title="'+integralList[i].title+'" href="integralAdd.php?&dopost=edit&id='+integralList[i].id+'" title="修改" class="edit">修改</a>');
