		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tiebaReply.php?dopost=del", "tid="+tid+"&id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("tiebaReply.php?dopost=updateState", "tid="+tid+"&id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("tiebaReply.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, tiebaReply = val.tiebaReply;
		...
		if(val.state == "100"){
			...
			for(i; i < tiebaReply.length; i++){
				list.push('<tr data-id="'+tiebaReply[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row50 left">帖子：<a href="'+tiebaReply[i].url+'" target="_blank">'+tiebaReply[i].title+'</a><br />回复：'+tiebaReply[i].content+'</td>');
				list.push('  <td class="row10"><a href="javascript:;" class="userinfo" data-id="'+tiebaReply[i].uid+'">'+tiebaReply[i].username+'</a></td>');
				list.push('  <td class="row15">'+tiebaReply[i].pubdate+'<br />'+tiebaReply[i].ip+'</td>');
				var state = "";
				switch (tiebaReply[i].state) {
					case "0":
