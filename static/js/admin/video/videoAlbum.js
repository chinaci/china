$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("videoAlbum.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("videoalbumEdit"+id, "video", title, "video/"+href);
		} catch(e) {}
	});
...
//新增
	$("#addNew").bind("click", function(){
		...
		try {
			event.preventDefault();
			parent.addPage("videoAlbumaddphp", "video", "新增专辑", "video/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("page="+page);
	huoniao.operaJson("videoAlbum.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < circleList.length; i++){
				...
				list.push('  <td class="row25">'+ circleList[i].username+'</td>');
				...
				list.push('<a data-id="'+circleList[i].id+'" data-title="'+circleList[i].title+'" href="videoAlbumadd.php?dopost=edit&id='+circleList[i].id+'" title="修改" class="edit">修改</a>');
