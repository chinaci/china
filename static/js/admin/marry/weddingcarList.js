$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("weddingcarList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("weddingcarList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("weddingcarEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("weddingcarList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, weddingcarList = val.weddingcarList;
		...
		if(val.state == "100"){
			...
			for(i; i < weddingcarList.length; i++){
				list.push('<tr data-id="'+weddingcarList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+weddingcarList[i].title+'</a>&nbsp;&nbsp;<a href="'+weddingcarList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(weddingcarList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+weddingcarList[i].zjurl+'" target="_blank">'+weddingcarList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+weddingcarList[i].duration+'/'+weddingcarList[i].kilometre+'</td>');
				list.push('  <td class="row5">'+weddingcarList[i].weight+'</td>');
				list.push('  <td class="row7">'+weddingcarList[i].price+'</td>');
				var state = "";
				switch (weddingcarList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+weddingcarList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+weddingcarList[i].id+'" data-title="'+weddingcarList[i].title+'" href="weddingcarAdd.php?dopost=edit&id='+weddingcarList[i].id+'" title="修改" class="edit">修改</a>');
