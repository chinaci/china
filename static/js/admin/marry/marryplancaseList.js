$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryplancaseList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryplancaseList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("marryplancaseEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryplancaseList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, plancaseList = val.plancaseList;
		...
		if(val.state == "100"){
			...
			for(i; i < plancaseList.length; i++){
				list.push('<tr data-id="'+plancaseList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+plancaseList[i].title+'</a>&nbsp;&nbsp;<a href="'+plancaseList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(plancaseList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+plancaseList[i].zjurl+'" target="_blank">'+plancaseList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+plancaseList[i].hoteltitle+'</td>');
				list.push('  <td class="row5">'+plancaseList[i].weight+'</td>');
				list.push('  <td class="row7">'+plancaseList[i].holdingtime+'</td>');
				var state = "";
				switch (plancaseList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+plancaseList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+plancaseList[i].id+'" data-title="'+plancaseList[i].title+'" href="marryplancaseAdd.php?dopost=edit&id='+plancaseList[i].id+'" title="修改" class="edit">修改</a>');
