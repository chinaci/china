$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhostList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhostList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("marryhostEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".apart", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("marryhostvideo"+id, "marry", title + "作品", "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryhostList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, hostList = val.hostList;
		...
		if(val.state == "100"){
			...
			for(i; i < hostList.length; i++){
				list.push('<tr data-id="'+hostList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+hostList[i].hostname+'</a>&nbsp;&nbsp;<a href="'+hostList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(hostList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+hostList[i].zjurl+'" target="_blank">'+hostList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+hostList[i].tel+'</td>');
				list.push('  <td class="row5">'+hostList[i].weight+'</td>');
				list.push('  <td class="row7">'+hostList[i].price+'</td>');
				var state = "";
				switch (hostList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+hostList[i].pubdate+'</td>');
				list.push('  <td class="row10">[&nbsp;');
				list.push('<a data-id="'+hostList[i].id+'" data-title="'+hostList[i].hostname+'" href="marryhostAdd.php?dopost=edit&id='+hostList[i].id+'" title="修改" class="link edit">修改</a>&nbsp;|&nbsp;');
				list.push('<a data-id="'+hostList[i].id+'" data-title="'+hostList[i].hostname+'" href="marryhostvideo.php?action=hostvideo&id='+hostList[i].id+'" title="作品" class="link apart">作品</a>&nbsp;|&nbsp;');
				list.push('<a href="javascript:;" title="删除" class="link del">删除</a>&nbsp;]</td>');
				list.push('</tr>');
			}
