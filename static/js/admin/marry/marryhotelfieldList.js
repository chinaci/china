$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhotelfieldList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhotelfieldList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("marryhotelfieldEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryhotelfieldList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, hotelfieldList = val.hotelfieldList;
		...
		if(val.state == "100"){
			...
			for(i; i < hotelfieldList.length; i++){
				list.push('<tr data-id="'+hotelfieldList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+hotelfieldList[i].title+'</a>&nbsp;&nbsp;<a href="'+hotelfieldList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(hotelfieldList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+hotelfieldList[i].zjurl+'" target="_blank">'+hotelfieldList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+hotelfieldList[i].maxtable+'</td>');
				list.push('  <td class="row5">'+hotelfieldList[i].weight+'</td>');
				list.push('  <td class="row7">'+hotelfieldList[i].besttable+'</td>');
				var state = "";
				switch (hotelfieldList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+hotelfieldList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+hotelfieldList[i].id+'" data-title="'+hotelfieldList[i].title+'" href="marryhotelfieldAdd.php?dopost=edit&id='+hotelfieldList[i].id+'" title="修改" class="edit">修改</a>');
