$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhotelmenuList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryhotelmenuList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("marryhotelmenuEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryhotelmenuList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, hotelmenuList = val.hotelmenuList;
		...
		if(val.state == "100"){
			...
			for(i; i < hotelmenuList.length; i++){
				list.push('<tr data-id="'+hotelmenuList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" >'+hotelmenuList[i].title+'</a>&nbsp;&nbsp;<a href="'+hotelmenuList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(hotelmenuList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+hotelmenuList[i].zjurl+'" target="_blank">'+hotelmenuList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+hotelmenuList[i].price+'</td>');
				list.push('  <td class="row7">'+hotelmenuList[i].weight+'</td>');
				var state = "";
				switch (hotelmenuList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+hotelmenuList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+hotelmenuList[i].id+'" data-title="'+hotelmenuList[i].title+'" href="marryhotelmenuAdd.php?dopost=edit&id='+hotelmenuList[i].id+'" title="修改" class="edit">修改</a>');
