		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryplanmealList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("marryplanmealList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("marryplanmealEdit"+id, "marry", title, "marry/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("marryplanmealList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, planmealList = val.planmealList;
		...
		if(val.state == "100"){
			...
			for(i; i < planmealList.length; i++){
				list.push('<tr data-id="'+planmealList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" >'+planmealList[i].title+'</a>&nbsp;&nbsp;<a href="'+planmealList[i].url+'" target="_blank"><i class="icon-share" style="margin-top:4px;"></i></a></td>');
				if(planmealList[i]['zjcom'] != "0"){
					list.push('  <td class="row25 left"><a href="'+planmealList[i].zjurl+'" target="_blank">'+planmealList[i].comname+"&nbsp;&nbsp;"+'</a></td>');
				}else{
					list.push('  <td class="row25 left"><span style="color:#f60;">个人</span></td>');
				}
				list.push('  <td class="row10">'+planmealList[i].price+'</td>');
				list.push('  <td class="row5">'+planmealList[i].weight+'</td>');
				list.push('  <td class="row7">'+planmealList[i].click+'</td>');
				var state = "";
				switch (planmealList[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+planmealList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+planmealList[i].id+'" data-title="'+planmealList[i].title+'" href="marryplanmealAdd.php?dopost=edit&id='+planmealList[i].id+"&typeid="+typeid+'" title="修改" class="edit">修改</a>');
