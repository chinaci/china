$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperForum.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperForum.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	//报刊日期
	$("#cDate").datetimepicker({format: 'yyyy-mm-dd', minView: 3, autoclose: true, language: 'ch'});

	//搜索
	$("#searchBtn").bind("click", function(){
		$("#sKeyword").html($("#keyword").val());
		$("#sCompany").html($("#cCompany").attr("data-id"));
		$("#sDate").html($("#cDate").val());
	...
	//二级菜单点击事件
	$("#cCompany a").bind("click", function(){
		var id = $(this).attr("data-id"), title = $(this).text();
		$("#cCompany").attr("data-id", id);
		$("#cCompany button").html(title+'<span class="caret"></span>');
	});
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("paperForumAdd", "paper", "新增报刊版面", "paper/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("paperForumEdit"+id, "paper", title, "paper/"+href);
		} catch(e) {}
	});

	//内容
	$("#list").delegate(".content", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			href = $(this).attr("href");
		...
		try {
			event.preventDefault();
			parent.addPage("paperContent"+id, "paper", title+"内容", "paper/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此版面下的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	$("#list").delegate(".delete", "click", function(){
		$.dialog.confirm('确定后将删除此版面下的所有信息<br />您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("sCompany="+sCompany);
		data.push("sDate="+sDate);
		data.push("state="+state);
		data.push("pagestep="+pagestep);
		data.push("page="+page);

	huoniao.operaJson("paperForum.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, paperForum = val.paperForum;
		...
		if(val.state == "100"){
			...
			for(i; i < paperForum.length; i++){
				list.push('<tr data-id="'+paperForum[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="'+paperForum[i].forum+'" target="_blank">'+paperForum[i].title+'</a></td>');
				list.push('  <td class="row15 left"><a href="'+paperForum[i].list+'" target="_blank">'+paperForum[i].company+'</a></td>');
				list.push('  <td class="row10 left">'+paperForum[i].date+'</td>');
				var type = "普通";
				if(paperForum[i].type == 1){
					type = "DM报刊";
				}
				list.push('  <td class="row10 left">'+type+'</td>');
				list.push('  <td class="row15 left">'+paperForum[i].pubdate+'</td>');
				var state = "";
				switch (paperForum[i].state) {
					case "0":
				...
				list.push('<a data-id="'+paperForum[i].id+'" data-title="'+paperForum[i].title+'" href="paperForumAdd.php?dopost=edit&id='+paperForum[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');

				if(paperForum[i].type == 0){
					list.push('<a data-id="'+paperForum[i].id+'" data-title="'+paperForum[i].title+'" href="paperContent.php?forum='+paperForum[i].id+'" title="内容" class="link content">内容</a>&nbsp;|&nbsp;');
				}
