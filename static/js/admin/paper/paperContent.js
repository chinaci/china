$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperContent.php?dopost=del&forum="+forum, "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperContent.php?dopost=updateState&forum="+forum, "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("paperContentAdd"+forum, "paper", "新增报刊内容", "paper/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("paperContentEdit"+id, "paper", title, "paper/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("paperContent.php?dopost=getList&forum="+forum, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, paperContent = val.paperContent;
		...
		if(val.state == "100"){
			...
			for(i; i < paperContent.length; i++){
				list.push('<tr data-id="'+paperContent[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><a href="'+paperContent[i].url+'" target="_blank">'+paperContent[i].title+'</a></td>');
				list.push('  <td class="row10 left">'+paperContent[i].author+'</td>');
				list.push('  <td class="row10 left">'+paperContent[i].from+'</td>');
				list.push('  <td class="row15 left">'+paperContent[i].pubdate+'</td>');
				var state = "";
				switch (paperContent[i].state) {
					case "0":
				...
				list.push('<a data-id="'+paperContent[i].id+'" data-title="'+paperContent[i].title+'" href="paperContentAdd.php?dopost=edit&id='+paperContent[i].id+'&forum='+forum+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
