$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperCompany.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("paperCompany.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("paperCompanyAdd", "paper", "新增报刊公司", "paper/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("paperCompanyEdit"+id, "paper", title, "paper/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".del", "click", function(){
		$.dialog.confirm('确定后将删除此公司下的所有信息<br />您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("paperCompany.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, paperCompany = val.paperCompany;
		...
		if(val.state == "100"){
			...
			for(i; i < paperCompany.length; i++){
				list.push('<tr data-id="'+paperCompany[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row40 left"><img src="'+cfg_attachment+paperCompany[i].litpic+'" width="100" />&nbsp;&nbsp;<a href="'+paperCompany[i].url+'" target="_blank">'+paperCompany[i].title+'</a></td>');
				list.push('  <td class="row10 left">'+paperCompany[i].cityname+'</td>');
				list.push('  <td class="row17 left">'+paperCompany[i].pubdate+'</td>');
				var state = "";
				switch (paperCompany[i].state) {
					case "0":
				...
				list.push('<a data-id="'+paperCompany[i].id+'" data-title="'+paperCompany[i].title+'" href="paperCompanyAdd.php?dopost=edit&id='+paperCompany[i].id+'" title="修改" class="edit">修改</a>');
