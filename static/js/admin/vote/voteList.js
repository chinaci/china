		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson(action+"List.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson(action+"List.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   //填充分站列表
	...
	$("#delAllGray").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			...
			huoniao.operaJson(action+"List.php?dopost=delAllGray", "", function(data){
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage(action+"Addphp", action, "发布"+actiontxt, action+"/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("editinfo"+id, action, title, action+"/"+href);
		} catch(e) {}
	});
	...
	$("#refreshBtn").bind("click", function(){
		...
		if(checked.length < 1){
			...
			huoniao.operaJson(action+"List.php?dopost=updateTime", "id="+id, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
					$("#selectBtn a:eq(1)").click();
					setTimeout(function() {
	...
	//添加选手
	$("#list").delegate(".addUser","click", function(){
		var href = $(this).attr("href");

		try {
			event.preventDefault();
			$('#nav-voteAddUserphp label s', window.parent.document).click();
			parent.addPage(action+"AddUserphp", action, "发布选手", action+"/"+href);
		} catch(e) {}
	});
	//管理选手
	$("#list").delegate(".manageUser","click", function(){
		var href = $(this).attr("href");

		try {
			event.preventDefault();
			$('#nav-voteUserphp label s', window.parent.document).click();
			parent.addPage(action+"Userphp", action, "选手管理", action+"/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
  	...
  	data.push("cityid="+$("#cityList").val());
		data.push("state="+state);
		data.push("pagestep="+pagestep);
		data.push("page="+page);

	huoniao.operaJson(action+"List.php?dopost=getList", data.join("&"), function(val){
		...
		$(".totalbaomingend").html(val.pageInfo.totalBaomingEnd);
		$(".totalEnd").html(val.pageInfo.totalEnd);
		...
		if(val.state == "100"){
			...
			for(i; i < infoList.length; i++){
				...
				var color = "";
				if(infoList[i].color != ""){
					color = " style='color:"+infoList[i].color+"'";
				}

				list.push('  <td class="row25 left"><a href="'+infoList[i].url+'" target="_blank">'+infoList[i].title+'</a></td>');
				list.push('  <td class="row10">'+infoList[i].cityname+'</td>');
				list.push('  <td class="row15">'+infoList[i].state+'</td>');
				list.push('  <td class="row12">'+(infoList[i].state != 0 ? infoList[i].date : '')+'</td>');
				list.push('  <td class="row12">'+infoList[i].end+'</td>');
				var arcrank = "";
				switch (infoList[i].arcrank) {
					case "等待审核":
				...
				list.push('  <td class="row13 state">'+arcrank+'<span class="more"><s></s></span></td>');

				list.push('  <td class="row10">');
				// list.push('<a data-id="'+infoList[i].id+'" data-title="'+infoList[i].title+'" href="'+action+'Add.php?dopost=edit&id='+infoList[i].id+'" title="修改" class="edit">修改</a>');
