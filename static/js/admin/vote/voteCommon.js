		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("voteCommon.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("voteCommon.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#userTitle").html(data.name);
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("voteCommon.php?dopost=delCommon", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("voteCommon.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("voteCommon.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row20 title left">信息：<a href="'+commonList[i].tUrl+'" target="_blank">'+commonList[i].tTitle+'</a><br />内容：'+commonList[i].commonContent+'</td>');
				list.push('  <td class="row15"><a href="'+commonList[i].uUrl+'" target="_blank">'+commonList[i].uName+'</a></td>');
