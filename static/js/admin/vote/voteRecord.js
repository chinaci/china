$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson(action+"Record.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson(action+"Record.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#delAllGray").bind("click", function(){
		...
		$.dialog.confirm('此操作不可恢复，您确定要删除吗？', function(){
			...
			huoniao.operaJson(action+"Record.php?dopost=delAllGray", "", function(data){
...
function getList(){
	...
	huoniao.operaJson(action+"Record.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < infoList.length; i++){
				...
				list.push('  <td class="row12"><a href="'+infoList[i].uUrl+'" target="_blank">'+infoList[i].user+'</a></td>');
				list.push('  <td class="row25">'+infoList[i].title+'</td>');
				list.push('  <td class="row12">'+infoList[i].pubdatef+'</td>');
				list.push('  <td class="row12">'+infoList[i].ip+'</td>');
				list.push('  <td class="row12">'+infoList[i].mname+'</td>');
				list.push('  <td class="row12">'+infoList[i].mtel+'</td>');
