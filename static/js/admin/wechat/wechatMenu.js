$(function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row40 left"><input type="text" data-type="typename" class="typename" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
						typeList.push('  <div class="row20 left"><input type="text" data-type="key" class="key" class="input-small" data-id="'+jsonArray["id"]+'" value="'+jsonArray["key"]+'"></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row40 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input type="text" data-type="typename" class="typename" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
						typeList.push('  <div class="row20 left"><input type="text" data-type="key" class="key" data-id="'+jsonArray["id"]+'" value="'+jsonArray["key"]+'"></div>');
					}
	...
	$("#searchBtn").bind("click", function(){
		...
		for(var i = 0; i < l; i++){
			(function(){
				var jsonArray =arguments[0], jArray = jsonArray.lower;
				if(jsonArray["typename"].indexOf(keyword) > -1 || jsonArray["key"].indexOf(keyword) > -1){
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row40 left"><input data-id="0" class="typename" type="text" value=""><a href="javascript:;" class="add-type" data-level="0">添加下级分类</a></div>');
		html.push('    <div class="row20 left"><input data-id="0" class="key" type="text" value=""></div>');
	...
	$(".root").delegate(".add-type", "click", function(){
		...
		html.push('    <div class="row40 left"><span class="plus-icon" style="margin-left:'+Number(margin+45)+'px"></span><input data-id="" class="typename" type="text" value="">'+addType+'</div>');
		html.push('    <div class="row20 left"><input data-id="" class="key" type="text" value=""></div>');
		html.push('    <div class="row20"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row17 left"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
		...
		huoniao.operaJson("wechatMenu.php?dopost=release", "", function(data){
			...
			if(data.state == 100){
				$.dialog({
					title: '提示消息',
					icon: 'success.png',
					content: '发布成功！<br />由于微信客户端缓存，需要24小时微信客户端才会展现出来。'
				});
			}else{
				$.dialog({
					title: '提示消息',
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), typename = $(html).find(".typename"), id = typename.attr("data-id"), typenameVal = typename.val(), key = $(html).find(".key"), id = key.attr("data-id"), keyVal = key.val();

			if(jArray.length > 0 && typenameVal != ""){
				json = json + '{"id": "'+id+'", "typename": "'+encodeURIComponent(typenameVal)+'", "key": "'+encodeURIComponent(keyVal)+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						arguments.callee(jArray[k]);
					}else{
						var typename = $(jArray[k]).find(".typename"), id = typename.attr("data-id"), typenameVal = typename.val(), key = $(jArray[k]).find(".key"), id = key.attr("data-id"), keyVal = key.val();
						if(typenameVal != ""){
							json = json + '{"id": "'+id+'", "typename": "'+encodeURIComponent(typenameVal)+'", "key": "'+encodeURIComponent(keyVal)+'", "lower": null},';
						}else{
							count++;
						}
					}
