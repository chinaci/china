		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationConstruction.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationConstruction.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationConstructionAdd", "renovation", "新增", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationConstructionEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".write", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationConstructionList"+id, "renovation", title+"日记", "renovation/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("renovationConstruction.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationConstruction = val.renovationConstruction;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationConstruction.length; i++){
				list.push('<tr data-id="'+renovationConstruction[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><span>'+renovationConstruction[i].title+'</span></td>');
				list.push('  <td class="row20 left"><a href="'+renovationConstruction[i].curl+'" target="_blank">'+renovationConstruction[i].company+'</a></td>');
				list.push('  <td class="row10 left">'+renovationConstruction[i].dqstagarr.stageName+'</td>');
				var state = "";
				switch (renovationConstruction[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+renovationConstruction[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+renovationConstruction[i].id+'" data-title="'+renovationConstruction[i].title+'" href="renovationConstructionAdd.php?dopost=edit&id='+renovationConstruction[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
