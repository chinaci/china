		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationArticlesList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationArticlesList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationArticlesListAdd", "renovation", "新增施工案例", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationArticlesListEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".write", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationArticlesListList"+id, "renovation", title+"日记", "renovation/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("renovationArticlesList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationArticlesList = val.renovationArticlesList;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationArticlesList.length; i++){
				list.push('<tr data-id="'+renovationArticlesList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><span><a href="'+renovationArticlesList[i].url+'" target="_blank">'+renovationArticlesList[i].title+'</a></span></td>');
				list.push('  <td class="row20 left"><a href="javascript:;" target="_blank">'+renovationArticlesList[i].typenam+'</a></td>');
				list.push('  <td class="row10 left"><a href="'+renovationArticlesList[i].url+'" target="_blank">'+renovationArticlesList[i].name+'</a></td>');
				var state = "";
				switch (renovationArticlesList[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+renovationArticlesList[i].pubdate+'</td>');
				list.push('  <td class="row12"><a href="javascript:;" title="删除" class="delete">删除</a></td>');
