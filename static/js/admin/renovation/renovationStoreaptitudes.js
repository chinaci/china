		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationStoreaptitudes.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationStoreaptitudes.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("renovationStoreaptitudes.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < renovationArticlesList.length; i++){
				...
				list.push('  <td class="row35 left"><img src="'+cfg_attachment+renovationArticlesList[i].litpic+'&type=small" class="litpic" /><span><a style="pointer-events:none;" href="'+renovationArticlesList[i].url+'" target="_blank">'+renovationArticlesList[i].title+'</a></span></td>');
				list.push('  <td class="row25 left"><a href="javascript:;" style="pointer-events:none;">'+renovationArticlesList[i].company+'</a></td>');
				var state = "";
