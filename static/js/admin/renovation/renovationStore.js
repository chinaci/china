$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationStore.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationStore.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationStoreAdd", "renovation", "新增装修公司", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationStoreEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".sale", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationStoreSale"+id, "renovation", title+"促销", "renovation/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此公司下的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("renovationStore.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationStore = val.renovationStore;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationStore.length; i++){
				list.push('<tr data-id="'+renovationStore[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left"><img src="'+cfg_attachment+renovationStore[i].logo+'&type=large" class="litpic" /><span><a href="'+renovationStore[i].url+'" target="_blank">'+renovationStore[i].company+'</a><br><small>【'+renovationStore[i].addrname+'】</small></span></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" class="userinfo" data-id="'+renovationStore[i].userid+'">'+renovationStore[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+renovationStore[i].contact+'</td>');
				var certi = "";
				switch (renovationStore[i].certi) {
					case "0":
				...
				list.push('  <td class="row9">'+certi+'</td>');
				var state = "";
				switch (renovationStore[i].state) {
					case "0":
				...
				list.push('  <td class="row13">'+renovationStore[i].pubdate+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+renovationStore[i].id+'" data-title="'+renovationStore[i].company+'" href="renovationStoreAdd.php?dopost=edit&id='+renovationStore[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
				// list.push('<a data-id="'+renovationStore[i].id+'" data-title="'+renovationStore[i].company+'" href="renovationStoreSale.php?store='+renovationStore[i].id+'" title="促销" class="link sale">促销</a>&nbsp;|&nbsp;');
