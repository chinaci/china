		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationEntrust.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("renovationEntrust.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#company").html(data.company);
							if(data.user){
							...
							self.parent.$("#appointment").html(huoniao.transTimes(data.appointment, 1));
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationEntrust.php?dopost=delEntrust", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationEntrust.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	var sKeyword = encodeURIComponent($("#sKeyword").html()),
    	...
    	cityid    = $("#cityList").val() ? $("#cityList").val() : "";
	...
	huoniao.operaJson("renovationEntrust.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < guestList.length; i++){
				...
				list.push('  <td class="row20 left">'+guestList[i].people+'</td>');
				list.push('  <td class="row20 left">'+guestList[i].contact+'</td>');
				list.push('  <td class="row17 left">'+guestList[i].pubdate+'</td>');
				...
				list.push('  <td class="row13 state">'+state+'<span class="more"><s></s></span></td>');
