		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationVisit.php?dopost=delVisit", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationVisit.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("renovationVisit.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < guestList.length; i++){
				...
				list.push('  <td class="row25 left"><a href="'+guestList[i].curl+'" target="_blank">'+guestList[i].case+'</a></td>');
				...
				list.push('  <td class="row10"><a href="javascript:;" data-id='+guestList[i].id+'" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
