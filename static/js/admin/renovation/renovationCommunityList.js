$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationCommunity.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationCommunity.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationCommunityAdd", "renovation", "新增小区", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationCommunityEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});


	//户型图
	$("#list").delegate(".renovationpic", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			href = $(this).attr("href");
		...
		try {
			event.preventDefault();
			parent.addPage("renovationCommunityAdd"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});

	// $("#list").delegate(".renovationpic1", "click", function(event){
	// 	var id = $(this).attr("data-id"),
	// 		title = $(this).attr("data-title"),
	// 		href = $(this).attr("href");
...
function getList(){
	...
	huoniao.operaJson("renovationCommunity.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationCommunity = val.renovationCommunity;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationCommunity.length; i++){
				list.push('<tr data-id="'+renovationCommunity[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+renovationCommunity[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row35 left">'+img+'<span><a href="'+renovationCommunity[i].url+'" target="_blank">'+renovationCommunity[i].title+'</a> </br> <a data-title="户型图"data-id="'+renovationCommunity[i].id+'" href="renovationCommunityAdd.php?dopost=editpic&type=1&id='+renovationCommunity[i].id+'" class="renovationpic">户型图</a> | <a data-title="实景图"data-id="'+renovationCommunity[i].id+'" href="renovationCommunityAdd.php?dopost=editpic&type=2&id='+renovationCommunity[i].id+'" class="renovationpic">实景图</a></span></td>');
				list.push('  <td class="row10"><a href="javascript:;" data-id="'+renovationCommunity[i].addrid+'" class="addr">'+renovationCommunity[i].addr+'</a></td>');
				list.push('  <td class="row10">'+renovationCommunity[i].weight+'</td>');
				list.push('  <td class="row12">&yen;'+renovationCommunity[i].price+'</td>');
				var state = "";
				switch (renovationCommunity[i].state) {
					case "0":
				...
				list.push('<a data-id="'+renovationCommunity[i].id+'" data-title="'+renovationCommunity[i].title+'" href="renovationCommunityAdd.php?dopost=edit&id='+renovationCommunity[i].id+'" title="修改" class="edit">修改</a>');
