		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationDiary.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationDiary.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
   ...
   huoniao.choseCity($(".choseCity"),$("#cityList"));
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationDiaryAdd", "renovation", "新增施工案例", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationDiaryEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});

	//写日记
	$("#list").delegate(".write", "click", function(event){
		var id = $(this).attr("data-id"),
			title = $(this).attr("data-title"),
			href = $(this).attr("href");
		...
		try {
			event.preventDefault();
			parent.addPage("renovationDiaryList"+id, "renovation", title+"日记", "renovation/"+href);
		} catch(e) {}
	});

	//删除
	$("#delBtn").bind("click", function(){
		$.dialog.confirm('确定后将删除此日记下的所有信息<br />此操作不可恢复，您确定要删除吗？', function(){
			init.del();
		});
	});
	...
	$("#list").delegate(".delete", "click", function(){
		$.dialog.confirm('确定后将删除此日记下的所有信息<br />您确定要删除吗？', function(){
			init.del();
		});
	});
...
function getList(){
	...
	huoniao.operaJson("renovationDiary.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationDiary = val.renovationDiary;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationDiary.length; i++){
				list.push('<tr data-id="'+renovationDiary[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left"><img src="'+cfg_attachment+renovationDiary[i].litpic+'&type=small" class="litpic" /><span><a href="'+renovationDiary[i].url+'" target="_blank">'+renovationDiary[i].title+'</a></span></td>');
				list.push('  <td class="row20 left"><a href="'+renovationDiary[i].curl+'" target="_blank">'+renovationDiary[i].company+'</a></td>');
				list.push('  <td class="row10 left"><a href="'+renovationDiary[i].durl+'" target="_blank">'+renovationDiary[i].teamname+'</a></td>');
				var state = "";
				switch (renovationDiary[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+renovationDiary[i].pubdate+'</td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+renovationDiary[i].id+'" data-title="'+renovationDiary[i].title+'" href="renovationDiaryAdd.php?dopost=edit&id='+renovationDiary[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
				// list.push('<a data-id="'+renovationDiary[i].id+'" data-title="'+renovationDiary[i].title+'" href="renovationDiaryList.php?diary='+renovationDiary[i].id+'" title="写日记" class="link write">写日记</a>&nbsp;|&nbsp;');
