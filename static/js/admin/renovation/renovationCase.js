$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationCase.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationCase.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationCaseAdd", "renovation", "新增效果图", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationCaseEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">未审核</a></li>');
...
function getList(){
	...
	huoniao.operaJson("renovationCase.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationCase = val.renovationCase;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationCase.length; i++){
				list.push('<tr data-id="'+renovationCase[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
			// <img src="'+cfg_attachment+renovationCase[i].litpic+'&type=small" class="litpic" />
				list.push('  <td class="row20 left"><span>'+renovationCase[i].title+'</span></td>');
				list.push('  <td class="row30 left"><a href="'+renovationCase[i].curl+'" target="_blank">'+renovationCase[i].company+'</a></td>');
				// list.push('  <td class="row10 left"><a href="'+renovationCase[i].durl+'" target="_blank">'+renovationCase[i].teamname+'</a></td>');
				list.push('  <td class="row15 left">'+renovationCase[i].apartment+'</td>');
				var state = "";
				switch (renovationCase[i].state) {
					case "0":
				...
				list.push('  <td class="row12">'+renovationCase[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+renovationCase[i].id+'" data-title="'+renovationCase[i].title+'" href="renovationCaseAdd.php?dopost=edit&id='+renovationCase[i].id+'" title="修改" class="edit">修改</a>');
