$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationToubiao.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationToubiao.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function(){
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("renovationToubiaoEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">待业主评选</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">入围</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">中标</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">淘汰</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("renovationToubiao.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationToubiao = val.renovationToubiao;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationToubiao.length; i++){
				list.push('<tr data-id="'+renovationToubiao[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row15 left"><a href="javascript:;" data-id="'+renovationToubiao[i].userid+'" class="user">'+renovationToubiao[i].username+'</a></td>');
				list.push('  <td class="row40 left"><a href="/zhaobiao.php?id='+renovationToubiao[i].id+'" target="_blank">'+renovationToubiao[i].title+'</a></td>');
				list.push('  <td class="row15 left">'+renovationToubiao[i].pubdate+'</td>');
				var state = "";
				switch (renovationToubiao[i].state) {
					case "0":
						state = '<span class="gray">待审核</span>';
						break;
					case "1":
						state = '<span>待业主评选</span>';
						break;
					case "2":
						state = '<span>入围</span>';
						break;
					case "3":
						state = '<span class="audit">中标</span>';
						break;
					case "4":
						state = '<span class="refuse">淘汰</span>';
						break;
				}
				list.push('  <td class="row12 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row15">');
				list.push('<a data-id="'+renovationToubiao[i].id+'" data-title="'+renovationToubiao[i].username+'" href="renovationToubiaoAdd.php?dopost=edit&id='+renovationToubiao[i].id+'" title="修改" class="edit">修改</a>');
