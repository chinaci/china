$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationZhaobiao.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("renovationZhaobiao.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$(".dropdown-toggle").bind("click", function(){
		if($(this).parent().attr("id") != "industryBtn" && $(this).parent().attr("id") != "addrBtn"){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			...
			parent.addPage("renovationZhaobiaoAdd", "renovation", "新增装修招标", "renovation/"+href);
		} catch(e) {}
	});

	//投标管理
	$("#toubiao").bind("click", function(event){
		var href = $(this).attr("href");

		try {
			event.preventDefault();
			parent.addPage("renovationToubiao", "renovation", "投标管理", "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("renovationZhaobiaoEdit"+id, "renovation", title, "renovation/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".more", "click", function(event){
		...
		var htmlCreateStateMenu = function(){
			...
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">招标中</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">招标成功</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">招标结束</a></li>');
			htmlMenu.push('      <li class="smart_menu_li"><a href="javascript:" class="smart_menu_a">停止招标</a></li>');
			htmlMenu.push('    </ul>');
			htmlMenu.push('  </div>');
			htmlMenu.push('</div>');
...
function getList(){
	...
	huoniao.operaJson("renovationZhaobiao.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, renovationZhaobiao = val.renovationZhaobiao;
		...
		if(val.state == "100"){
			...
			for(i; i < renovationZhaobiao.length; i++){
				var companystr = '';
				var company = renovationZhaobiao[i].company;

				for (var a = 0; a < company.length; a++) {
					companystr  += '<a href="'+ company[a].url +'" target="_blank">'+ company[a].company+'</a>,'
				}

				list.push('<tr data-id="'+renovationZhaobiao[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left">'+renovationZhaobiao[i].title+'('+companystr+')</td>');
				list.push('  <td class="row10 left">'+renovationZhaobiao[i].people+'</td>');
				list.push('  <td class="row12 left">'+renovationZhaobiao[i].contact+'</td>');
				list.push('  <td class="row10 left">'+renovationZhaobiao[i].addr+'</td>');
				var state = "";
				switch (renovationZhaobiao[i].state) {
					case "0":
						state = '<span class="gray">待审核</span>';
						break;
					case "1":
						state = '<span class="audit">招标中</span>';
						break;
					case "2":
						state = '<span class="audit">招标成功</span>';
						break;
					case "3":
						state = '<span class="gray">招标结束</span>';
						break;
					case "4":
						state = '<span class="refuse">停止招标</span>';
						break;
				}
				list.push('  <td class="row10 state">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row15">'+renovationZhaobiao[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+renovationZhaobiao[i].id+'" data-title="'+renovationZhaobiao[i].title+'" href="renovationZhaobiaoAdd.php?dopost=edit&id='+renovationZhaobiao[i].id+'" title="修改" class="edit">修改</a>');
