$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("gwList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("gwList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#stateBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#typeBtn")
				.attr("data-id", "")
				.find("button").html('全部开发商<span class="caret"></span>');
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("gwListEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("gwList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, gwList = val.gwList;
		...
		if(val.state == "100"){
			...
			for(i; i < gwList.length; i++){
				list.push('<tr data-id="'+gwList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row25 left">'+gwList[i].typename+'</td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+gwList[i].userid+'" class="userinfo">'+gwList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+gwList[i].addr+'</td>');
				list.push('  <td class="row25 left">'+gwList[i].stores+'</td>');
				list.push('  <td class="row5">'+gwList[i].weight+'</td>');
				var state = "";
				switch (gwList[i].state) {
					case "0":
				...
				list.push('  <td class="row10">'+state+'<span class="more"><s></s></span></td>');
				list.push('  <td class="row12"><a data-id="'+gwList[i].id+'" data-title="'+gwList[i].username+' 顾问" href="gwList.php?dopost=edit&action=edit&id='+gwList[i].id+'" title="修改" class="edit">修改</a><a href="javascript:;" title="删除" class="del">删除</a></td>');
				list.push('</tr>');
			}
