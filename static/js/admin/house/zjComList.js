$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("zjComList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("zjComList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			...
			parent.addPage("zjComEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("zjComList.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, zjComList = val.zjComList;
		...
		if(val.state == "100"){
			...
			for(i; i < zjComList.length; i++){
				list.push('<tr data-id="'+zjComList[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row20 left"><a href="'+zjComList[i].url+'" target="_blank">'+zjComList[i].title+'</a></td>');
				list.push('  <td class="row5">'+zjComList[i].cityname+'</td>');
				list.push('  <td class="row7"><a href="javascript:;" data-id="'+zjComList[i].userid+'" class="userinfo">'+zjComList[i].username+'</a></td>');
				list.push('  <td class="row10 left">'+zjComList[i].tel+'</td>');
				list.push('  <td class="row15 left">'+zjComList[i].address+'</td>');
				var flag = "";
				switch (zjComList[i].flag) {
					case "0":
				...
				list.push('  <td class="row13">'+zjComList[i].pubdate+'</td>');
				list.push('  <td class="row10">');
				list.push('<a data-id="'+zjComList[i].id+'" data-title="'+zjComList[i].title+'" href="zjComAdd.php?dopost=edit&id='+zjComList[i].id+'" title="修改" class="edit">修改</a>');
