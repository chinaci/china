$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("releaseschoolList.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("releaseschoolList.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#list").delegate(".modify", "click", function(event){
		...
		try {
			...
			parent.addPage("releaseschoolListEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("releaseschoolList.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < communityList.length; i++){
				...
				var img = '<img src="'+cfg_attachment+communityList[i].logo+'&type=small" class="litpic" />';
				...
				list.push('  <td class="row20"><a href="javascript:;" data-id="'+communityList[i].addrid+'" class="addr">'+communityList[i].address+'</a></td>');
				// list.push('  <td class="row10">'+communityList[i].username+'</td>');
				list.push('  <td class="row12">'+communityList[i].typename+'</td>');
				var state = "";
				...
				list.push('<a data-id="'+communityList[i].id+'" data-title="'+communityList[i].title+'" href="releaseschoolAdd.php?dopost=edit&id='+communityList[i].id+'" title="修改" class="link modify">修改</a>&nbsp;|&nbsp;');
