$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("mytag.php?action="+module+"&dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("mytag.php?action="+module+"&dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("add"+module+"Mytag", module, "生成新标签", module+"/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("edit"+module+"Mytag"+id, module, title, module+"/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("mytag.php?action="+module+"&dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, mytag = val.mytag;
		...
		if(val.state == "100"){
			...
			for(i; i < mytag.length; i++){
				list.push('<tr data-id="'+mytag[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row30 left">ID：'+mytag[i].id+'（'+mytag[i].name+'）</td>');
				var type = "";
				switch (mytag[i].type){
					case "loupan":
						type = "楼盘";
				...
				var date = "不限制";
				if(mytag[i].start != "" && mytag[i].end != ""){
					date = mytag[i].start + " 至 " + mytag[i].end;
				}else if(mytag[i].start != "" && mytag[i].end == ""){
					date = mytag[i].start + " 至 " + "不限制";
				}else if(mytag[i].start == "" && mytag[i].end != ""){
					date = "不限制" + " 至 " + mytag[i].end;
				}
				list.push('  <td class="row20">'+date+'</td>');
				var state = "";
				switch (mytag[i].state) {
					case "0":
				...
				list.push('  <td class="row15">'+mytag[i].pubdate+'</td>');
				list.push('  <td class="row12">');
				list.push('<a data-id="'+mytag[i].id+'" data-title="'+mytag[i].name+'" href="mytag.php?action='+module+'&dopost=edit&id='+mytag[i].id+'" title="修改" class="edit">修改</a>');
