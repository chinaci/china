$(function(){
		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCommon.php?dopost=getDetail&action="+action, "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("houseCommon.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							for (var i=0; i<picpath.length;i++){
								str += '<a href="'+picpath[i]+'" class="pics" target="_blank"><img style="height: 100px; margin-right: 15px;" src="'+picpath[i]+'" alt=""></a>';
							}
							self.parent.$("#pisc").html(str);
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCommon.php?dopost=delCommon", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("houseCommon.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	//点击文章标题查找此文章的所有评论
	$("#list").delegate(".article", "click", function(){
		var txt = $(this).text();
		$("#keyword").val(txt);
		$("#typeBtn").attr("data-id", 1);
		$("#typeBtn").find("button").html('文章标题<span class="caret"></span>');
		$("#searchBtn").click();
	});
...
function getList(){
	...
	huoniao.operaJson("houseCommon.php?dopost=getList&action="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">学校：<a href="javascript:;" class="article" data-id='+commonList[i].articleId+'">'+commonList[i].articleTitle+'</a><br />内容：'+commonList[i].commonContent+'</td>');
				
				var user = '<a href="javascript:;" data-id="'+commonList[i].commonUserId+'" class="user">'+commonList[i].commonUserName+'</a>';
