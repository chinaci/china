$(function () {
	...
	$("#btnSubmit").bind("click", function(event){
		...
		$.ajax({
			type: "POST",
			url: "loupanVideo.php?dopost="+$("#dopost").val(),
						...
						$.dialog({
							...
							ok: function(){
								try{
									...
									//parent.reloadPage($("body",parent.document).find("#body-loupanVideophp")[0].contentWindow);
									parent.reloadPage($("body",parent.document).find("#body-360qjloupan"+$("#loupan").val()));
									$("body",parent.document).find("#nav-loupanVideoEdit"+id+" s").click();
								}catch(e){
									location.href = thisPath + "loupanVideo.php?loupan="+$("#loupan").val();
								}
							},
							cancel: false
						});
