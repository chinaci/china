$(function(){
		...
		printTypeTree: function(){
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					typeList.push('<div class="tr clearfix tr_'+jsonArray["id"]+'" data-longitude="'+jsonArray["longitude"]+'" data-latitude="'+jsonArray["latitude"]+'">');
					if(jsonArray["parentid"] == 0){
						typeList.push('  <div class="row3"><a href="javascript:;" class="fold">折叠</a></div>');
						typeList.push('  <div class="row60 left"><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'"><a href="javascript:;" class="markditu" title="标注区域位置"><img src="/static/images/admin/markditu.jpg" /></a>'+addType+'</div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row60 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'"><a href="javascript:;" class="markditu" title="标注区域位置"><img src="/static/images/admin/markditu.jpg" /></a>'+addType+'</div>');
					}
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('<li class="li0" data-longitude="" data-latitude="">');
		html.push('  <div class="tr clearfix">');
		html.push('    <div class="row3"><a href="javascript:;" class="fold">折叠</a></div>');
		html.push('    <div class="row60 left"><input data-id="0" type="text" value=""><a href="javascript:;" class="markditu" title="标注区域位置"><img src="/static/images/admin/markditu.jpg" /></a><a href="javascript:;" class="add-type" data-level="0">添加下级分类</a></div>');
	...
	$(".root").delegate(".add-type", "click", function(){
		...
		html.push('<li class="li'+Number(level+1)+'" data-longitude="" data-latitude="">');
		html.push('  <div class="tr clearfix">');
		html.push('    <div class="row3"></div>');
		html.push('    <div class="row60 left"><span class="plus-icon" style="margin-left:'+Number(margin+45)+'px"></span><input data-id="" type="text" value=""><a href="javascript:;" class="markditu" title="标注区域位置"><img src="/static/images/admin/markditu.jpg" /></a>'+addType+'</div>');
		html.push('    <div class="row20"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row17 left"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
	...
	$(".list").on("click", ".markditu", function(){
		var t = $(this), longitude = t.closest(".tr").attr("data-longitude"), latitude = t.closest(".tr").attr("data-latitude"), addr = t.prev("input").val();
		var lnglat = latitude != "" && longitude != "" ? (longitude+","+latitude) : "";
		$.dialog({
			id: "markDitu",
			title: "标注 "+addr+" 地图位置<small>（请点击/拖动图标到正确的位置，再点击底部确定按钮。）</small>",
			content: 'url:'+adminPath+'../api/map/mark.php?mod=house&lnglat='+lnglat+"&city="+mapCity+"&addr="+addr,
			width: 800,
			height: 500,
			max: true,
			ok: function(){
				...
				t.closest(".tr").attr("data-longitude", lng);
				t.closest(".tr").attr("data-latitude", lat);
			},
			cancel: true
		});
	});
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("houseAddr.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("houseAddr.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){
						...
						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("houseAddr.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	for(var i = 0; i < first.length; i++){
		(function(){
			var html =arguments[0], count = 0, jArray = $(html).find(">ul>li"), tr = $(html).find(".tr input"), id = tr.attr("data-id"), val = tr.val(), longitude = $(html).find(".tr").attr("data-longitude"), latitude = $(html).find(".tr").attr("data-latitude");

			if(jArray.length > 0 && val != ""){
				json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "longitude": "'+longitude+'", "latitude": "'+latitude+'", "lower": [';
				for(var k = 0; k < jArray.length; k++){
					if($(jArray[k]).find(">ul>li").length > 0){
						arguments.callee(jArray[k]);
					}else{
						var tr = $(jArray[k]).find(".tr input"), id = tr.attr("data-id"), val = tr.val(), longitude = $(jArray[k]).find(".tr").attr("data-longitude"), latitude = $(jArray[k]).find(".tr").attr("data-latitude");
						if(val != ""){
							json = json + '{"id": "'+id+'", "name": "'+encodeURIComponent(val)+'", "longitude": "'+longitude+'", "latitude": "'+latitude+'", "lower": null},';
						}else{
							count++;
						}
					}
	...
	huoniao.operaJson("houseAddr.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
