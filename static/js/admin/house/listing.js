$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("listing.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("listing.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() { 
								getList();
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage("listingAdd"+loupanid, "house", "新增"+loupaname+"房源", "house/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("listingEdit"+id, "house", title, "house/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("listing.php?dopost=getList&id="+loupanid, data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, listing = val.listing;
		...
		if(val.state == "100"){
			...
			for(i; i < listing.length; i++){
				list.push('<tr data-id="'+listing[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				var img = '<img src="'+cfg_attachment+listing[i].litpic+'&type=small" class="litpic" />';
				list.push('  <td class="row30 left">'+img+'<span><a href="/article.php?id='+listing[i].id+'" target="_blank">'+listing[i].title+'</a></span></td>');
				list.push('  <td class="row5">'+listing[i].weight+'</td>');
				list.push('  <td class="row10">'+listing[i].price+'</td>');
				list.push('  <td class="row10">'+listing[i].room+'室'+listing[i].hall+'厅'+listing[i].guard+'卫</td>');
				list.push('  <td class="row5">'+listing[i].area+'㎡</td>');
				list.push('  <td class="row10">'+listing[i].username+'</td>');
				list.push('  <td class="row7">'+listing[i].salable+'套</td>');
				var state = "";
				switch (listing[i].state) {
					case "0":
				...
				list.push('<a data-id="'+listing[i].id+'" data-title="'+listing[i].title+'" href="listingAdd.php?dopost=edit&loupanid='+loupanid+'&id='+listing[i].id+'" title="修改" class="edit">修改</a>');
