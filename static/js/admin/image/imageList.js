	...
	init = {
		...
		,quickEdit: function(){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("imageJson.php?action=getArticleDetail&dopost="+action, "id="+id, function(data){
					if(data != null && data.length > 0){
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("imageJson.php?action=updateDetail&dopost="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
			...
			if(checked.length < 1){
				...
				$.dialog({
					...
					ok: function(){
						...
						huoniao.operaJson("imageJson.php?action="+type+"Property&dopost="+action, "id="+id+"&attr="+attr, function(data){
							if(data.state == 100){
								huoniao.showTip("success", data.info, "auto");
								$("#selectBtn a:eq(1)").click();
								setTimeout(function() {
		...
		,del: function(type){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("imageJson.php?action="+type+"&dopost="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						$("#selectBtn a:eq(1)").click();
						setTimeout(function() {
		...
		,revert: function(){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("imageJson.php?action=revert&dopost="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							getList();
			...
			if(checked.length < 1){
				...
				$.dialog({
					...
					ok: function(){
						var typeid = self.parent.$("#typeid").val();
						if(typeid != ""){
							huoniao.operaJson("imageJson.php?action=move&dopost="+action, "id="+id+"&typeid="+typeid, function(data){
								if(data.state == 100){
									huoniao.showTip("success", data.info, "auto");
									setTimeout(function() {
										getList();
		...
		,updateState: function(type){
			...
			if(checked.length < 1){
				...
				huoniao.operaJson("imageJson.php?action=updateState&dopost="+action, "id="+id+"&arcrank="+arcrank, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							getList();
...
$(function(){
	...
	$("#addNew").bind("click", function(){
		...
		try {
			...
			parent.addPage(action+"Add", "image", "添加图片", "image/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("edit"+action+id, "image", title, "image/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("imageJson.php?dopost="+action, data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < articleList.length; i++){
				...
				if(recycle != ""){
					...
					list.push('<a data-id="'+articleList[i].id+'" data-title="'+articleList[i].title+'" href="imageAdd.php?dopost=edit&id='+articleList[i].id+'&action='+action+'" title="修改" class="edit">修改</a>');
				}
				list.push('</td>');
