...
$(function(){
  ...
  $("#stateBtn, #pendBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
	...
	$('#list').delegate('.backcount', 'click', function(){
		var t = $(this), id = t.closest('tr').attr('data-id'), total = t.attr('data-total'), count = t.attr('data-count'), level = t.attr('data-level');
		...
		$.dialog({
			...
			ok: function(){
				//提交
				var level = self.parent.$("#fenxiaoLevel_").val();
				var count = self.parent.$("#count").val();

				huoniao.operaJson("fenxiaoUser.php?action=updateCount", "id="+id+"&level="+level+"&count="+count, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							getList();
		...
		//填充信息
		self.parent.$("#fenxiaoLevel_").val(level);
		self.parent.$("#total_").html(total);
		self.parent.$("#count_").html(count);
		self.parent.$("#count").val(count);
	...
	huoniao.operaJson("?action=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < fenxiaoUser.length; i++){
				...
				list.push('  <td class="row10 left">'+fenxiaoUser[i].city+'</td>');
				list.push('  <td class="row15 left"><code>ID:'+fenxiaoUser[i].userid+'</code>&nbsp;<small style="display: inline-block;"><a href="javascript:;" data-id="'+fenxiaoUser[i].userid+'" class="audit userinfo">'+fenxiaoUser[i].username+'</a></small><br /><small>'+fenxiaoUser[i].phone+'</small></td>');
