$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("memberDepositLog.php?dopost=del", "id="+id, function(data){
						huoniao.hideTip();
						if(data.state == 100){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("memberDepositLogEdit"+id, "member", title, "member/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("memberDepositLog.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, memberDepositLog = val.memberDepositLog;
		...
		if(val.state == "100"){
			...
			for(i; i < memberDepositLog.length; i++){
				list.push('<tr data-id="'+memberDepositLog[i].id+'">');
				// list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row3">&nbsp;</td>');
				var mtype = "个人";
				if(memberDepositLog[i].mtype == 2){
					mtype = "企业";
				}
				list.push('  <td class="row5 left">['+memberDepositLog[i].userid+']<br>'+mtype+'</td>');
				list.push('  <td class="row15 left"><img onerror="javascript:this.src=\'/static/images/default_user.jpg\';" src="'+cfg_attachment+memberDepositLog[i].photo+'&type=small" class="litpic" style="width:60px; height:60px;" /><span>'+memberDepositLog[i].username+'<br /></small></span></td>');
				var company = "";
				var certify = "";
				if(memberDepositLog[i].mtype == 2){
					if(memberDepositLog[i].licenseState == 3){
						certify = " label-warning";
					}else if(memberDepositLog[i].licenseState == 2){
						certify = " label-important";
					}else if(memberDepositLog[i].licenseState == 1){
						certify = " label-success";
					}
					company = '<br /><small>'+memberDepositLog[i].realname+'</small>';
				}else{
					if(memberDepositLog[i].certifyState == 3){
						certify = " label-warning";
					}else if(memberDepositLog[i].certifyState == 2){
						certify = " label-important";
					}else if(memberDepositLog[i].certifyState == 1){
						certify = " label-success";
					}
					company = '<br />'+memberDepositLog[i].realname;
				}



				list.push('  <td class="20 left">'+memberDepositLog[i].nickname+'【'+memberDepositLog[i].sex+'】'+company+'</td>');
				var emailCheck = "", phoneCheck = "";
				if(memberDepositLog[i].emailCheck == 1){
					emailCheck = " label-success";
				}
				if(memberDepositLog[i].phoneCheck == 1){
					phoneCheck = " label-success";
				}
				list.push('  <td class="row12 left"><span class="label'+emailCheck+'" style="margin-right:3px;">验</span>'+memberDepositLog[i].email+'<br /><span class="label'+phoneCheck+'" style="margin-right:3px;">验</span>'+memberDepositLog[i].phone+'</td>');
				list.push('  <td class="row10 left">'+memberDepositLog[i].amount+'</td>');
				list.push('  <td class="row10 left">'+memberDepositLog[i].money+'</td>');

				var paytype = '';
				switch(memberDepositLog[i].paytype){
					case 'wxpay':
						paytype = '微信';
						break;
					case 'alipay':
						...
						break;
					default:
						paytype = memberDepositLog[i].paytype
				}
				list.push('  <td class="row10 left">'+paytype+'</td>');
				list.push('  <td class="row15 left">'+memberDepositLog[i].date+'</td>');
				// list.push('  <td class="row8"><a href="memberDepositLog.php?dopost=Edit&id='+memberDepositLog[i].id+'" data-id="'+memberDepositLog[i].id+'" class="edit" data-title="'+memberDepositLog[i].username+'" title="修改会员信息">修改</a></td>');
				list.push('</tr>');
