		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("pointsLogs.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
...
function getList(){
	...
	huoniao.operaJson("pointsLogs.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), listArr = [], i = 0, list = val.list;
		obj.attr("data-totalpage", val.pageInfo.totalPage);
		$(".totalCount").html(val.pageInfo.totalCount);
		$("#totalAdd").html(val.totalAdd);
		...
		if(val.state == "100"){
			...
			for(i; i < list.length; i++){
				...
				listArr.push('  <td class="row10 left"><a href="javascript:;" data-id="'+list[i].userid+'" class="userinfo">'+list[i].username+'</a></td>');
				listArr.push('  <td class="row25 left">'+list[i].info+'</td>');
				...
				listArr.push('  <td class="row15 left">'+amount+'</td>');
				listArr.push('  <td class="row17 left">'+list[i].date+'</td>');
				listArr.push('  <td class="row10 left"><a href="javascript:;" title="删除" class="del">删除</a></td>');
			}
