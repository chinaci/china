$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("fenxiaoPoster.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("fenxiaoPosterAdd", "member", "添加海报", "member/"+href);
		} catch(e) {}
	});
	...
	$("#typeManage").bind("click", function(event){
		...
		$.dialog({
			...
			ok: function(){
				...
				$.ajax({
					url: "fenxiaoPoster.php?dopost=updateType",
					data: "data=["+data.join(",")+"]",
					type: "POST",
					dataType: "json",
		...
		parent.$('.menu-itemlist').delegate("a", "click", function(){
			...
			if(id != ""){
				$.dialog.confirm("此操作将同时删除分类下的图片，确定要删除吗？", function(){
					...
					$.ajax({
						url: "fenxiaoPoster.php?dopost=delType",
						data: "id="+id,
						type: "POST",
						success: function(){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("fenxiaoPosterEdit"+id, "member", "修改海报", "member/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("fenxiaoPoster.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < memberCoverBg.length; i++){
				...
				list.push('  <td class="row50 left"><img src="'+cfg_attachment+memberCoverBg[i].litpic+'" style="width: 50px; height: 90px; object-fit: cover;" /></td>');
				list.push('  <td class="row30 left">'+memberCoverBg[i].date+'</td>');
				list.push('  <td class="row17">');
				list.push('<a data-id="'+memberCoverBg[i].id+'" data-title="'+memberCoverBg[i].title+'" href="fenxiaoPoster.php?dopost=edit&id='+memberCoverBg[i].id+'" title="修改" class="edit">修改</a>');
