$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("datingCoverBg.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-datingCoverBgphp").click();
								parent.reloadPage($("body",parent.document).find("#body-datingCoverBgphp"));
								$("body",parent.document).find("#nav-datingCoverBgEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "datingCoverBg.php";
							}
						},
						cancel: false
					});
