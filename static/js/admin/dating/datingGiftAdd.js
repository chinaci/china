$(function(){
	...
	$("#btnSubmit").bind("click", function(event){
		...
		//异步提交
		huoniao.operaJson("datingGift.php", $("#editform").serialize() + "&submit="+encodeURI("提交"), function(data){
			if(data.state == 100){
				if($("#dopost").val() == "Add"){
					$.dialog({
						fixed: true,
					...
					$.dialog({
						...
						ok: function(){
							try{
								$("body",parent.document).find("#nav-datingGiftphp").click();
								parent.reloadPage($("body",parent.document).find("#body-datingGiftphp"));
								$("body",parent.document).find("#nav-datingGiftEdit"+id+" s").click();
							}catch(e){
								location.href = thisPath + "datingGift.php";
							}
						},
						cancel: false
					});
