$(function(){
	...
	var init = {
			...
			for(var i = 0; i < l; i++){
				(function(){
					...
					if(jsonArray["parentid"] == 0){
						...
						typeList.push('  <div class="row35 left"><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
						typeList.push('  <div class="row15 vh"></div>');
						typeList.push('  <div class="row10 vh"></div>');
						typeList.push('  <div class="row7 vh"></div>');
						// typeList.push('  <div class="row15 vh">'+(jsonArray["icon"] != '' ? '<img src="'+jsonArray["iconturl"]+'" class="img" alt="" style="height:40px;">' : '')+'<a href="javascript:;" class="upfile" title="删除">上传图标</a><input type="file" name="Filedata" value="" class="imglist-hidden Filedata hide" id="Filedata_'+iconIdx+'"><input type="hidden" name="icon" class="icon" value="'+jsonArray["icon"]+'"></div>');
						// typeList.push('  <div class="row10 vh"><a href="javascript:;" class="item" title="字段">字段</a></div>');
						// typeList.push('  <div class="row7 right vh"><a href="javascript:;" class="edit" title="编辑">编辑</a></div>');
					}else{
						typeList.push('  <div class="row3"></div>');
						typeList.push('  <div class="row35 left"><span class="plus-icon" style="margin-left:'+cl+'px;"></span><em>['+jsonArray["id"]+']</em><input type="text" data-id="'+jsonArray["id"]+'" value="'+jsonArray["typename"]+'">'+addType+'</div>');
						typeList.push('  <div class="row15">'+(jsonArray["icon"] != '' ? '<img src="'+jsonArray["iconturl"]+'" class="img" alt="" style="height:40px;">' : '')+'<a href="javascript:;" class="upfile" title="删除">上传图标</a><input type="file" name="Filedata" value="" class="imglist-hidden Filedata hide" id="Filedata_'+iconIdx+'"><input type="hidden" name="icon" class="icon" value="'+jsonArray["icon"]+'"></div>');
						typeList.push('  <div class="row10"><a href="javascript:;" class="item" title="字段">字段</a></div>');
						typeList.push('  <div class="row7 right"><a href="javascript:;" class="edit" title="编辑">编辑</a></div>');
					}
					typeList.push('  <div class="row15"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
					typeList.push('  <div class="row15 left"><a href="javascript:;" class="del" title="删除">删除编辑</a></div>');
					typeList.push('</div>');
		...
		,quickEdit: function(id, rank){
			if(id == ""){
				...
				huoniao.operaJson("datingActivityType.php?dopost=getTypeDetail&action="+action, "id="+id, function(data){
					if(data != null){
						data = data[0];
						huoniao.hideTip();
						...
						$.dialog({
							...
							ok: function(){
								...
								huoniao.operaJson("datingActivityType.php?dopost=updateType&action="+action, "id="+id+"&"+serialize, function(data){
									if(data.state == 100){
	...
	$("#addNew_").bind("click", function(){
		...
		html.push('    <div class="row67 left"><input data-id="0" type="text" value=""><a href="javascript:;" class="add-type vh" data-level="0">添加下级分类</a></div>');
	...
	$(".root").delegate(".add-type", "click", function(){
		...
		html.push('    <div class="row67 left"><span class="plus-icon" style="margin-left:'+Number(margin+45)+'px"></span><input data-id="" type="text" value="">'+addType+'</div>');
		html.push('    <div class="row15"><a href="javascript:;" class="up">向上</a><a href="javascript:;" class="down">向下</a></div>');
		html.push('    <div class="row10 left"><a href="javascript:;" class="del">删除</a></div>');
		html.push('  </div>');
		html.push('</li>');
	...
	$("#list").delegate("input", "blur", function(){
		...
		if(id != "" && id != 0){
			huoniao.operaJson("datingActivityType.php?dopost=updateType&id="+id+"&action="+action, "type=single&typename="+value, function(data){
				if(data.state == 100){
					huoniao.showTip("success", data.info, "auto");
				}else if(data.state == 101){
	...
	//字段
	$(".root").delegate(".item", "click", function(){
		var t = $(this), id = t.parent().parent().find("input").attr("data-id"), title = t.parent().parent().find("input").val();
		try {
			parent.addPage("infoItem"+id, "info", title+"字段", "info/infoItem.php?tid="+id);
		} catch(e) {
			location.href = "infoItem.php?tid="+id;
		}
	});
	...
	$(".root").delegate(".del", "click", function(event){
		...
		if(type.indexOf("编辑") > -1){
			$.dialog.confirm(tip+"删除后无法恢复，请谨慎操作！！！", function(){
				...
				huoniao.operaJson("datingActivityType.php?dopost=del&action="+action, "id="+id, function(data){
					if(data.state == 100){
						huoniao.showTip("success", data.info, "auto");
						setTimeout(function() {
							location.reload();
		...
		$.dialog({
			...
			ok: function(){
				...
				if(ids.length <= 0){
					...
					$.dialog.confirm("确定后，此分类下的子级以及信息也将同时删除！<br />删除后无法恢复，请谨慎操作！！！", function(){

						huoniao.showTip("loading", "正在删除，，请稍候...");
						huoniao.operaJson("datingActivityType.php?dopost=del&action="+action, "id="+ids.join(","), function(data){
							if(data.state == 100){
...
function saveOpera(type){
	...
	huoniao.operaJson("datingActivityType.php?dopost=typeAjax", "data="+json, function(data){
		if(data.state == 100){
			huoniao.showTip("success", data.info, "auto");
			if(type == ""){
				//window.scroll(0, 0);
