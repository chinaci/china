$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingStory.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingStory.php?dopost=updateState", "id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
	...
	$("#stateBtn, #pageBtn, #paginationBtn").delegate("a", "click", function(){
		...
		if(obj.attr("id") == "paginationBtn"){
			...
			$("#typeBtn")
				.attr("data-id", "")
				.find("button").html('全部进程<span class="caret"></span>');
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingStoryAdd", "dating", "新增故事", "dating/"+href);
		} catch(e) {}
	});
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingStoryEdit"+id, "dating", title, "dating/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("datingStory.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, datingStory = val.datingStory;
		...
		if(val.state == "100"){
			...
			for(i; i < datingStory.length; i++){
				list.push('<tr data-id="'+datingStory[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row35 left"><img src="'+cfg_attachment+datingStory[i].litpic+'&type=small" width="100" class="litpic" /><a href="'+datingStory[i].url+'" target="_blank">'+datingStory[i].title+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+datingStory[i].fid+'" class="userinfo">'+datingStory[i].fidname+'</a></td>');
				list.push('  <td class="row10 left"><a href="javascript:;" data-id="'+datingStory[i].tid+'" class="userinfo">'+datingStory[i].tidname+'</a></td>');
				var process = "";
				switch (datingStory[i].process){
					case "0":
						process = "约会中";
						break;
					case "1":
				...
				list.push('  <td class="row7 left">'+process+'</td>');
				list.push('  <td class="row15 left">'+datingStory[i].pubdate+'</td>');
				var state = "";
				switch (datingStory[i].state) {
					case "0":
				...
				list.push('<a data-id="'+datingStory[i].id+'" data-title="'+datingStory[i].title+'" href="datingStoryAdd.php?dopost=edit&id='+datingStory[i].id+'" title="修改" class="edit">修改</a>');
