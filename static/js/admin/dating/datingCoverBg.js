$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingCoverBg.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingCoverBgAdd", "dating", "添加新背景", "dating/"+href);
		} catch(e) {}
	});
	...
	$("#typeManage").bind("click", function(event){
		...
		$.dialog({
			...
			ok: function(){
				...
				$.ajax({
					url: "datingCoverBg.php?dopost=updateType",
					data: "data=["+data.join(",")+"]",
					type: "POST",
					dataType: "json",
		...
		parent.$('.menu-itemlist').delegate("a", "click", function(){
			...
			if(id != ""){
				$.dialog.confirm("此操作将同时删除分类下的图片，确定要删除吗？", function(){
					...
					$.ajax({
						url: "datingCoverBg.php?dopost=delType",
						data: "id="+id,
						type: "POST",
						success: function(){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingCoverBgEdit"+id, "dating", title, "dating/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("datingCoverBg.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, datingCoverBg = val.datingCoverBg;
		...
		if(val.state == "100"){
			...
			for(i; i < datingCoverBg.length; i++){
				list.push('<tr data-id="'+datingCoverBg[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');

				var rec = "";
				if(datingCoverBg[i].rec == 1){
					rec = '&nbsp;&nbsp;<span class="label label-info" style="margin-right:5px;">推荐</span>';
				}

				list.push('  <td class="row60 left"><img src="'+cfg_attachment+datingCoverBg[i].litpic+'" width="100" />&nbsp;&nbsp;'+datingCoverBg[i].title+rec+'</td>');
				list.push('  <td class="row17 left">'+datingCoverBg[i].date+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+datingCoverBg[i].id+'" data-title="'+datingCoverBg[i].title+'" href="datingCoverBg.php?dopost=edit&id='+datingCoverBg[i].id+'" title="修改" class="edit">修改</a>');
