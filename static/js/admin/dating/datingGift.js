$(function(){
		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("datingGift.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
	...
	$("#addNew").bind("click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingGiftAdd", "dating", "添加新礼物", "dating/"+href);
		} catch(e) {}
	});
	...
	$("#typeManage").bind("click", function(event){
		...
		$.dialog({
			...
			ok: function(){
				...
				$.ajax({
					url: "datingGift.php?dopost=updateType",
					data: "data=["+data.join(",")+"]",
					type: "POST",
					dataType: "json",
		...
		parent.$('.menu-itemlist').delegate("a", "click", function(){
			...
			if(id != ""){
				$.dialog.confirm("此操作将同时删除分类下的图片，确定要删除吗？", function(){
					...
					$.ajax({
						url: "datingGift.php?dopost=delType",
						data: "id="+id,
						type: "POST",
						success: function(){
	...
	$("#list").delegate(".edit", "click", function(event){
		...
		try {
			event.preventDefault();
			parent.addPage("datingGiftEdit"+id, "dating", title, "dating/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	huoniao.operaJson("datingGift.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, datingGift = val.datingGift;
		...
		if(val.state == "100"){
			...
			for(i; i < datingGift.length; i++){
				list.push('<tr data-id="'+datingGift[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');

				var rec = "";
				if(datingGift[i].rec == 1){
					rec = '&nbsp;&nbsp;<span class="label label-info" style="margin-right:5px;">推荐</span>';
				}

				list.push('  <td class="row50 left"><img src="'+cfg_attachment+datingGift[i].litpic+'" width="100" />&nbsp;&nbsp;'+datingGift[i].title+rec+'</td>');
				list.push('  <td class="row10 left">'+datingGift[i].price+'</td>');
				list.push('  <td class="row17 left">'+datingGift[i].date+'</td>');
				list.push('  <td class="row20">');
				list.push('<a data-id="'+datingGift[i].id+'" data-title="'+datingGift[i].title+'" href="datingGift.php?dopost=edit&id='+datingGift[i].id+'" title="修改" class="edit">修改</a>');
