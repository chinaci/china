		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleTopic.php?dopost=del", "id="+id, function(data){
						if(data.state == 100){
...
//新增
	$("#addNew").bind("click", function(){
		...
		try {
			event.preventDefault();
			parent.addPage("circleAddphp", "circle", "新增话题", "circle/"+href);
		} catch(e) {}
	});
...
function getList(){
	...
	var data = [];
		...
		data.push("page="+page);
	huoniao.operaJson("circleTopic.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < circleList.length; i++){
				...
				var img = '';
				if(circleList[i].litpic!=''){
					img = '<img src="'+cfg_attachment+circleList[i].litpic+'&type=small" class="litpic" />';
				}
				list.push('  <td class="row30 left">'+img+'<span>'+circleList[i].title+'</span></td>');
				list.push('  <td class="row15">'+ circleList[i].username+'</td>');

				// list.push('  <td class="row25 center">'+img+'</small></td>');

				var type = circleList[i].usertype == 0 ? "(个人)" : "(中介)";
				var username = "";
				if(circleList[i].userid != 0){
					username = '<a href="javascript:;" data-id="'+circleList[i].userid+'" class="userinfo">'+circleList[i].username+'</a>';
				}else{
					username = circleList[i].username;
				}

				var rec = circleList[i].rec == '1'?'是':'否';
				list.push('  <td class="row15">'+circleList[i].browse+'</td>');
				list.push('  <td class="row8">'+rec+'</td>');
				var state = "";
				...
				list.push('  <td class="row15">'+circleList[i].pubdate+'</td>');
				list.push('  <td class="row14">');
				list.push('<a data-id="'+circleList[i].id+'" data-title="'+circleList[i].title+'" href="circleAdd.php?dopost=edit&id='+circleList[i].id+'" title="修改" class="edit">修改</a>');
