		...
		init = {
			...
			,quickEdit: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleCommon.php?dopost=getDetail", "id="+id, function(data){
						if(data != null && data.length > 0){
							...
							$.dialog({
								...
								ok: function(){
									...
									huoniao.operaJson("circleCommon.php?dopost=updateDetail", "id="+id+"&"+serialize, function(data){
										if(data.state == 100){
							...
							//填充信息
							self.parent.$("#store").html('<a href="javascript:;">'+data.storeTitle+'</a>');
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleCommon.php?dopost=delComment", "id="+id, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleCommon.php?dopost=updateState", "id="+id+"&arcrank="+arcrank, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("circleCommon.php?dopost=getList", data.join("&"), function(val){
		...
		if(val.state == "100"){
			...
			for(i; i < commonList.length; i++){
				...
				list.push('  <td class="row35 title left">标题：<a href="javascript:;">'+commonList[i].storeTitle+'</a><br />内容：'+commonList[i].content);
