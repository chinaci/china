		...
		init = {
			...
			,del: function(){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleReply.php?dopost=del", "tid="+tid+"&id="+id, function(data){
						if(data.state == 100){
			...
			,updateState: function(type){
				...
				if(checked.length < 1){
					...
					huoniao.operaJson("circleReply.php?dopost=updateState", "tid="+tid+"&id="+id+"&state="+state, function(data){
						if(data.state == 100){
							huoniao.showTip("success", data.info, "auto");
							setTimeout(function() {
								getList();
...
function getList(){
	...
	huoniao.operaJson("circleReply.php?dopost=getList", data.join("&"), function(val){
		var obj = $("#list"), list = [], i = 0, circleReply = val.circleReply;
		...
		if(val.state == "100"){
			...
			for(i; i < circleReply.length; i++){
				list.push('<tr data-id="'+circleReply[i].id+'">');
				list.push('  <td class="row3"><span class="check"></span></td>');
				list.push('  <td class="row50 left">动态：<a href="'+circleReply[i].url+'" target="_blank">'+circleReply[i].title+'</a><br />回复：'+circleReply[i].content+'</td>');
				list.push('  <td class="row10"><a href="javascript:;" class="userinfo" data-id="'+circleReply[i].uid+'">'+circleReply[i].username+'</a></td>');
				list.push('  <td class="row15">'+circleReply[i].pubdate+'<br />'+circleReply[i].ip+'</td>');
				var state = "";
				switch (circleReply[i].state) {
					case "0":
