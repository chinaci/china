...
_path = (typeof adminPath !== 'undefined' ? adminPath + "../static/" : staticPath) + 'js/ui/',
...
(function(style){
	if(!style)
	{
		...
		link.href = _path + 'skins/' + _skin + '.css?v=' + Math.random();
	    link.rel = 'stylesheet';
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	close: function()
	{
		...
		else
		{
			...
			DOM.wrap[0].style.cssText = 'left:-9999em;top:0;';
...
lhgdialog.setting =
{
	...
	title: 'title',     // 标题,默认'视窗'
	button: null,	     		// 自定义按钮
	ok: null,					// 确定按钮回调函数
	cancel: null,				// 取消按钮回调函数
	init: null,					// 对话框初始化后执行的函数
	close: null,				// 对话框关闭前执行的函数
	okVal: typeof langData != "undefined" && langData ? langData['siteConfig'][6][0] : '\u786e\u8ba4',		// 确定按钮文本,默认'确定'
	cancelVal: typeof langData != "undefined" && langData ? langData['siteConfig'][6][12] : '\u53d6\u6d88',	// 取消按钮文本,默认'取消'
	skin: '',					// 多皮肤共存预留接口
	esc: true,					// 是否支持Esc键关闭
...
_use = function(event)
{
	...
	limit =	(function(fixed)
	{
		var	ow = wrap.offsetWidth,
		    ...
		    // 坐标最大值限制(在可视区域内)
		    maxX = ww - ow + dl;
			//maxY = wh - oh + dt;   //向下拖动区域为标题
		    maxY = wh - wrap.offsetHeight + dt;
...
{
	return lhgdialog({
		title: typeof langData != "undefined" && langData ? langData['siteConfig'][19][287] : '\u8b66\u544a',
		id: 'Alert',
		zIndex: _zIndex(),
		icon: 'alert.png',
		fixed: true,
...
{
	return lhgdialog({
		title: typeof langData != "undefined" && langData ? langData['siteConfig'][6][0] : '\u786e\u8ba4',
		id: 'confirm.png',
		zIndex: _zIndex(),
		icon: 'confirm.png',
		fixed: true,
