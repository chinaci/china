...
(function(root, factory) {
	...
	function photoClip(container, option) {
		var clipWidth = option.width,
			...
			rotatedisable = option.rotatedisable;
		...
		if ($ok.length) {
			$ok.click(function() {
				if($ok.hasClass('disabled')) return;
				clipImg();
			});
		}
		...
		function initEvent() {
			...
			if (is_mobile) {
				...
				hammerManager.on("rotateend", function(e) {
					if (atRotation || rotatedisable) return;
		...
		function clipImg() {
			...
			var posx = curX - local.x / scale;
			var posy = curY - local.y / scale;
