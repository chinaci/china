$(function(){
  ...
  function getList(){
    ...
    showMsg.loading();
    $.ajax({
      url: masterDomain + '/include/ajax.php?service=dating&action=myGift&page='+page+'&pageSize=10&u=1',
      type: 'get',
      dataType: 'jsonp',
      success: function(data){
        if(data && data.state == 100){
          ...
          for(var i = 0; i < data.info.list.length; i++){
            ...
            html.push('    <a href="'+d.member.url+'"><img src="'+d.member.phototurl+'" alt=""></a>'+(d.member.my_video_state ? '<span class="user_video">' : '')+'</span>');
            html.push('  </div>');
