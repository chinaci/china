$(function(){
    ...
    $('.renoform_go').click(function(){
        var keyWord = $('.reno_form2 #keywords').val();
        if(keyWord){
            getList(1);
            return false;
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.state == 100) {
                    ...
                    $(".com_list ul").html('<li class="empty">'+data.info+'</li>');//已显示全部
                }
            },
            error: function(){
	...
	$(".apply").bind("click", function(){
		...
		if($('.qjimg_box .img').length == 0){
			if (r) {
				errmsg(qjimg_box, langData['renovation'][7][15]);//请上传照片
			}
			r = false;
		}
		...
		if(work_yearv == '') {
			if (r) {
				work_year.focus();
				errmsg(work_year, langData['renovation'][14][50]);//请输入工作经验
			}
			r = false;
		}

		//身份类别
        var command_type = $('input[name="command_type"]:checked').val();
        ...
        $.ajax({
            url: "/include/ajax.php?service=renovation&action=joinTeam",
            type: "get",
            data: data.join("&"),
            dataType: "jsonp",
            success: function (data) {
