$(function(){
  var city = '';
    var gzAddress         = $(".container"),  //选择地址页
        gzAddrList        = $("#p2"),    //选择收货地址页
        gzAddrHeaderBtn   = $(".gz-addr-header-btn"),  //删除按钮
        gzAddrListObj     = $(".address_box"),  //地址列表
        gzAddNewAddrBtn   = $(".addAddress"),  //新增地址按钮
        gzAddNewObj       = $("#p3"),   //新增地址页
        gzSelAddr         = $("#gzSelAddr"),     //选择地区页
        gzAddrSeladdr     = $(".gz-addr-seladdr.map_detail"),  //选择所在地区按钮
		...
		$(".mapresults").delegate("li", "click", function(){
			...
			console.log(lng)
			gzAddrSeladdr.find('input#lnglat').val(lng+','+lat)
			gzAddrSeladdr.find("input#posi").val(title);
      gzAddrMap.hide();
      // gzAddress.show();
		});
      					...
      					if (local.getStatus() == BMAP_STATUS_SUCCESS){
      						...
      						for (var i = 0; i < results.getCurrentNumPois(); i ++){
      							if(i == 0){
									...
									gzAddrSeladdr.attr('data-lng',lng).attr('data-lat',lat);
									gzAddrSeladdr.find("input#lnglat").val(lng+','+lat)
      								gzAddrSeladdr.find("input#posi").val(_value.business);
									gzAddrMap.hide();
									// gzAddress.show();
      							}
      						}
        ...
        var places = new google.maps.places.Autocomplete(input, {placeIdOnly: true});
            ...
            geocoder = new google.maps.Geocoder();
        		geocoder.geocode({'address': address}, function(results, status) {
                  ...
                  gzAddrSeladdr.attr('data-lng', lng).attr('data-lat', lat);
				 gzAddrSeladdr.find('input#lnglat').val(lng+','+lat)
                  gzAddrSeladdr.find("input#posi").val(results[0].formatted_address);
                  gzAddrMap.hide();
                  // gzAddress.show();
        ...
        map.plugin('AMap.Autocomplete', function () {
          console.log('Autocomplete loading...')
            ...
            AMap.event.addListener(autocomplete, 'select', function(result){
              ...
              gzAddrSeladdr.find("input#posi").val(r);
              gzAddrMap.hide();
              // gzAddress.show();
            });
        });
        ...
        function operation(){
          if(lng == '' || lat == ''){
            ...
            $("body").delegate("#qqmapSearch tbody tr", "click", function(e){
              ...
              gzAddrSeladdr.find("input#posi").val(t.text());
              gzAddrMap.hide();
              // gzAddress.show();
              $("#qqmapSearch").hide();
            }).on("click", function(e){
    ...
    gzAddNewAddrBtn.bind("click", function(){
        ...
        // $("#people").val("");
        // $("#mobile").val("");
        gzAddrSeladdr.removeClass("gz-no-sel").addClass("gz-no-sel").removeAttr("data-lng").removeAttr("data-lat").find("input#posi").attr("placeholder","选择您的所在区域");
        // $("#address").val("");
        ...
        HN_Location.init(function(data){
              if (data == undefined || data.address == "" || data.name == "" || data.lat == "" || data.lng == "") {
                ...
                lat = data.lat;
                city = data.city;
                gzAddrSeladdr.find("input#posi").val(name);
