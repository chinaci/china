...
shopInit = {
		...
		$.ajax({
			...
			success: function (data) {
				if(data && data.state == 100){
					...
					var info = data.info;
					utils.setStorage('shopCartList',JSON.stringify(info));
					thi.formList();
	...
	formList:function(){
		var info = utils.getStorage('shopCartList');
		cartul.html('');
		if(info){
			for(var i = 0; i < info.length; i++){
				var li = $('<li data-id="'+info[i].id+'" data-specation="'+info[i].specation+'" data-count="'+info[i].count+'" data-price="'+info[i].price+'" data-limit="'+info[i].limit+'">'+
							'<a href="'+info[i].url+'" target="_blank" class="pic" title="'+info[i].title+'"><img src="'+info[i].thumb+'" /></a>'+
							'<div class="info">'+
								'<h5><a href="'+info[i].url+'" target="_blank" title="'+info[i].title+'">'+info[i].title+'</a></h5>'+
								'<p title="'+info[i].speInfo+'">'+info[i].speInfo+'</p>'+
