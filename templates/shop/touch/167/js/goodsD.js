$(function() {
	...
	if(cartList){
		cartList.forEach(function(cart){
			if(cart.id == detailID && cart.count >= maxCount && maxCount>0){
				addFlag = 1;
				return false;
			}
		})
		...
		start: function(){
			...
			for(var i = 0; i < data.length; i++){
				...
				for(var ii = 0; ii < options.length; ii++){
					if(options[ii].pic){
						proDataArr.push('<li class="sku" attr_id="'+options[ii].id+'"><i><img src="'+options[ii].pic+'"></i>'+options[ii].name+'</li>');
					}else{
						proDataArr.push('<li class="sku" attr_id="'+options[ii].id+'"><i></i>'+options[ii].name+'</li>');
					}
	...
	$('.sku').each(function() {
			...
			}else if(self.parents("ul").prev("h3").text()=="颜色："){
	...
	$('body').delegate('.addCart,.gobuy,.add-cart,.buy-cart', 'click', function() {
		...
		if(addFlag && ($btn.hasClass('addCart') ||$btn.hasClass("add-cart")) && maxCount>0  ){
			alert('此商品限购'+maxCount+'件');
			return false;
		}
