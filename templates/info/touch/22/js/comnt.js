$(function(){
  ...
  $('.wcmt_send, .submit_top').click(function(){
      ...
      alert("请输入您要评论的内容！");
		...
		t.addClass("load").html('提交中...');
		$.ajax({
			url: "/include/ajax.php?service=member&action=sendComment&type=info-detail&aid="+newsid,
			data: "content="+content,
			type: "POST",
			dataType: "json",
			success: function (data) {
				t.removeClass("load").html('评论');
	...
	$(window).scroll(function(){
      ...
      var orderby = 'time';
      var url = masterDomain + "/include/ajax.php?service=member&action=getComment&type=info-detail&son=1&aid="+newsid+"&page="+page+"&orderby="+orderby+"&pageSize=" + pageSize;
      $.ajax({
          url: url,
          type: "GET",
          dataType: "jsonp",
