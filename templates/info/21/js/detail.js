$(function(){
	...
	function loadComment(){
		if(id && id != undefined){
			...
			//异步获取用户信息
			$.ajax({
				url: "/include/ajax.php?service=member&action=getComment&type=info-detail&son=1&aid="+id+"&page="+page+"&orderby="+orderby,
				type: "GET",
				dataType: "jsonp",
	...
	function getCommentList(list){
		...
		for(var i = 0; i < list.length; i++){
			...
			html.push(' <a target="_blank" href="'+ masterDomain +'/user/'+list[i].user['userid']+'#info"> <img onerror="javascript:this.src=\''+staticPath+'images/noPhoto_40.jpg\';" data-uid="'+list[i].user['userid']+'" src="'+photo+'" alt="'+list[i].user['nickname']+'"></a>');
			html.push('  <div class="c-body">');
			html.push('    <div class="c-header">');
			html.push('      <a target="_blank" href="'+ masterDomain +'/user/'+list[i].user['userid']+'#info" data-id="'+list[i].user['userid']+'">'+list[i].user['nickname']+'</a>');
	...
	$(".comment").delegate(".subtn", "click", function(){
		...
		if(cid == undefined){
            var url = '/include/ajax.php?service=member&action=sendComment&check=1&type=info-detail&aid=' + id;
        }else{
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data && data.state == 100){
					...
					list.push(' <a target="_blank" href="'+ masterDomain +'/user/'+info.userinfo['userid']+'#info"> <img data-uid="'+info.userinfo['userid']+'" src="'+huoniao.changeFileSize(info.userinfo['photo'], "small")+'" alt="'+info.userinfo['nickname']+'"></a>');
					list.push('  <div class="c-body">');
					list.push('    <div class="c-header">');
					list.push('      <a target="_blank" href="'+ masterDomain +'/user/'+info.userinfo['userid']+'#info" data-id="'+info.userinfo['userid']+'">'+info.userinfo['nickname']+'</a>');
