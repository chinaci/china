$(function(){
    ...
    $('.toptab li').click(function(event) {
        ...
        $('.mainIntro').eq(i).addClass('show').siblings().removeClass('show');
    });
    $(".slideBox2").slide({titCell:".hd ul", mainCell:".bd ul",effect:"leftLoop",autoPlay:true,autoPage:"<li></li>"});


    //模块集合
    $('.mainIntro .bInfoCon').find('.bIbox').hover(function(){
        $(this).find('.conbox').hide();
        $(this).find('.ifbcover').html($(this).find('.conbox').html().replace('.png', '1.png'));
        $(this).find('.ifbcover').css('display','block');
        $(this).find('.ifbcover.wechat .content').append('<p class="fontcode">点击扫描二维码</p>');
        $(this).find('.ifbcover.qq .content').append('<p class="fontcode">点击跳转QQ聊天</p>');
        $(this).find('.ifbcover.tel .content').append('<p class="fontcode"></p>');
        $('.wx_box .wechat').on('click',function(){
            $('html').addClass('nos');
            $('.modal-wx').addClass('curr');

        })
    },function(){
        $(this).find('.ifbcover').css('display','none');
        $(this).find('.conbox').show();
    });
    ...
    $('.st_tab .rbox').click(function(event) {
                ...
                if(data && data.state == 100){
                    ...
                    for(var i = 0; i < len; i++){
                        ...
                        html += '<li>' +
                            ...
                            //'<p class="info_price"><b>¥</b>'+list[i].price+'</p>' +
                        ...
                        //         '<h3><a href="'+list[i].url+'">'+list[i].title+'<i></i></a></h3>' +
                        //         '<div class="box_mark fn-clear">' +
                        ...
                        //         '</div>' +
                        //         '</div>' +
                        //         '<div class="r_tel"><i></i> <span>'+list[i].member.phone+'</span></div>' +
                        //         '</div>' +
                        //         '</div>' +
                        //         '</li>';
                        // }else{
