$(function() {
    ...
    if(souKey.length > 0){
        $('.ser-div').show();
        $('.textIn-box .sousuo_text').text(souKey);
        $('.textIn-box input').addClass('change');
    }
    $('.ser-div em').click(function() {
        $('.textIn-box input').removeClass('change');
        sou_input.style.width="85%";
        $(this).parent('.ser-div').remove();
    })
    ...
    $('.jz_baomu').delegate('li', 'click', function(){
        ...
        detailList.insertHtmlStr(dataInfo, $("#coursessearch").html(), {lastIndex: page});
    ...
    function getList(tr){
        ...
        if(keyword!=''){
            data.push("keywords="+keyword);
        }
        if($("#keywords").val()!=''){
            data.push("keywords="+$("#keywords").val());
        }
