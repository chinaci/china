$(function() {
    ...
    $('.jz_baomu').delegate('li', 'click', function(){
        ...
        detailList.insertHtmlStr(dataInfo, $("#tutor").html(), {lastIndex: page});
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain + "/include/ajax.php?service=education&action=tutorList&"+data.join("&"),
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<div class="left_b"><img src="'+huoniao.changeFileSize(list[i].photo, "small")+'" class="new_descrip_img"></div>');
                        html.push('<div class="right_b fn-clear">');
                        var sex = list[i].sex==1 ? 'tec_man' : 'tec_name';
                        var fullclass = list[i].fullclass==1 ? '<h3 class="full">'+langData['education'][7][28]+'</h3>' : list[i].fullclass==0 ? '<h3>'+langData['education'][7][27]+'</h3>' : '';
                        html.push('<div class="'+sex+'"><h1>'+list[i].username+'</h1><span></span>'+fullclass+'</div>');
                        html.push('<div class="recom_zu">');
                        html.push('<p>'+list[i].subjectsArr[0]+'-'+list[i].teachingagename+'<em>|</em>'+list[i].university+'-'+list[i].educationname+'</p>');
                        ...
                        html.push('<div class="tec_hour"><span>'+list[i].price+'</span><span>'+echoCurrency('short')+langData['education'][7][17]+'</span><span class="tec_add">'+(list[i].addrname!=undefined ? list[i].addrname[0]+list[i].addrname[1] : '')+'</span></div>');
                        html.push('</div>');
                        html.push('</a>');
                        html.push('</li>');
                    }
