$(function() {
    ...
    $('.jz_baomu').delegate('.cont_li', 'click', function(){
    ...
    $('#choose-area li').click(function(){
        ...
        if (index == 0) {
            ...
            //不限
            $('.choose-tab li').eq(localIndex).removeClass('active').attr("data-id", 0).find('span').text(langData['education'][3][17]);
            mask.hide();
            mask.hide();
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain + "/include/ajax.php?service=education&action=storeList&"+data.join("&"),
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<ul class="recom_zu">');
                        html.push('<li><span>'+langData['education'][7][22]+'</span><span>'+list[i].coursesnums+'</span></li>');
                        html.push('<li><span>'+langData['education'][7][23]+'</span><span>'+list[i].teachernums+'</span></li> ');
                        html.push('</ul>');
                        html.push('<ul class="tag">');
                        if(list[i].flag==1){
                            html.push('<li>'+langData['education'][7][24]+' </li>');
                        }
                        if(list[i].tag!=''){
                            for(var m=0;m<list[i].tagAll.length;m++){
                                if(m>1) break;
                                html.push('<li>'+list[i].tagAll[m].jc+'</li>');
                            }
                        }
                        if(list[i].taglist!=''){
                            for(var a=0;a< (list[i].tagAll.length -  list[i].taglist.length);a++){
                                if(a>1) break;
                                html.push('<li>'+list[i].taglist[m]+'</li>');
                            }
                        }
                        html.push('</ul>');
                        html.push('<p class="bm_salary"><span>'+list[i].tel+'</span><span class="recom_sistance">'+list[i].addrname[list[i].addrname.length-1]+'</span></p>');
                        html.push('</div>');
                        html.push('</a>');
                        html.push('</li>');
                    }
                    ...
                    if(page == 1){
                         $(".jz_baomu .cont_ul").html("");

                         $(".jz_baomu .cont_ul").html(html.join(""));
                    }else{

                        $(".jz_baomu .cont_ul").append(html.join(""));
                    }
                    isend = false;
                    ...
                    $(".jz_baomu .cont_ul").html('<div class="empty">'+data.info+'</div>');
                }
            },
            error: function(){
                isload = false;
