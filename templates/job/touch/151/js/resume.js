					...
					if(data.state == 100){
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								//薪资
								var salary= list[i].salary > 0 ? list[i].salary:'面议';
								html.push(is_buy_htm);
								html.push('<p class="fn-right list-tit-tag">');
								html.push('<span class="area">'+list[i].addr[list[i].addr.length-1]+'</span>');
								...
								html.push('<div class="img-box fn-left"><img src="'+(list[i].photo ? list[i].photo : '/static/images/noPhoto_60.jpg')+'"></div>');
								html.push('<div class="img-txt fn-clear">');
                                html.push('</a>');
								html.push('<div class="zhiwei_main fn-clear"><h3>期望职位<span class="post">'+list[i].type+'</span></h3>');
								html.push(is_jiesuo);
                                html.push('</div>');
								html.push('<p class="now fn-clear">期望薪资<span>'+salary+'</span><span class="time">'+list[i].pubdate1+'</span></p>');
								html.push('</div>');
								html.push('</div>');
