$(function(){
    ...
    if($.browser.msie && parseInt($.browser.version) >= 8){
        $('.boxlist li:nth-child(2n)').css('margin-right','0');
    }
		...
		if(t.hasClass("no")){
			...
			if(t.hasClass("curr")){
				...
				if(t.parent().find(".area"+id+" li").length){
					...
					location.href = '?'+(location.href.indexOf('company') > 0 ? 'addrid' : 'addr')+'='+addr+'#J_crumbs';
				}
			}
			return false;
		}
	...
	$('body').delegate('.unlock', 'click', function() {
		...
		var _name = par.attr('data-name');
		var _type = par.attr('data-type');
		$('.resInfo dt').html(_name);
		$('.resInfo dd').html(_type);
	...
	//关闭
	$('.popup-buy').delegate('.tit s', 'click', function(){
	...
	$('body').delegate('#btnBuy', 'click', function() {
 	...
 	function showMsg(msg){
	    $('.popup-buy').append('<p class="ptip">'+msg+'</p>')
	    setTimeout(function(){
	    $('.ptip').remove();
	    },2000);
  	}
