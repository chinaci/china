$(function(){
    ...
    $.ajax({
        type: "GET",
        url: "/include/ajax.php?service=article&action=alist&pageSize=4&thumb=1&isAjax=1&flag=r&typeid=5",
        data: '',
        dataType: "json",
        success: function(data){
            if(data.state == 100){
                var info2 = data.info.list;
                var obj2 = $(".video .hdc ul");
                var html2 = '';
                for (i = 0; i < info2.length; i++){
                    html2 += "<li><a href="+info2[i].url+" target=\"_blank\"><img src="+info2[i].litpic+" alt="+info2[i].title+" /><i></i><span>"+info2[i].title+"</span><p>"+info2[i].click+"</p></a></li>";
                }
                obj2.html(html2);
            }
        }
    ...
    $.ajax({
        type: "GET",
        url: "/include/ajax.php?service=article&action=alist&pageSize=8&thumb=1&isAjax=1&page=1&typeid=8",
        data: '',
        dataType: "json",
        success: function(data){
            if(data.state == 100){
                var info2 = data.info.list;
                var obj2 = $(".piclist .hdc ul");
                var html2 = '';
                for (i = 0; i < info2.length; i++){
                    html2 += "<li><a href="+info2[i].url+" target=\"_blank\"><img src="+info2[i].litpic+" alt="+info2[i].title+" />"+info2[i].title+"</a></li>";
                }
                obj2.html(html2);
            }
        }
