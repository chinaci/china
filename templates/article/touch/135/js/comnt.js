$(function(){
    ...
    $('.wcmt_send, .submit_top').click(function(){
		...
		$.ajax({
			url : "/include/ajax.php?service=member&action=sendComment&type=article-detail"+'&aid='+newsid+"&content="+content,
			type: "POST",
			dataType: "json",
			success: function (data) {
    ...
    function getComment(tr){
        ...
        $.ajax({
          url : '/include/ajax.php?service=member&action=getComment&type=article-detail&son=1&aid='+newsid+'&page='+page+'&pageSize='+pageSize,
          type: 'get',
          dataType: 'json',
          success: function(data){
              if(data && data.state == 100){
