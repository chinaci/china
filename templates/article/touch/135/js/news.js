    ...
    $('#nav_list .item').each(function(i){
        ...
        if(!type){
            ...
            if(i == 0) {
                ...
                $('body').append('<div class="swrip-panel" id="chanel_'+id+'"></div>');
                panel = $("#chanel_" + id);
            }
                panel.width(bodyWidth + 'px');
                panel.height(winH + 'px');
                panel.css('minHeight', winH + 'px');
            var panelBox = $('#maincontent');
            i && panelBox.append(panel);
        } else {
            $('.swrip-panel').width(bodyWidth + 'px;');
        }
        len++;
    })
    panelBox.width(bodyWidth * len + 'px');
}
...
function getData(id, page, leval){
    ...
    if(id == 0) {
        url = masterDomain + "/include/ajax.php?service=article&action=alist&group_img=1&flag=r&page=" + page + "&pageSize=10";
    } else {
        url = masterDomain + "/include/ajax.php?service=article&action=alist&typeid="+id+"&group_img=1&page=" + page + "&pageSize=10";
    }
    $.ajax({
        url: url,
        type: "GET",
            ...
            if(data && data.state != 200){
                if(data.state == 101){
                    ...
                    for(var i = 0; i < list.length; i++){
                        ...
                        if(list[i].group_img){
                            item.push('<div class="m_item fn-clear alonenews">');
                                item.push('<a href="javascript:;" data-url="' + url + '">');
                                    ...
                                    item.push('<div class="pic clearfix">');
                                        ...
                                        for (var g = 0; g < list[i].group_img.length; g++) {
                                            ...
                                            if(src && n < 3) {
                                                item.push('<img src="'+src+'" onerror=this.src="'+litpic+'" data-url="' + src + '" alt="title">');
                                                n++;
                                                if(n == 3) break;
                                            }
                                        }
                            ...
                            item.push('<div class="m_item fn-clear">');
                                item.push('<a href="javascript:;" class="clearfix" data-url="' + url + '">');
                                    if(img !== undefined && img != ''){
                                        item.push('<div class="m_item-img">');
                                            if(flag.indexOf('r') > -1){
                                                // item.push('<span class="tag">推荐</span>');
                                            }
                                            item.push('<img src="'+litpic+'" onerror=this.src="'+litpic+'" data-url="' + litpic + '" alt="' + title + '">');
                                        item.push('</div>');
                                    }
                    ...
                    // detailList.insertHtmlStr(dataInfo, $("#maincontent").html(), {lastIndex: (page + 1)});
...
if (device.indexOf('huoniao_iOS') > -1) {
  $('.navfixbox, .nav').addClass('padTop20');
}
...
$(function(){
  ...
  $('#maincontent').delegate('.m_item', 'click', function(){
    ...
    $('#picscroll').on('touchstart',function(e){
        scrollLock = true;
    })
    ...
    // 上滑下滑导航隐藏
    var upflag = 1, downflag = 1, fixFooter = $(".navfixbox");
    //scroll滑动,上滑和下滑只执行一次！
