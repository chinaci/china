			...
			if(data && data.state == 100){
				...
				if(len){
					for(var i = 0; i < len; i++){
						...
						html.push('	  	<p class="price">'+obj.price+''+echoCurrency('short')+' + '+obj.point+pointName+'</p>');
						html.push('	  	<p class="des">使用'+pointName+'抵'+obj.point/pointRatio+''+echoCurrency('short')+'</p>');
						html.push('	</div>');
