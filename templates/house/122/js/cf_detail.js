$(function() {
  ...
  $(".btnJb").bind("click", function(){
    ...
    $.dialog({
      fixed: false,
      title: "房源举报",
      content: 'url:'+domainUrl+'/complain-house-cf-'+pageData_.id+'.html',
      width: 460,
      height: 300
    });
  });
  ...
  function collect(type, check, callback){
    $.ajax({
      url: "/include/ajax.php?service=member&action=collect&module=house&temp=cf_detail&type="+type+"&check="+check+"&id="+pageData_.id,
      type: 'get',
      dataType: 'json',
      success: function(data){
  ...
  if (house_cf_history.indexOf(pageData_.id) == -1) {
    if (house_cf_history.length > 0) {
      house_cf_history += ':'+pageData_.id;
    } else {
      house_cf_history += pageData_.id;
    }
