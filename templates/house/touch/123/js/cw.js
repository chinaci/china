$(function() {
  ...
  setTimeout(function(){detailList.removeLocalStorage();}, 800);
	...
	$('#scroll-sptype').delegate('.btn_submit', 'click', function() {
        ...
        $('.tab-type span').html(area_1+'-'+area_2 + echoCurrency('areaname'));
        if (!area_1 && !area_2) {
		 			...
		 			if(data.state == 100){
		 				...
		 				if(list.length > 0){
							for(var i = 0; i < list.length; i++){
	                            ...
	                            if(list[i].type==1){
									if(list[i].price>0){
										elevatortxt = '<em class="r">' + (list[i].price / list[i].area).toFixed(0) + '万/'+echoCurrency('areasymbol')+'</em>';
									 }
	                           	}else if(list[i].type==2){
	                           		...
	                           		if(list[i].price>0){
	                           			elevatortxt = '<em class="r">' + (list[i].price / list[i].area).toFixed(0) + ''+echoCurrency('short')+'/'+echoCurrency('areasymbol')+'•月</em>';
	                           		}
	                           	}

								html.push('<dd class="item-area xzl-item-area"><em>'+list[i].area+echoCurrency('areasymbol')+'|'+list[i].protype+'</em>');
