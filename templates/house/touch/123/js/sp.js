$(function() {
  ...
  setTimeout(function(){detailList.removeLocalStorage();}, 800);
	...
	$('.choose-more li[data-id="'+detail.protype+'"]').addClass('active').siblings('li').removeClass('active')
	...
	$('#scroll-area').delegate('.btn_submit', 'click', function() {
        ...
        $('.tab-area span').html(area_1+'-'+area_2 + echoCurrency('areasymbol'));
        if (!area_1 && !area_2) {
	...
	function getList(tr){
		...
		data.push("protype="+detail.protype);
						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								...
								if (list[i].sellstate==1) {
									...
									var cla = 'over_house';
								}else if(list[i].sellstate==3){
									var saleIcon = '<i class="transfer_icon"></i>';
									var cla = 'over_house';
								}else{
									var saleIcon = '';
									var cla = '';
								...
								html.push('<em>'+list[i].area+echoCurrency('areasymbol')+'|'+list[i].loupan+'|'+list[i].zhuangxiu+'</em>');
	                            ...
	                            if(list[i].type==1){
									if(list[i].price>0){
										elevatortxt = (list[i].price / list[i].area).toFixed(1) + '万/'+echoCurrency('areasymbol');
									 }
	                           	}else if(list[i].type==2){
	                           		if(list[i].transfer>0){
	                           			elevatortxt = '转让费： ' + parseInt(list[i].transfer).toFixed(1) + '万';
	                           		}
