						...
						if(list.length > 0){
							for(var i = 0; i < list.length; i++){
								 ...
								 if (list[i].sellstate==1) {
								 	var cla = 'over_house';
									var zuIcon = '<i class="zu_icon"></i>';
								}else{
									var cla = '';
									var zuIcon = '';
								}
									html.push('<div class="house-box '+cla+'">');
									html.push(zuIcon);//房源已售
									...
									html.push('<em>'+list[i].room+'</em><em>'+list[i].area+echoCurrency('areasymbol')+'</em>');
									...
									html.push('<em>'+(list[i].distances == '0.0m' || list[i].distances == '0.0km' ? '' : list[i].distances)+'</em><em>'+list[i].community+'</em>');
									html.push('</dd>')
