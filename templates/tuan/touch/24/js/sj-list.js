$(function(){
  ...
  setTimeout(function(){detailList.removeLocalStorage();}, 500);
	...
	$('.shop-box').delegate('.shop-list', 'click', function(){
					...
					if(data.state == 100){
						...
						if(list.length > 0){
							...
							for(var i = 0; i < list.length; i++){
								html.push('<div class="shop-list" data-id="'+list[i].id+'">');
								html.push('<a href="javascript:;" data-url="'+list[i].url+'" data-id="">');
								html.push('<dl>');
								...
								html.push('<span class="price"><small>'+echoCurrency('symbol')+'</small>'+list[i].price+'</span>');
