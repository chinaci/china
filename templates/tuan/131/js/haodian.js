$(function(){
    ...
    function getTuanData(type){
        ...
        $.ajax({
            url: "/include/ajax.php?service=tuan&action=storeList&pageSize="+pageSize+"&typeid="+typeid+"&addrid="+addrid+"&business="+business+"&subway="+subway+"&station="+station+"&item="+item.join("$$")+"&orderby="+orderby+"&flag="+flag.join(",")+"&title="+keywords+"&page="+atpage,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                ...
                if(data && data.state == 100 && data.info.list.length > 0){
                    ...
                    for(var i = 0; i < list.length; i++){
                        ...
                        html.push('<h4>'+list[i].company+'</h4>');
                        html.push('<p class="tuan"><span>发布团购 <em>'+list[i].tuannum+'</em></span> <span>综合评分 <em>'+list[i].rating+'</em></span></p>');
                        html.push('<p class="btn">进入店铺</p>');
                        html.push('</div>');
                        html.push('</a>');
                        html.push('</li>');
                    }
