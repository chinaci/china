$(function(){
	...
	setInterval(getCountDown,1000)
	 function getCountDown(){
		  $(".ptnow .slideGroup li").each(function(){
		  	var t = $(this);
			if(nowDate==-1){
				nowDate = t.find('.time').attr('data-now');
			}
		  	endtime = t.find('.time').attr('data-time');
			 ...
			 t.find('.time').text((hours>9?hours:"0"+hours)+":"+(minutes>9?minutes:"0"+minutes)+":"+(seconds>9?seconds:"0"+seconds))
			 nowDate ++;
		  });
