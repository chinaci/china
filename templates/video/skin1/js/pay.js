$(function(){
	...
	var anotherPay = {
		...
		,resetTotalMoney: function(){
			...
			if(realMoney == 0){
				...
				$('.all_paypwd').append('<span class="all_left">已使用账户资产全额抵扣</span>')//已使用账户资产全额抵扣
			}else{
	...
	$("#usePinput, #useBalance").bind("click", function(){
		...
		if(ischeck == "checked"){
			...
			if($('.no_paypwd').size() > 0){
				$('.code_div .code_tip').text('点击下方按钮，扫码支付余款');//点击下方按钮，扫码支付余款
			}
	...
	$('.code_div .pay_fresh').bind("click", function(){
		//验证密码输入
			...
			if(paypwd == ""){
				...
				$.ajax({
					url: "/include/ajax.php?service=video&action=sweepcode&final=1&qr=1",
					type: "POST",
					data:data,
					dataType: "json",
	...
	if(myBrowser() == "IE" ) {
		...
		$("#useBcount").bind("blur", function(){
			...
			if(!check){
				alert('请输入正确的数值，最多支持两位小数！');//请输入正确的数值，最多支持两位小数！
				$("#useBcount").val("0");
				$("#balMoney").html("0");
	...
	function getQrCode(){
	   // $('.payTab li:eq(0)').hasClass('curr') ? $('.stepBtn').hide() : null;
	    ...
	    $.ajax({
	      type: 'POST',
	      url: '/include/ajax.php?service=video&action=sweepcode',
	      data: data  + '&qr=1',
	      dataType: 'jsonp',
	...
	$(".pay_p").bind("click", function(event){
		...
		if($("#ordernum").val() == ""){
			alert('订单号获取失败，请刷新页面重试！');//订单号获取失败，请刷新页面重试！
			return false;
		}
		if($("#paytype").val() == ""){
		...
		if((pinputCheck == "checked" && point > 0) || (balanceCheck == "checked" && balance > 0)){
			...
			$.ajax({
				url: "/include/ajax.php?service=video&action=checkPayAmount",
				data: data.join("&"),
				type: "POST",
				dataType: "json",
				success: function (data) {
