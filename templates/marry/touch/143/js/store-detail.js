$(function () {
    ...
    // 全部菜单
    var menuslideHeight = 100;
    var menudefHeight = $('.menubox').height();
    if(menudefHeight >= menuslideHeight){
        $('.menubox').css('height' , menuslideHeight + 'px');
        $('.menuMore').append('<a href="javascript:;">'+langData['marry'][0][34]+'<i class="rmDown"></i></a>');//全部菜单
        $('.menuMore a').click(function(){
            var curHeight = $('.menubox').height();
            if(curHeight == menuslideHeight){
                $('.menubox').animate({height: menudefHeight}, "normal");
                $('.menuMore a').html(''+langData['marry'][0][33]+'<i class="rmUp"></i>');//全部收起
                $('.gradient').fadeOut();
            }else{
                $('.menubox').animate({height: menuslideHeight}, "normal");
                $('.menuMore a').html(''+langData['marry'][0][34]+'<i class="rmDown"></i>');//全部菜单
                $('.gradient').fadeIn();
            }
            return false;
        });
