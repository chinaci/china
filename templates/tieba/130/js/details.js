...
$(function(){
	...
	$(".emoji-hide").html(huoniao.appendEmoji());
...
//		如果没有回复，查看回复则显示回复框
	...
	var upLike = function(func){
	    $.post("/include/ajax.php?service=member&action=getZan&module=tieba&temp=detail&id="+id+"&uid="+louzuid, function(e){
		  func();
	    });
    }
...
function getReply(replyMore, rid, page){
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=member&action=getChildComment&sid="+rid+"&page="+page+"&pageSize="+replyPageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
			if(data && data.state == 100){
				...
				for(var i = 0; i < list.length; i++){
					...
					html.push('<li><div class="head_img"><img onerror="javascript:this.src=\''+staticPath+'images/noPhoto_100.jpg\';" src="'+src+'"  /></div><div class="reply-text"><div class="reply-info"><p class="reply-user"><span>'+list[i].user.nickname+'</span>：</p><p class="reply_time"><span>'+huoniao.transTimes(list[i].dtime, 1)+'</span> <span class="reply_btn">回复</span></p></div><div class="reply-p">'+list[i].content+'</div></div></li>')
...
//		alert('111');
		...
		var t = $(this), par = t.closest(".floor-members"), rid = par.data("id");
		$(".mytextarea img").each(function(){
			var img = $(this),txt= img.attr('data-txt');
				img.after('<em>'+txt+'</em>');
				img.remove()
		})
		var content = par.find(".mytextarea").html();
			...
			$.ajax({
				...
				success: function (data) {
					if(data && data.state == 100){
						...
						var html = '<li><div class="head_img"><img onerror="javascript:this.src=\''+staticPath+'images/noPhoto_100.jpg\';" src="'+info.userinfo.photo+'" /></div><div class="reply-text"><div class="reply-info"><p class="reply-user"><span>'+info.userinfo.nickname+'</span></p><p class="reply_time"><span>'+huoniao.transTimes(info.dtime, 1)+'</span> <span class="reply_btn">回复</span></p></div><div class="reply-p">'+info.content+'</div></div></li>';
