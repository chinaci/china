$(function(){
  ...
  if (userAgent.match(/MicroMessenger/i) == "micromessenger") {
    ...
    wx.ready(function() {
      ...
      $('#filePicker1').unbind().unbind().bind('click', function(){
  ...
  function createUploader(id) {
    ...
    //删除已上传视频
  	...
  	uploader.on('uploadSuccess', function(file, response){
        ...
        if (id == "2") {
          var $li   = $('#'+file.id);
          $li.html('<video class="video-js" data-setup="{}" controls src="/include/attachment.php?f='+response.url+'" data-val="'+response.url+'" webkit-playsinline="true" preload="auto" playsinline x5-video-player-type="h5" x5-video-player-fullscreen="true" x5-video-ignore-metadata="true" poster="'+response.poster+'"><source src="/include/attachment.php?f='+response.url+'"></source></video></div>');
...
if(site_map == 'baidu'){
    ...
    function getlocalList(){
       var myGeo = new BMap.Geocoder();
          myGeo.getLocation(new BMap.Point(lng, lat), function mCallback(rs){
        ...
        function callback(results, status) {
            if (status === 'complete' && results.info === 'OK') {
                ...
                for(var i = 0; i < allPois.length; i++){
                    list.push("<li>");
                    list.push("<p class='tit'>"+ allPois[i].name + "</p>");
                    list.push("<p>"+ allPois[i].address + "122</p>");
                    list.push("</li>");
                }
                if(list.length > 0){
                    $('.area-list').html(list.join(''));
                }
            }else{
                $(".area-list").html('');
            }
        ...
        map.plugin('AMap.Autocomplete', function () {
            ...
            AMap.event.addListener(autocomplete, 'select', function(result){
                ...
                $("#keywords").html(r);
                $('.anotherbox .area').text(r);
                $('.area-list .active').removeClass('active');
                $('.layer-area').css('left','200%');
            });
        });
     ...
     var s = function(){
          if(lng != '' && lat != ''){
             mapOptions = {
              ...
              getLocation(mapOptions.center,map)
          }else{
              setTimeout(s,1000)
          }
      }
      s();
      function getLocation(pos,map){
        ...
        function callback(results, status) {
          if (status === google.maps.places.PlacesServiceStatus.OK) {
            for (var i = 0; i < results.length; i++) {
                  list.push("<li>");
                  list.push("<p class='tit'>"+ results[i].name + "</p>");
                  list.push("<p>"+ results[i].vicinity + "</p>");
                  list.push("</li>");
            }
             if(list.length > 0){
                $('.area-list').html(list.join(''));
        ...
        google.maps.event.addListener(places, 'place_changed', function () {
            ...
            geocoder.geocode({'address': address}, function(results, status) {
              if (status == google.maps.GeocoderStatus.OK) {
                ...
                if (lng && lat) {
                  $("#keywords").html(results[0].formatted_address);
                  $('.anotherbox .area').text(results[0].formatted_address);
                  $('.area-list .active').removeClass('active');
                  $('.layer-area').css('left','200%');
