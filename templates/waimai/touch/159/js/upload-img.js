...
//			$('#uploadbtn').append('<li>1111<li>')
		...
		uploader.on('error', function(code){
			...
			switch(code){
				...
				case "F_EXCEED_SIZE":
					...
					break;
				case "F_DUPLICATE":
					txt = langData['siteConfig'][20][305] ;   /* "此图片已上传过"; */
					break;
			}
			showErr(txt);
	...
	function updateStatus(){
		...
		if(atlasMax<fileCount){
			showErr(langData['waimai'][7][146].replace('1',9))    /* '图片最多上传1张'*/
		}else if(atlasMax>fileCount){
			$('.upload_btn').show();
		}else{
			$('.upload_btn').hide();
		}
	...
	function addFile(file) {
		...
		uploader.makeThumb(file, function(error, src) {
				if(error){
					$img.replaceWith('<span class="thumb-error">'+langData['siteConfig'][20][305]+'</span>'); /*  不能预览  */
					return;
				}
