$(function(){
  ...
  function mysub(obj, id){
    ...
    $.ajaxFileUpload({
      ...
      error: function() {
        uploadError(langData['siteConfig'][20][183], id);//网络错误，请重试！
      }
    });
  ...
  $(".Filedata").bind("change", function(){
	...
	var commonChange = function(t){
		...
		surp = surp <= 0 ? 0 : surp;
		var txt = langData['siteConfig'][23][63].replace("1", "<strong>" + surp + "</strong>");//还可输入1个字。
		tip.html(txt);
  ...
  $("#commonBtn").bind("click", function(){
    ...
    $(".comment").each(function(){
      ...
      if(score1 == ""){
        alert(langData['shop'][4][30]); //请给商品描述打分
        tj = false;
        return false;
      }

      if(score1 == ""){
        alert(langData['shop'][4][31]);//请给商家服务打分
        tj = false;
        return false;
      }

      if(score1 == ""){
        alert(langData['shop'][4][32]);//请给商品质量打分
        tj = false;
        return false;
      }

      if(note == ""){
        alert(langData['shop'][4][33]);//请输入评价内容！
        tj = false;
        return false;
      }
      ...
      t.attr("disabled", true).html(langData['siteConfig'][6][35]+"...");//提交中
  		...
  		$.ajax({
            ...
            location.reload();
  			...
  			error: function(){
  				alert(langData['siteConfig'][20][183]);//网络错误，请重试！
  				t.attr("disabled", false).html(langData['siteConfig'][8][3]);//发表评价
  			}
  		});
