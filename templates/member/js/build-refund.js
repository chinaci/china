$(function(){
	...
	var commonChange = function(t){
		...
		surp = surp <= 0 ? 0 : surp;
		var txt = langData['siteConfig'][19][320]+" <strong>" + surp + "</strong> "+langData['siteConfig'][19][321];  //还可输入--个字
		tip.html(txt);
	...
	uploader = WebUploader.create({
		...
		server: '/include/upload.inc.php?mod=build&type=atlas',
		pick: '#filePicker',
		fileVal: 'Filedata',
		accept: {
			title: 'Images',
	...
	$list.delegate(".left", "click", function(){
		...
		huoniao.rotateAtlasPic("build", "left", val, function(data){
	...
	$list.delegate(".right", "click", function(){
		var t = $(this), li = t.closest("li"), img = li.find("img"), val = img.attr("data-val"), url = img.attr("data-url");
		huoniao.rotateAtlasPic("build", "right", val, function(data){
			if(data.state == "SUCCESS"){
				url = huoniao.changeFileSize(url, "small");
				img.attr("src", hideFileUrl == 1 ? url+"&v="+Math.random() : url+"?v="+Math.random());
			}else{
	...
	$("#refundBtn").bind("click", function(){
		...
		$.ajax({
			url: masterDomain+"/include/ajax.php?service=build&action=refund",
			data: data,
			type: "POST",
			dataType: "jsonp",
