				...
				if(data.state != 100){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<p>'+langData['siteConfig'][19][393]+'：'+typename+'&nbsp;&nbsp;·&nbsp;&nbsp;'+pubdatetime+arcrank+'</p>');//分类
							html.push('<h5 class="liTitle"><a href="'+url+'" target="_blank" >'+startaddr+'<s></s>'+endaddr+'</a></h5>');
							html.push(langData['siteConfig'][19][394]+'：'+onclick);
							//浏览--次
							// if(isvalid){
							...
							if(refreshSmart || isbid == 1){
								...
								if(isbid && bid_type == 'plan'){
								    ...
								    //记录置顶详情
                                    topPlanData['sfcar'] = Array.isArray(topPlanData['sfcar']) ? topPlanData['sfcar'] : [];
                                    topPlanData['sfcar'][id] = bid_plan;

                                    html.push('<p class="topPlanDetail" data-module="sfcar" data-id="'+id+'" title="'+langData['siteConfig'][6][113]+'">'+langData['siteConfig'][31][131]+'<s></s></p>');
                                    //查看详情---已开通计划置顶
								}
								html.push('</div>');
							}
