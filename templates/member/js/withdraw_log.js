					...
					if(list.length > 0){
						for(var i = 0; i < list.length; i++){
							...
							if(list[i].tab == 'w'){
                                ...
                                var account = (bank == 'weixin' ? langData['siteConfig'][27][139] : (bank == 'alipay' ? langData['siteConfig'][19][302] : langData['siteConfig'][19][301])) + (bank != 'weixin' ? " | "+cardnum+" | "+cardname : '');//微信 -- 支付宝 --银行卡
								...
								var stateTxt = "";
								if(type == 'bank'){
									account = (bank == 'weixin' ? langData['siteConfig'][27][139] : (bank == 'alipay' ? langData['siteConfig'][19][302] : langData['siteConfig'][19][301])) + (bank != 'weixin' ? " | "+order_id.substr(order_id.length-4)+" | "+"*"+cardname.substr(1) : '');//微信 -- 支付宝 --银行卡
