...
$(function(){
	...
	function checkStore(title){
		$.ajax({
			url: '/include/ajax.php?service=car&action=storeList&keywords='+title,
			type: 'get',
			dataType: 'jsonp',
			success: function(data){
				if(data.state == 100){
					...
					for(var i = 0; i < data.info.list.length; i++){
						...
						html.push('<li data-id="'+d.id+'" data-title="'+d.title+'" data-logo="'+d.logo+'" data-tel="'+d.tel+'" data-address="'+d.address+'" data-url="'+d.url+'">'+d.title+'</li>');
					}
            ...
            success: function(data){
                if(data && data.state == 100){
                   ...
                   if(listLen == 1 && codeArea == 86){//当数据只有一条 并且这条数据是大陆地区86的时候 隐藏区号选择
                        $('.areaCode').hide();
                        $('.enter_zjcom #fabuForm .inp#tel').css({'padding-left':'10px','width':'295px'});
                        $('.enter_zjcom .inp#phone').css({'padding-left':'10px','width':'295px'});
                        return false;
                   }
...
function getList(title){
	...
	$.ajax({
		url: '/include/ajax.php?service=car&action=storeList',
		type: 'get',
		data: data.join("&"),
		dataType: 'jsonp',
		success: function(data){
			if(data.state == 100){
				...
				for(var i = 0; i < data.info.list.length; i++){
					...
					html.push('	<a href="'+d.url+'" target="_blank" class="logo"><img src="'+d.logo+'" alt="'+d.id+'"></a>');
					html.push('	<div class="txt">');
					html.push('		<a href="'+d.url+'" target="_blank" class="name">'+d.title+'</a>');
					html.push('		<p>'+d.city+'<span>'+d.address+'</span></p>');
					html.push('	</div>');
					html.push('	<a href="javascript:;" class="join" data-id="'+d.id+'" data-title="'+d.title+'" data-logo="'+d.logo+'" data-tel="'+d.tel+'" data-address="'+d.address+'" data-url="'+d.url+'">'+langData['siteConfig'][30][48]+'</a>');
					//申请加入
					html.push('</li>');
				}
