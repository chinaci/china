$(function(){
	...
	function mysub(){
		...
		$("#choose").val(langData['siteConfig'][6][178]);   //重新选择
	...
	function uploadError(info){
		$("#choose").val(langData['siteConfig'][6][49]);  //选择图片
	...
	$("#save").bind("click", function(){
		...
		if(coordinate && picid != 0){
			...
			$("#save").attr("disabled", true).val(langData['siteConfig'][7][9]+"...");  //保存中
			$(".upload-form").hide();
			...
			$.ajax({
				url: "/include/cropupload.php?mod=dating",
				data: coordinate,
				dataType: "jsonp",
				...
				error: function(){
					saveFailed(langData['siteConfig'][20][183]);   //网络错误，请稍候重试！
				}
			});
		}
	...
	function saveFailed(info){
		...
		$(".btns").addClass("normal");
		$("#choose").val(langData['siteConfig'][6][49]);   //选择图片
		$("#save").attr("disabled", false).val(langData['siteConfig'][6][48]);   //保存头像
		$(".upload-form, .tips").show();
	}
