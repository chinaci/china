			...
			if(data && data.state != 200){
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						objId.html('<table><thead class="thead"><tr><th class="fir"></th><th>'+langData['siteConfig'][19][289]+'</th><th>'+langData['siteConfig'][19][290]+'</th><th>'+langData['siteConfig'][19][291]+'</th></tr></thead>'+html.join("")+'</table>');
					//登录时间 - -IP地址--参考地点
					}else{
						objId.html("<p class='loading'>"+langData['siteConfig'][20][126]+"</p>");  //暂无相关信息！
					}
					...
					$("#totalTip").html("&nbsp;&nbsp;("+langData['siteConfig'][20][1].replace('1', pageInfo.totalCount)+")");  //1位数字
					showPageInfo();
				}
			}else{
				objId.html("<p class='loading'>"+langData['siteConfig'][20][126]+"</p>"); //暂无相关信息！
