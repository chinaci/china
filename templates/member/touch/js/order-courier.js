...
$(function () {
    ...
    function getList(item) {
        ...
        // var grabtype = $('.tab .curr').attr('data-grabid') ? $('.tab .curr').attr('data-grabid') : '';
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=homemaking&action=orderList&state="+state+"&page="+atpage+"&pageSize="+pageSize+"&dispatchid="+dispatchid,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                if(data && data.state != 200){
                    if(data.state == 101){
                        ...
                        var list = data.info.list, pageInfo = data.info.pageInfo, html = [], coutresult = data.info.coutresult, durl = $(".tab ul").data("url"), rUrl = $(".tab ul").data("refund"), cUrl = $(".tab ul").data("comment");
                            ...
                            for(var i = 0; i < list.length; i++){
                                ...
                                switch(orderstate){
                                    ...
                                    case "2":
                                        ...
                                        if (grabtype==1) {
                                            btn = '<span data-dispatchid="'+dispatchidid+'" data-orderid="'+id+'" class="grab">抢单</span>';//抢单
                                    ...
                                    case "6":
                                        ...
                                        btn = '<span class="statement" data-id="'+id+'" >'+langData['homemaking'][10][35]+'</span>';//结单
                                        // if(aftersale==1){
                                ...
                                if(orderstate==11){
                                html.push(' <p class="jz_order_time"><span>'+langData['homemaking'][10][36]+'：</span><span>'+statementtime+'</span></p>');
                                }
                                if(homemakingtype==1){
                                ...
                                if (grabtype==1&&orderstate==2&&dispatchid==0) {
                                    html.push(' <p class="jz_order_time"><span>'+langData['homemaking'][6][3]+'：</span><span>'+useraddr+'</span></p>');//预约地址
                                }
                                html.push('</div>');
                                html.push('</a>');
                                html.push('</div>');
                        ...
                        //         var orderid       = coutresult[i].id,
                        //             proid         = coutresult[i].proid,
                        //             yuyue         = coutresult[i].yuyue,
                        //             follow        = coutresult[i].follow,
                        //             title         = coutresult[i].title,
                        //             num           = coutresult[i].num,
                        //             zong          = Number(coutresult[i].yuyue)+Number(coutresult[i].follow)
                        //             ;
                        //         spanHtml.push('<p class="aaaa">名称:'+title+';数量:'+num+';预约金额:'+yuyue+';后续收入:'+follow+';总收入:'+zong+'</p><br/>')
                        //     }
                        //     var spantext=spanHtml.join("");
                        //     $(spantext).prependTo($(".jz_data"));//prependTo(),将子元素追加到父级的最前面
    ...
    $('.jz_data').delegate('.grab','click',function(){
        ...
        $.ajax({
            ...
            data:{dispatchid:dispatchid,orderid:orderid},
            type: 'post',
            dataType: 'json',
            success: function(data){
                if(data.state == 100){
	...
	if(getParam('state')){
		$(".tab li[data-id='"+getParam('state')+"']").click()
	}
