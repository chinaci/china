$(function(){
  ...
  var djs = $('#djs em');
  var dataGeetest = "";
  var ftype = $('.tab-head li.active').attr('data-type');
  //手机 邮箱切换
  $('.tab-head li').click(function(){
    var t = $(this), index = t.index();
    t.addClass('active').siblings('li').removeClass('active');
    if(index == 0){//手机
      $('.pho').addClass('show').siblings('dd').removeClass('show');
      ftype = 'phone';
    }else{//邮箱
      $('.ema').addClass('show').siblings('dd').removeClass('show');
      ftype = 'email';
    }
  })
  // 文本框输入
  ...
  $('.inpbox input').on('input propertychange', function(){
    ...
    var emailVal = $('#email')
    if ((account.val() != "" && vdimgck.val() != "") ||(emailVal.val() != "")) {
            ...
            success: function(data){
                if(data && data.state == 100){
                   ...
                   if(listLen == 1 && codeArea == 86){//当数据只有一条 并且这条数据是大陆地区86的时候 隐藏区号选择
                        $('.areacode_span').hide();
                        $('.inpbox dd input').css('width','5.4rem')
                        return false;
                   }
  ...
  function sendPhoneVerCode(){
    ...
    if(ftype =='phone'){
      ...
      if (emVal == '') {
        alert(langData['siteConfig'][20][497]);//请输入邮箱账号
        return false;
      }
    ...
    if(ftype == "phone"){
      ...
      $.ajax({
        ...
        data: "vericode="+vericode+"&type=1&email="+$('#email').val()+dataGeetest,
        type: "GET",
        dataType: "jsonp",
        success: function (data) {
  ...
  if(!geetest){
    ...
    var handlerPopupFpwd = function (captchaObjFpwd) {
      ...
      captchaObjFpwd.onSuccess(function () {
        ...
        if(ftype == "email"){
          ...
          $.ajax({
            url: masterDomain+"/include/ajax.php?service=member&action=backpassword",
            data: "type=1&email="+$('#email').val()+dataGeetest,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
    ...
    $('.get-yzm').click(function(){
      ...
      var number   = $('#account').val();
      var emVal = $('#email').val();
      if (number == '' && ftype=='phone') {
        alert(langData['siteConfig'][20][166]);
        return false;
      }else if (emVal == '' && ftype=='email') {
        alert(langData['siteConfig'][20][497]);//请输入邮箱账号
        return false;
      }else {
        if(ftype == "email"){
