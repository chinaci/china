    ...
    success: function (data) {
      ...
      if(data && data.state != 200){
        if(data.state == 101){
          ...
          $("#state1").html(0);
          $("#state0").html(0);
          objId.append("<p class='loading'>"+langData['siteConfig'][21][64]+"</p>");
        }else{
          ...
          if(list.length > 0){
            for(var i = 0; i < list.length; i++){
              ...
              var tozj = '';
              if(entrustment == null){
                tozj = '<span>'+langData['siteConfig'][31][112]+'</span>';//被委托方不存在或状态异常
              }else if(list[i]['zjuid'] != '0'){
                tozj = ' <span><a href="'+entrustment.url+'" target="_blank">'+entrustment.nickname+'</a></span>';
              }else if(list[i]['zjcom'] != '0'){
                tozj = ' <span><a href="'+entrustment.domain+'" target="_blank">'+entrustment.title+'</a></span>';
              }

              html.push('<div class="item state'+istate+'" data-id="'+id+'">');
              html.push('  <p class="type">'+langData['siteConfig'][31][105]+'：'+typename+'</p>')//委托类型
              html.push('  <p class="type">'+langData['siteConfig'][31][104]+'：'+tozj+'</p>')//经纪人/中介公司
              ...
              if(type == 2){
                ...
                html.push(' <p class="title">'+langData['siteConfig'][6][203]+'：'+(price == 0 ? langData['siteConfig'][31][108] : price + (type == 0 ? langData['siteConfig'][31][111] : langData['siteConfig'][45][4]) )+'</p>');//报价 -- 未填写 --元/月 -- 万元
              }
