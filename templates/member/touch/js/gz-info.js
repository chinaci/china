...
$(function(){
    ...
    var gzAddress         = $(".gz-address"),  //选择地址页
        ...
        gzAddrInit = {
            ...
            ,getAddrArea: function(id){
                ...
                $.ajax({
                    url: masterDomain + "/include/ajax.php?service="+(window.modelType ? window.modelType : 'siteConfig')+"&action="+gzAction,
                    data: "type="+id+param,
                    type: "GET",
                    dataType: "jsonp",
                    success: function (data) {
                        if(data && data.state == 100){
                            var list = data.info, hotList = [],cityArr = [], areaList = [],html1 = [];
    ...
    gzAddrSeladdr.bind("click", function(){
        ...
        //已有默认数据
            ...
            gzSelAddrList.find('ul:last').show();
    ...
    //自动定位
    if(typeof HN_Location == 'object' && gzAddrSeladdr.attr('data-ids') == '' && gzAddrSeladdr.attr('data-action') != 'type'){
