$(function(){
  ...
  // 工作经历
  var experienceHtml = '<div class="experience"><div class="ml4r"><dl class="inpbox"><dt><span><label for="company">'+langData['siteConfig'][19][354]+'</label></span></dt><dd><input type="text" class="inp company" placeholder="'+langData['siteConfig'][20][274]+'" id="company"></dd></dl></div><div class="ml4r"><dl class="inpbox ex_date"><dt><span>'+langData['siteConfig'][19][138]+'</span></dt><dd><div data-role="fieldcontain" class="dom_select date1"><input autocomplete="off" class="demo-test-date" placeholder="2017-01-01"></div><font>'+langData['siteConfig'][13][7]+'</font><div data-role="fieldcontain" class="dom_select date2"><input autocomplete="off" class="demo-test-date" placeholder="2017-01-01"></div></dd></dl></div><div class="ml4r"><dl class="inpbox"><dt><span>'+langData['siteConfig'][19][139]+'</span></dt><dd><input type="text" class="inp bumen" placeholder="'+langData['siteConfig'][20][275]+'"></dd></dl></div><div class="ml4r"><dl class="inpbox"><dt><span>'+langData['siteConfig'][19][140]+'</span></dt><dd><input type="text" class="inp zhiwei" placeholder="'+langData['siteConfig'][20][276]+'"></dd></dl></div><div class="mulinpbox mb4r"><p class="multit">'+langData['siteConfig'][19][141]+'</p><div class="mulcon"><textarea placeholder="'+langData['siteConfig'][7][0]+'" class="textarea"></textarea></div></div><div class="exbtn"><a href="javascript:;" class="btn add">'+langData['siteConfig'][6][136]+'</a><a href="javascript:;" class="btn del">'+langData['siteConfig'][6][8]+'</a></div></div>';
...
//国际手机号获取
  ...
  $("#submit").bind("click", function(event){
    ...
    var cityid = 0, cityids = $('.gz-addr-seladdr').attr('data-ids');
    if(cityids){
