...
$(function(){
    ...
    update_zjuser_btn: function(type, need){
      ...
      if(refreshTopModule == 'house' && rtConfig.zjuserMeal.iszjuser == "1" && rtConfig.zjuserMeal.meal_check.state != 101){
        ...
        if(type == "refresh"){
          ...
          requestDate = $("#topPlanBeganObj").val();
          $("#topPlanEndObj").val($("#topPlanBeganObj").val());
           $('#topPlanEndObj').mobiscroll().date({minDate: new Date(requestDate), dateFormat: 'yyyy-mm-dd', onSelect: endonSelect});
        ...
        if(check_zjuser){
          ...
          $('.rtToppingType li:eq(1) span').text('普通置顶')
  ...
  var beganonSelect = function(valueText, inst){
      ...
      if(diffDays < houseTop){
        var endDate_ = getRtDate(beganDate, houseTop);
