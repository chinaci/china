...
//项目
  ...
  objId.delegate(".del", "click", function(){
          ...
          M.dialog = jqueryAlert({
              ...
              'buttons' :{
                  '是' : function(){
                      M.dialog.close();
                      $.ajax({
                          url: "/include/ajax.php?service=renovation&action=delArtilcle&id="+id,
                          type: "GET",
                          dataType: "json",
    ...
    function getList(tr) {
      ...
      isload = true;
      objId.append('<div class="loading">'+langData['siteConfig'][20][184]+'...</div>');//加载中，请稍候id

      var url ="/include/ajax.php?service=renovation&action=article&u=1&orderby=1&type="+Identity.typeid+"&fid="+Identity.id+"&state="+state+"&page="+orderpage +"&pageSize=10";
              ...
              if(data && data.state == 100){
                  ...
                  if(list.length > 0){
                    ...
                    var id= list[i].id;
                      ...
                      html.push('                <span class="or_num">'+langData['renovation'][14][96].replace('1',list[i].click)+'</span>');//1次浏览
                      html.push('              </dd>');
