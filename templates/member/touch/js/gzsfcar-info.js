...
$(function(){
    ...
    gzAddrSeladdr.bind("click", function(){
        ...
        //第一次点击
        if((ids == undefined && id == undefined) ||(ids == '' && id == '')){
    ...
    gzSelAddrList.delegate("li", "click", function(){
        ...
        if(id && addr){
            ...
            if(just == "1"){
                ...
                gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('has').attr("data-ids", ids.join(" ")).attr("data-id", id).html(addrname.join(" "));
                gzAddrInit.hideNewAddrMask();
                gzAddrSeladdrActive.siblings('.fakeTXT').css('display','block');
                gzAddrSeladdrActive.siblings('input').val(id);
           ...
           console.log('没点到')
            //gzAddrSeladdrActive.removeClass("gz-no-sel").addClass('has').attr("data-ids", '').attr("data-id", 0).html('不限');
            gzAddrInit.hideNewAddrMask();
            //gzAddrSeladdrActive.siblings('.fakeTXT').css('display','block');
            //gzAddrSeladdrActive.siblings('input').val(0);
        }
    });
