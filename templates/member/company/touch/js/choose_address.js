$(function(){
    ...
    if (site_map == "baidu") {
      ...
      myGeo.getPoint(addr, function(point){
        if (point) {
          ...
          if(isManager){
            setMap();
            showMsg.alert(langData['siteConfig'][45][92], 1000);//地址解析失败
          }
        }
      }, "");
    }
    ...
    // 腾讯地图
      ...
      function operation(){
        if(lng == '' || lat == ''){
          ...
          var keywrodsArr = [langData['siteConfig'][45][93][0],langData['siteConfig'][45][93][1],langData['siteConfig'][45][93][2],langData['siteConfig'][45][93][3],langData['siteConfig'][45][93][4],langData['siteConfig'][45][93][5],langData['siteConfig'][45][93][6],langData['siteConfig'][45][93][7],langData['siteConfig'][45][93][8],langData['siteConfig'][45][93][9],langData['siteConfig'][45][93][10],langData['siteConfig'][45][93][11],langData['siteConfig'][45][93][12],langData['siteConfig'][45][93][13],langData['siteConfig'][45][93][14],langData['siteConfig'][45][93][15],langData['siteConfig'][45][93][16]], idx = 0, list = [], has = [];
          searchService.searchNearBy(keywrodsArr[idx], new qq.maps.LatLng(lat, lng) , 1000);
          ...
          $("#address").keyup(function(){
            var t = $(this), v = t.val();
            if(v){
              v = v.replace(langData['siteConfig'][22][3], "");//号
  ...
  $(".submit").click(function(){
    ...
    if(addrid == "0" || addrid == ""){
      alert(langData['siteConfig'][44][40]);//请选择省市区
      return;
    }
    ...
    if(address == ""){
      alert(langData['siteConfig'][38][81]);//请填写详细地址
      return;
    }

    if(showMap && (lng == 0 || lat == 0) ){
      alert(langData['siteConfig'][44][39]);//请选择坐标
      return;
    }
