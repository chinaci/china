$(function(){
    var gzSelMask         = $(".gz-sel-addr-mask"),  //选择地区遮罩层
        ...
        gzAddrSeladdr     = $("#citybtn"),  //选择所在地区按钮
                    ...
                    success: function (data) {
                        if(data && data.state == 100){
                            ...
                            for (var i = 0, area, lower; i < list.length; i++) {
                                area = list[i];
                                var selarea = gzAddrSeladdr.val();
                                if(selarea != ''){
                                    if(area.typename == gzAddrSeladdr.val().split(' ')[0]){
                                        gzAddrSeladdr.attr('data-ids',area.id+' '+gzAddrSeladdr.attr('data-id'));
                                    }
                                }
    ...
    gzSelAddrList.delegate("li", "click", function(){
        ...
        if(id && addr){
            ...
            //没有下级
                ...
                gzAddrSeladdr.val(addrname.join(" "));
                $('#addr').val(id);
                gzAddrInit.hideNewAddrMask();
