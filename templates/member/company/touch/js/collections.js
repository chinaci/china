  ...
  objId.delegate(".del", "click", function(){
    ...
    if(id){
      if(confirm(langData['siteConfig'][20][211])){
        ...
        $.ajax({
          url: masterDomain+"/include/ajax.php?service=member&action=collect&module=job&temp=resume&type=del&id="+id,
          type: "GET",
          dataType: "jsonp",
        ...
        if(data.state == 101){
          ...
          if(list.length > 0){
            for(var i = 0; i < list.length; i++){
              if (list[i]['detail']) {
                var pic = hideFileUrl ? list[i]['detail']['company'].logoSource : list[i]['detail']['photo'];
                ...
                html.push('<h3>'+list[i]['detail'].name+'</h3>');
                html.push('<p class="grey">'+list[i]['detail']['home']+'</p>');
                html.push('<p class="grey area"><span>'+list[i]['detail']['college']+'</span><span>'+list[i]['detail']['educationalname']+'</span></p>');
                html.push('</div>');
                html.push('</a>');
                html.push('</div>');
              }else {
