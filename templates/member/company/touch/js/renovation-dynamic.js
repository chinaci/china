...
$(window).scroll(function() {
  ...
  function getList(tr) {
    ...
    var url ="/include/ajax.php?service=renovation&action=article&u=1&fid="+Identity.store.id+"&type="+Identity.store.typeid+"&orderby=1&state="+state+"&page="+orderpage +"&pageSize=5";
            ...
            if(data && data.state == 100){
                ...
                if(list.length > 0){
                  ...
                  for(var i=0;i<list.length;i++){
                    ...
                    html.push('                <em class="or_num">'+langData['renovation'][14][96].replace('1',click)+'</em>');//1次浏览
                    html.push('              </dd>');
                    html.push('            </a>');
                    html.push('          </dl>');
                    html.push('        </div>');
