$(function () {
    ...
    function getList(tr){
        ...
        $.ajax({
            url: masterDomain+"/include/ajax.php?service=marry&action=rentalList&u=1&orderby=5&state="+state+"&page="+atpage+"&pageSize="+pageSize,
            type: "GET",
            dataType: "jsonp",
            success: function (data) {
                isload = false;
                if(data && data.state == 100){
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html.push('<dd class="item-area"><em class="sp_room">'+list[i].duration+langData['marry'][5][37]+'</em><em class="sp_area">'+list[i].price+langData['marry'][5][36]+'</em></dd>');
                        html.push('<dd class="price"><em>'+echoCurrency('symbol')+'</em>'+list[i].price+'</dd>');
                        html.push('</a></dl>');
                        html.push('</div>');
