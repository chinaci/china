$(function(){
  ...
  $(".mapresults").delegate("li", "click", function(){
    ...
    //客服电话
    if($.trim(contact.val()) == "" || contact.val() == 0){
            alert(langData['siteConfig'][4][47]);
            return
    }
    ...
    //logo
    var logo = "", imgList = [];
    if($("#fileList1 li").length > 1 && $("#fileList1 .face").length == 0){
      $("#fileList1 li").eq(1).append('<div class="face on">LOGO</div>');
    }
    $("#fileList1 li").each(function(){
        var x = $(this),
            u = x.index(),
            islogo = x.children('.face').hasClass('on') ? 1 : 0,
            url = x.find('img').attr("data-val");
            des = x.find('img').attr("data-des");
        if (u > 0) {
          if(islogo){
            logo = url;
          }else{
            des = des ? des : '';
            imgList.push(url+'||'+des);
          }
        }
    })

    $("#logo").val(logo);
    $("#pics").val(imgList.join('###'));
  ...
  $('.welfareTags span').click(function(){
    var t = $(this);
    t.toggleClass('active');
    if(t.hasClass('active')){
      t.next('input').prop('disabled', false);
    }else{
      t.next('input').prop('disabled', true);
    }
  })
...
$(function(){
  uploaderSuccess()
  // $('#fileList1').delegate('.face', 'click', function(){
  //   $('#fileList1 .face').removeClass('on');
  //   $(this).addClass('on');
  // })
})
function uploaderSuccess(){
  var item = $("#fileList1 li");
  if(item.length <= 1){
    return;
  }
  item.find('.face').remove();
  item.eq(1).append('<div class="face on">LOGO</div>');
  return;

  // item.each(function(i){
  //   if(i > 0){
  //     var t = $(this), face = t.children('.face');
  //     if(face.length == 0){
  //       t.append('<div class="face">LOGO</div>');
  //     }
  //   }
  // })
  // if($("#fileList1 .face.on").length == 0){
  //   item.eq(1).children('.face').remove();
  //   item.eq(1).append('<div class="face on">LOGO</div>');
  // }
}
