			...
			if(data && data.state != 200){
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						objId.html('<table><thead class="thead"><tr><th class="fir"></th><th>'+langData['siteConfig'][19][382]+'</th><th style="padding-left: 30px;">'+langData['siteConfig'][19][385]+'</th><th>'+langData['siteConfig'][19][384]+'</th></tr></thead>'+html.join("")+'</table>');
					...
					$("#total label").html("&nbsp;&nbsp;（"+langData['siteConfig'][19][380]+" <strong style='color:#30992e;'>"+Number(pageInfo.totalAdd).toFixed(0)+"</strong>&nbsp;&nbsp;|&nbsp;&nbsp;"+langData['siteConfig'][19][381]+" <strong style='color:#b1000f;'>"+Number(pageInfo.totalLess).toFixed(0)+"</strong>）");
					showPageInfo();
				}
			}else{
				objId.html("<p class='loading'>"+langData['siteConfig'][20][126]+"</p>");
