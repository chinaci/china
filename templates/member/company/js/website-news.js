...
$(function(){
	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][211], function(){
				...
				$.ajax({
					url: masterDomain+"/include/ajax.php?service=website&action=delnews&id="+id,
					type: "GET",
					dataType: "jsonp",
	...
	$(".menuType").bind("click", function(){
		$.ajax({
			url: "/include/ajax.php?service=website&action=sitetype&act=article",
			type: "POST",
			dataType: "json",
			success: function(data){
				var content = [];
				...
				$.dialog({
					...
					ok: function(){
						...
						$.ajax({
							url: "/include/ajax.php?service=website&action=updateSiteType&act=article",
							data: "data=["+data.join(",")+"]",
							type: "POST",
							dataType: "json",
							success: function(data){
				...
				$('.menu-itemlist').delegate("a", "click", function(){
					...
					if(id != ""){
						$.dialog.confirm(langData['siteConfig'][27][128], function(){
							parent.remove();
							$.ajax({
								url: masterDomain+"/include/ajax.php?service=website&action=delSiteType&act=article&id="+id,
								dataType: "jsonp",
								success: function(data){
									if(data && data.state == 100){
...
function getList(is){
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=website&action=news&u=1&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
