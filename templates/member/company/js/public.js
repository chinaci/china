    ...
    $('.custom-nav').bind('click', function(){
        $.dialog({
            ...
            ok: function(){
				...
				//获取未勾选模块
				var hide = [];
				$('.popup_module_config input').each(function(){
					var t = $(this);
					if(!t.is(":checked")){
						hide.push($(this).val());
					}
			    ...
			    $.ajax({
			        ...
			        data: 'service=member&action=updateBusinessModule&hide=' + hide.join(','),
			        type: 'post',
			        dataType: 'json',
			        ...
			        error: function (res) {
			            alert(langData['siteConfig'][6][201], 2000, false); //网络错误，保存失败，请稍候重试！
			        }
			    })
