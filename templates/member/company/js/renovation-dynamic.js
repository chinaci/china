...
function getList(is){
	...
	var url ="/include/ajax.php?service=renovation&action=article&u=1&fid="+Identity.store.id+"&type="+Identity.store.typeid+"&orderby=1&state="+state+"&page="+atpage +"&pageSize="+pageSize;
	$(".pagination").hide();
                  ...
                  totalCount = pageInfo.totalCount;
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<p>'+langData['siteConfig'][19][394]+'：'+click+'</p>');
							html.push('</div>');
							html.push('</div>');
