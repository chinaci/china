	...
	objId.delegate(".del", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][20][211], function(){
				...
				$.ajax({
					url: masterDomain+"/include/ajax.php?service=business&action=delalbums&id="+id,
					type: "GET",
					dataType: "jsonp",
				...
				$('.menu-itemlist').delegate("a", "click", function(){
					...
					if(id != ""){
						$.dialog.confirm(langData['siteConfig'][20][211]+"<br />"+langData['siteConfig'][27][80], function(){
							parent.remove();
							$.ajax({
								url: masterDomain+"/include/ajax.php?service=business&action=delAlbumsType&id="+id,
								dataType: "jsonp",
								success: function(data){
									if(data && data.state == 100){
...
function getList(is){
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=business&action=albums&u=1&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<div class="photo"><a href="'+url+'" target="_blank"><i></i><img src="'+litpic+'" /></a></div>');
							html.push('<div class="name">'+typename+'</div>');
