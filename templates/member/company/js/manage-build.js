...
$(function(){
	...
	objId.delegate(".offShelf", "click", function(){
		...
		if(id){
			$.dialog.confirm(langData['siteConfig'][27][116], function(){//确定要下架这个商品吗？
				t.siblings("a").hide();
				t.addClass("load");

				$.ajax({
					url: masterDomain+"/include/ajax.php?service=build&action=offShelf&id="+id,
					type: "GET",
					dataType: "jsonp",
	...
	objId.delegate(".upShelf", "click", function(){
		...
		if(id){
			...
			$.ajax({
				url: masterDomain+"/include/ajax.php?service=build&action=upShelf&id="+id,
				type: "GET",
				dataType: "jsonp",
				success: function (data) {
					if(data && data.state == 100){
...
function getList(is){
	...
	$.ajax({
		url: masterDomain+"/include/ajax.php?service=build&action=blist&u=1&orderby=5&state="+state+"&page="+atpage+"&pageSize="+pageSize,
		type: "GET",
		dataType: "jsonp",
		success: function (data) {
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							if(sta == "1"){
								html.push('<a href="javascript:;" class="offShelf"><s></s>'+langData['siteConfig'][19][558]+'</a>');//下架
							}else if(sta == "2"){
								html.push('<a href="javascript:;" class="upShelf"><s></s>'+langData['siteConfig'][26][166]+'</a>');//上架
							}
							html.push('</div>');
							...
							if(sta == "0"){
								...
								arcrank = '&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][26][167];//销售中
							}else if(sta == "2"){
								arcrank = '&nbsp;&nbsp;·&nbsp;&nbsp;<span class="red">'+langData['siteConfig'][19][557]+'</span>';//已下架
							}
