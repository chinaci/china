	...
	$(".menuType").bind("click", function(){
		$.ajax({
			...
			success: function(data){
				...
				$.dialog({
					id: "ManageType",
					title: langData['siteConfig'][26][182],
					content: content,
					width: 360,
					ok: function(){
				...
				if(data.state == 101){
					...
					if(list.length > 0){
						...
						for(var i = 0; i < list.length; i++){
							...
							html.push('<p>'+langData['siteConfig'][19][393]+'：'+typeName+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][19][428]+'：'+price+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['shop'][3][16]+'：'+sale+'&nbsp;&nbsp;·&nbsp;&nbsp;'+langData['siteConfig'][19][477]+'：'+pubdate+'</p>');
							html.push('</div>');
							html.push('</div>');
