$(function(){
	...
	$('.appMapBtn').attr('href', OpenMap_URL);
      ...
      $('.follow-wrapper').click(function(){
        ...
        if(type == 'add'){
            ...
            t.find('.text-follow').html('已收藏');
        }else{
            t.find('.follow-icon').removeClass('active')
            t.find('.text-follow').html('收藏');
        }
    ...
    $(".userInfo ul li a").on("click",function(){
        ...
        if(href.indexOf('javascript') >= 0){
            ...
            $('.userInfo .userCon').eq(i).addClass("show").siblings().removeClass("show");
        }
    })
     // 用户评论
    $(".shopInfo ul li a.userCom").on("click",function(){
        $(this).parents().addClass("curr").siblings().removeClass("curr");
    ...
    // 优惠信息
    $(".shopInfo ul li a").on("click",function(){
        $(this).parents().addClass("scurr").siblings().removeClass("scurr");
        var i = $(this).parents().index();
        $('.shopInfo .userCon').eq(i).addClass("show").siblings().removeClass("show");
    })
    var slideHeight1 = 240;
    var defHeight1 = $('.conInfo .mainInfo .m-ul1').height();
    if(defHeight1 >= slideHeight1){
        $('.conInfo .mainInfo .m-ul1').css('height' , slideHeight1 + 'px');
        $('.more1').append('<a href="javascript:;">查看其他团购内容<i class="rmDown"></i></a>');
        $('.more1 a').click(function(){
            var curHeight1 = $('.conInfo .mainInfo .m-ul1').height();
            if(curHeight1 == slideHeight1){
                $('.conInfo .mainInfo .m-ul1').animate({height: defHeight1}, "normal");
                $('.more1 a').html('全部收起<i class="rmUp"></i>');
                $('.gradient').fadeOut();
            }else{
                $('.conInfo .mainInfo .m-ul1').animate({height: slideHeight1}, "normal");
                $('.more1 a').html('查看其他团购内容<i class="rmDown"></i>');
                $('.gradient').fadeIn();
            }
            return false;
        });
    }
    var slideHeight2 = 240;
    var defHeight2 = $('.conInfo .mainInfo .m-ul2').height();
    if(defHeight2 >= slideHeight2){
        $('.conInfo .mainInfo .m-ul2').css('height' , slideHeight2 + 'px');
        $('.more2').append('<a href="javascript:;">查看更多商品<i class="rmDown"></i></a>');
        $('.more2 a').click(function(){
            var curHeight2 = $('.conInfo .mainInfo .m-ul2').height();
            if(curHeight2 == slideHeight2){
                $('.conInfo .mainInfo .m-ul2').animate({height: defHeight2}, "normal");
                $('.more2 a').html('全部收起<i class="rmUp"></i>');
                $('.gradient').fadeOut();
            }else{
                $('.conInfo .mainInfo .m-ul2').animate({height: slideHeight2}, "normal");
                $('.more2 a').html('查看更多商品<i class="rmDown"></i>');
                $('.gradient').fadeIn();
            }
            return false;
        });
            ...
            success: function(data){
                if(data && data.state == 100){
                    ...
                    for(var i = 0; i < list.length; i++){
                        ...
                        html.push('          <a href="comdetail-'+d.id+'.html" class="link">');
                        ...
                        html.push('                <a href="comdetail-'+d.id+'.html"><span class="comment"><i></i><em>评论</em></span></a>');
                        html.push('            </div>');
                        html.push('        </div>');
                        html.push('    </div>');
                        html.push('</li>');
