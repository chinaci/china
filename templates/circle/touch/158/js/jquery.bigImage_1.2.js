    ...
    $.fn.bigImage = function (options) {
        ...
        isImgLoad(function(){
            ...
            for(var i=0 ; i<len; i++){
                ...
                var imgs = new Image(); 
                imgs.src = src;
				imgs.setAttribute('data-id', i);
                imgs.onload = function(){
                    var w = this.width, h = this.height;
                    var imgid = this.getAttribute('data-id');
					$('#bigImageItem_' + imgid)
						.attr('href', this.src)
						.attr('data-size', (w*5)+'x'+(h*5))
						.attr('data-med', this.src)
						.attr('data-med-size', w+'x'+h);
					$('#bigImageItem_' + imgid).find('img').attr('src', this.src);
                }
				div.append('<a id="bigImageItem_'+i+'"><img /></a>');
            }
        });
