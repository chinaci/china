...
$(function(){
	...
	$('.tab_ul li').click(function(){
		...
		if(!t.hasClass('on_chose')){
			...
			$('.tab_con>ul').eq(index).addClass('show').siblings('ul').removeClass('show');
			if($('.tab_con .show').find('li').length==0){
				getlist($('.tab_con .show'));
			}else{
				console.log('已经加载过')
			}
  	...
  	//调起原生应用
	$('.tab_con').delegate('.li_box .dt_detail','click',function(e){
	...
	$(window).scroll(function(){
		var top = $('.tab_con').offset().top;
		var wsc = $(window).scrollTop();
		if(top<=wsc){
			$('.tab_ul').addClass('fixedTop');
		}else{
			$('.tab_ul').removeClass('fixedTop');
		}

		if (wsc >= windowTop) { //当B>A时，表示页面在向上滑动
			//需要执行的操作
			windowTop = wsc;
			$('.bottom_join').hide();
		...
		console.log($(window).scrollTop() >= (scroll+20))
	...
	var scroll = 0,
		pHeight = 0;
	// 评论
	$('body').delegate('.comt_btn,.commt_btn', 'click', function() {
	...
	// 回复评论
	$('body').delegate('.comt_li','click',function(){
	...
	$('body').delegate('.btn_group .r_btn', 'click', function() {
		...
		var name = t.parents('.li_box').find('.r_info h4').text();
	...
	function getlist(p) {
		isload = 1;
		var page = $('.tab_ul .on_chose').attr('data-page');
		var orderby = $('.tab_ul .on_chose').attr('data-order');
			...
			success: function(data) {
				if (data.state == 100) {
					...
					for (var i = 0; i < datalist.length; i++) {
						...
						if (p == $('.recgz_ul')) { //关注板块的
							list.push('<div class="rec_tip"><p>'+langData['circle'][3][26]+'</p></div>')
						}

						/* 作者信息s */
						list.push('<div class="art_ib"><a href="'+masterDomain+'/user/'+d.userid+'" class="left_head">');
						if (d.photo) {
								var phonimg = d.photo;
							}else{
								var phonimg = "/static/images/noPhoto_40.jpg";
							}
						list.push('<img src="' + phonimg + '" />');
						if(d.usertype=='2'){
							list.push('<div class="v_log"><img src="' + templets_skin + 'upfiles/vip.png" /></div>');
						}
						list.push('</a><div class="r_info vip_icon">');
						list.push('<h4><span class="artname">' + (d.username) + '</span><div class="lBox level_'+level+'"><i></i><span>'+levelname+'</span></div></h4>');

						list.push('<p class="pub_time">' + d.pubdate1 + '</p></div></div>');
						/* 作者信息e */
						list.push('<div class="art_con"><a href="' + d.url + '" class="dt_detail">' + (d.content ? "<h2>" + d.content +	"</h2>" : ""));
						//图片内容
						if (d.media.length == 2 || d.media.length == 4) {
							list.push('<div class="img_box fn-clear">');
							for (var m = 0; m < d.media.length; m++) {
								list.push('<div class="img_item d_img"><img src="' + d.media[m] + '"></div>');
							}
							list.push('</div>');
							...
							for (var l = 0; l < imglen; l++) {
								list.push('<div class="img_item m_img"><img src="' + d.media[l] + '"></div>');
							}
							list.push('</div>');
							...
							list.push('<div class="img_item v_img"><img src="' + d.media[0] + '"></div>')
							list.push('</div>');
						}else if(d.media.length==0 && d.videoadr!=''){
							list.push('<div class="img_box fn-clear">');
							var img_v = (d.thumbnail=="")?templets_skin+'images/sv_icon.png':d.thumbnail
							list.push('<div class="img_item h_img video_item"><img data-url="'+img_v+'" src="'+img_v+'"></div>')
							list.push('</div>');
						}
						list.push('</a>')

						// 链接
                        if( d.commodity!='' && d.commodity!=null){
						...
						// 定位
                        if(d.addrname){
                           list.push('<a href="#" class="posi_link">'+d.addrname+'</a>');
                           }

						// 点赞按钮
						list.push('<div class="btn_group"><a href="javascript:;" class="r_btn"></a>');
						list.push('<div class="left_btns"><a href="javascript:;" class="comt_btn">' + (d.reply > 0 ? d.reply : "") + '</a>');
						if(d.isdz == '1'){
						...
						if (d.up != 0 || d.reply != 0) {
							...
							if (d.up != 0) {
								...
								for (var a = 0; a < (d.dianres.length>8?8:(d.dianres.length)); a++) {
									list.push('<li class="zan_li"> <a href="'+masterDomain+'/user/'+ d.dianres[a].id +'#circle"><img src="' +  d.dianres[a].photo + ' " onerror="this.src= \'/static/images/noPhoto_100.jpg\'"></a> </li>')
								}
							...
							if (d.reply != 0) {
								...
								for (var n = 0; n < d.lastReply.length; n++) {
									list.push('<li class="comt_li" data-plid = "'+d.lastReply[n].id+'"> <a href="#">'+d.lastReply[n].nickname+'</a>：'+d.lastReply[n].content+' </li>')
								}
								if (d.lastReply.length == "3") {
									list.push('<a href="'+d.url+'" class="comt_detail">'+langData['circle'][3][35].replace("1",d.reply)+'></a>');   //全部1条评论
								}
					...
					setTimeout(function() {
						isload = 0;
						page++;
						$('.tab_ul li.on_chose').attr('data-page',page)
