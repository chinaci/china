...
ComplexCustomOverlay.prototype.initialize = function(map){
  ...
  div.style.background = "url("+skin+"images/Map/"+zb_style_arr[zb_style].img+") no-repeat";
	...
	onSearchComplete: function(results){
		if (local.getStatus() == BMAP_STATUS_SUCCESS){
			...
			for (var i = 0; i < results.getCurrentNumPois(); i ++){
				...
				results.getPoi(i).distance = (map.getDistance(mPoint,p1)).toFixed(0)+'m';
				var TPL=$('#tp').html().replace(/[\n\t\r]/g, '');
				html.push(Mustache.to_html(TPL, results.getPoi(i)));

			}
			$('#r-result').html(html.join(""));
			setTimeout(function(){
          ...
          $('html,body').animate({
            scrollTop: $('.tab-container').offset().top - $('.fixedpane').height()
          },300)
				});
			},200);
		}else{
			$('#r-result').html('<li class="loading">没有相关结果</li>');
		}
	}
};
