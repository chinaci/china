...
//评论登录
	...
	function getCommentList(list){
		...
		for(var i = 0; i < list.length; i++){
			...
			html.push('      <span class="c-name"  data-id="'+list[i].user['userid']+'">'+list[i].user['nickname']+'</span>');
	...
	$(".comment").delegate(".subtn", "click", function(){
		var t = $(this), cid = t.closest(".tab-content").attr("data-id");
		...
		$.ajax({
			...
			success: function (data) {
				...
				if(data && data.state == 100){
					...
					list.push('      <span class="c-name"  data-id="'+info.userinfo['userid']+'">'+info.userinfo['nickname']+'</span>');
